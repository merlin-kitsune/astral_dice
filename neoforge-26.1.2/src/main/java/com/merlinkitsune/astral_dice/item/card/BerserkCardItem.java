 ...
 *
 * <p><b>26.1.2 移植说明</b>：本批（效果牌「手持即选择」）与 1.21.1 基准逐字同形 —— 只做
 * 「注册选择器动作 + 覆写 {@code selectorActionId()}` + 删除旧的 {@code canUseOnOtherPlayers()} 覆写」，
 * 无任何平台 API 触点；{@code MobEffects} 在本牌里不涉及（用的是自定义效果 {@code ModEffects.BERSERK}）。
 */
