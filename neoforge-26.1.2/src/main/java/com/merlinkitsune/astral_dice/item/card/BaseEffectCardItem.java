 ...
 * <ol>
 *   <li>{@code Item#use} 在 26.1.2 返回 {@link InteractionResult}（1.21.1 为
 *       {@code InteractionResultHolder<ItemStack>}）⇒ {@code use}/{@code useViaSelector} 的返回类型与
 *       「客户端成功 = {@code SUCCESS} / 服务端成功 = {@code SUCCESS_SERVER}」写法按本线既有形态书写；</li>
 *   <li>{@code Level#isClientSide} 是**方法**（{@code isClientSide()}），不再是字段；</li>
 *   <li>客户端 actionbar 文案用 {@code Player#sendOverlayMessage(Component)}
 *       （1.21.1 的 {@code displayClientMessage(text, true)} 在 26.1.2 已改名）；</li>
 *   <li>背包遍历用 {@code Inventory#getNonEquipmentItems()}（1.21.1 的 {@code Inventory#items} 字段已删），
 *       见 {@link #findHeldCard};</li>
 *   <li>服务端 → 客户端发包仍走 {@code PacketDistributor.sendToPlayer}（26.1.2 只拆了 clientbound 方向）；</li>
 *   <li>其余（{@code ItemStack#consume(int, LivingEntity)} / {@code ItemStack#isSameItemSameComponents} /
 *       {@code ItemStack#getItem()} / {@code Player#getMainHandItem()} / {@code Component.translatable} /
 *       {@code TargetSelectionRegistry#register} / {@code GameplayConstants.ACTIONBAR_DURATION_TICKS}）
 *       在 26.1.2 逐字可用。</li>
 * </ol>
 */
public abstract class BaseEffectCardItem extends Item {
    ...
    private static final class SelectorAction implements TargetSelectionAction, SelfTargetable, HoldToSelect {
        ...
        public void apply(ServerPlayer player, LivingEntity target) {
            ...
            live.consume(1, player);
        }
    }

    /** 在玩家背包(含快捷栏/副手/盔甲槽)里找出与快照同一物品、同一数据组件的那个非空栈 */
    private static ItemStack findHeldCard(Player player, ItemStack snapshot) {
        // 26.1.2 平台差异:Inventory#items 字段已删,改用 getNonEquipmentItems()
        for (ItemStack candidate : player.getInventory().getNonEquipmentItems()) {
            if (!candidate.isEmpty() && ItemStack.isSameItemSameComponents(candidate, snapshot)) {
                return candidate;
            }
        }
    ...
    /** 目标选择器路径的服务端/客户端反馈(actionbar,沿用本模组的统一发送通道) */
    ...
    public InteractionResult interactLivingEntity(ItemStack stack, Player player, LivingEntity target, InteractionHand hand) {
        ...
        if (selectorActionId() != null) {
            return useViaSelector(player.level(), player, stack).consumesAction()
     ...
     */
    private InteractionResult useViaSelector(Level level, Player player, ItemStack stack) {
        if (level.isClientSide()) {
            return isBlockedOnClient(player, stack) ? InteractionResult.FAIL : InteractionResult.SUCCESS;
        }
