...
public final class TemporaryCardUtil {
     ...
     *  卡牌物品可堆叠({@code stacksTo(64)}),且同 id 的临时牌组件相同 ⇒ {@code Inventory#add} 走合并分支
     ...
     *       {@code min(}{@value #GRANT_BATTLE_COUNT}{@code , budget)} 张战斗牌(攻击 + 防御混合池,允许两张同类);</li>
    ...
    public static int countTemporary(Player player) {
        ...
        if (isTemporary(player.getInventory().getItem(Inventory.SLOT_OFFHAND))) {
            count += player.getInventory().getItem(Inventory.SLOT_OFFHAND).getCount();
        }
        return count;
    }
    ...
    private static int purgeInventory(Player player) {
        ...
        // 26.1.2 平台差异:副手字段已删 ⇒ 经 Container 索引读写(SLOT_OFFHAND 落在 EntityEquipment 上)
        ItemStack offhandStack = player.getInventory().getItem(Inventory.SLOT_OFFHAND);
        if (isTemporary(offhandStack)) {
            removed += offhandStack.getCount();
