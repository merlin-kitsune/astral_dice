...
public class BonnieSignItem extends BaseSignItem {
    ...
    // `neoforge-1.21.1/.../item/sign/BonnieSignItem.java` 第 57-94 行(静态块)**逐字移植**,
    ...
    // bonnie_undercover 动作的 apply 在确认时写入)。
    // 语义基准 = 1.21.1 `BonnieSignItem.java` 第 108-115 行(逐字同形,纯平台无关代码)。
    // ⚠️ 本批次为**叠加**门控:下方 READY_TYPE / sign_ready_type 旧待命等待器机制按用户要求保持原样,
    //    门控生效后本类 handleUse 不再被 performSkill 到达(该次主动改由 bonnie_undercover 的 apply 施加)。
    @Override
