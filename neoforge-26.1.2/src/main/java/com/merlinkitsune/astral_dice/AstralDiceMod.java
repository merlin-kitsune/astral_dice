...
public class AstralDiceMod {
    ...
    public AstralDiceMod(IEventBus modEventBus, ModContainer modContainer) {
        ...
        // CardInventoryScreen 继承 AbstractContainerScreen、签名里带 net.minecraft.client.gui.screens.Screen,
        // 属纯客户端类;而本类是**主模组入口,专用服务端同样会加载**。
        // 在此处用 `FMLEnvironment.getDist() == Dist.CLIENT` 分支守卫只能拦住**执行**,拦不住**符号解析**:
        // `this::registerScreens` 会在本类常量池生成一个 BootstrapMethods 项
        // (`REF_newInvokeSpecial CardInventoryScreen."<init>"`),而 invokedynamic 的引导方法参数
        // 是**按类初始化时解析**的(JVMS §5.4.3.6)⇒ 专用服务端一加载本类就 NoClassDefFoundError。
        // 故注册点必须整体搬到**永不加载于服务端**的类:
    ...
    private void onCommonSetup(FMLCommonSetupEvent event) {
        ...
        com.merlinkitsune.astral_dice.init.ModCompatibilityCheck.verifyOrThrow();
        event.enqueueWork(() -> {
