 ...
 * 计数源 = **主物品栏** {@code player.getInventory().items},与
 * {@code item/chip/SmartWatchChipItem#countCards} 同口径 ⇒ 放进普通容器后不再计入);
 * 层数只由 {@code item/card/HuoCardItem} 的玩家级 tick 按实际张数镜像,
 * 与「每 2:00 周期伤害」的计时器**完全解耦**(计时器见
 * {@code ModAttachments#HUO_CARD_NEXT_DAMAGE_TICK}:张数在 >0 区间的增减不改写它,
 * 伤害取结算时刻的张数)。
 *
...
public class MisfortuneEffect extends MobEffect {
    ...
    public static int getStacks(Player player) {
        ...
        MobEffectInstance instance = player.getEffect(ModEffects.MISFORTUNE);
    ...
    public static void mirror(Player player, int count) {
        ...
        ModEffectRemoval.remove(player, ModEffects.MISFORTUNE);
        if (count > 0) {
            player.addEffect(new MobEffectInstance(ModEffects.MISFORTUNE,
