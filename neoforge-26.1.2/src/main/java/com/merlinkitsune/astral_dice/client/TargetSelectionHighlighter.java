 ...
 * </ul>
 *
 * <p>「已指向」判定（{@link TargetSelectionClient} 的准星射线）与渲染**同源**：两处都走
 * {@link #hitFrameBox}（= {@link #borderBox} + 命中档线宽）—— 2026-09-19 用户要求
 * 「只需指向目标外框范围即视为指向，不必完全对准目标本身」，故判定盒必须与描边几何一致，
 * 否则会出现「看着在框里、却点不中」的观感缺陷。
 *
...
public final class TargetSelectionHighlighter {
     ...
     * 暴露成包内方法供判定复用。
     *
     * <p>26.1.2 移植：与 1.21.1 基准逐字同形（纯几何 + {@link TargetOutlineCapture#outlineOf}，
     * 无版本相关 API）；调用方 {@code client/TargetSelectionClient#pickFrameTarget} 用
     * {@code AABB#contains(Vec3)}（视点已在盒内 ⇒ 距离 0）与 {@code AABB#clip(Vec3, Vec3)} 取最近命中。
     */
