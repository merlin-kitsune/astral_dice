...
public final class DiceCombatModifiers {
    ...
    static {
        ...
        registerDefenseModifier((ctx, dp) -> {
            var unwavering = ctx.target.getEffect(ModEffects.UNWAVERING.get());
            if (unwavering != null) {
        ...
        registerDefenseModifier((ctx, dp) -> {
            ...
            var curios = CuriosCompat.getCuriosInventory(tp);
            if (curios.isPresent()) {
                var r = curios.get().findFirstCurio(s -> s.is(ModItems.PADMAN_SIGN.get()));
                if (r.isPresent()) {
        ...
        registerDefenseModifier((ctx, dp) -> {
            if (ctx.target.level().isClientSide()) return dp;
            if (!(ctx.target instanceof Player tp)) return dp;
            boolean active = tp.getHealth() <= tp.getMaxHealth() / 2.0f || tp.hasEffect(ModEffects.PAPARA_BITE.get());
