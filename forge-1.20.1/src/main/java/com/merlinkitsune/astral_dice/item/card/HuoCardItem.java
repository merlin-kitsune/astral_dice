 ...
 * <h2>持有代价(厄运)</h2>
 * <ul>
 *   <li><b>厄运效果层数 == 持有张数</b>:由 {@link #tick} 每 20 tick 把背包(主背包 + 副手)里的
 *       符卡-祸总张数镜像到 {@code astral_dice:misfortune}(张数 0 ⇒ 无该效果);</li>
 ...
 *       ⇒ 张数增减**不影响计时器**(计时器与结算分离)。</li>
 * </ul>
 *
 * <p><b>2026-09-20 变更</b>:原先的「持有期间禁止丢弃」(Q 键丢弃被拒 / 拖出 GUI 兜底退还 /
...
public class HuoCardItem extends BaseEffectCardItem {
     ...
     *
     * <p>口径 = 主物品栏 {@code player.getInventory().items}(与
     ...
     * ⚠️ 1.20.1 的 {@code Inventory#getContainerSize()} 含护甲与副手槽 ⇒ 本方法**不得**用它遍历
     * (否则副手既被计入、又会被下面的副手读取重复计一次);只遍历 {@code items} + 单独读一次副手,
     * 与 1.21.1 逐字同形。
     */
    ...
    /**
     * 移除背包(主背包 + 副手)里的**全部**符卡-祸,返回移除的总张数。
     ...
     * 之后**只做 {@code next += 2:00} 的推进**,不因为张数变化重新起算;张数归 0 时
     * 按 §9.2 冻结口径**整段清除**(清效果 + 计时器归 0 ⇒ 下一次持有时重新起算),
     * 故「N&gt;0 区间内的张数增减不影响计时器」。
     *
    ...
    public static void tick(Player player) {
        ...
        if (count <= 0) {
            // N=0:既无该效果、也无周期伤害;计时器一并归 0(下一次持有时重新起算;§9.2 冻结口径)
            MisfortuneEffect.clear(player);
            if (next != 0L) {
