...
public class FenSignItem extends BaseSignItem {
    ...
    protected boolean startActiveLockOnUse(Player player, long now) {
        net.minecraft.world.effect.MobEffectInstance instance = player.getEffect(ModEffects.FEN_FRENZY.get());
    ...
    protected boolean isGateEffectActive(Player player) {
        return player.hasEffect(ModEffects.FEN_FRENZY.get());
    }
    ...
    /**
     * 养精蓄锐 +{@code amount} 层(佩戴立牌且未满上限时;唯一加点入口)。
     *
     * <p>复用既有的养精蓄锐附件计数({@code fen_recharge}),**不新建效果/计数器**。
     * 调用方:
     * <ul>
     *   <li>使用治疗类效果牌(既有被动);</li>
     *   <li>风水师立牌(zhao)被动「完美帮手」:对装备本立牌者施加「白泽赐福」时 +1 层
     *       (见 {@code ZhaoSignItem} 的主动动作 apply)。</li>
     * </ul>
     */
    public static void addRecharge(Player player, int amount) {
        if (player == null || amount <= 0) return;
        if (player.level().isClientSide()) return;
        if (!isEquipped(player)) return;
        int current = ModAttachments.getFenRecharge(player);
        if (current >= MAX_RECHARGE) return;
        ModAttachments.setFenRecharge(player, Math.min(MAX_RECHARGE, current + amount));
    }
