 ...
 * (1.20.1 由 {@code AstralData#onPlayerClone} 的死亡保留白名单实现,对应 1.21.1 的
 * {@code AttachmentType.Builder#copyOnDeath()})。本效果纯粹是可丢弃、可重建的**显示载体**。
 *
 ...
 * 自 {@code textures/item/sherry_sign.png} 逐字节复制)。**两线逐字同形,仅注册项类型不同**
 * (本线 {@code ModEffects.SHERRY_REASONING} 是 {@code RegistryObject},故一律加 {@code .get()})。
 */
public class SherryReasoningEffect extends MobEffect {
    ...
    public static int getStacks(Player player) {
        ...
        MobEffectInstance instance = player.getEffect(ModEffects.SHERRY_REASONING.get());
    ...
    public static void mirror(Player player, int count) {
        ...
        ModEffectRemoval.remove(player, ModEffects.SHERRY_REASONING.get());
        if (count > 0) {
            player.addEffect(new MobEffectInstance(ModEffects.SHERRY_REASONING.get(),
