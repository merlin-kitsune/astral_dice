 ...
 *
 * <p>故条件源改为 {@link RenShieldVisibility} —— 两线共用的镜像类,由
 * {@code ModNetwork.RenShieldStateMessage} 维护(服务端在护盾状态变化时全服广播、玩家登录时补发
 * 全量)。另注:1.20.1 的玩家附件同步同样只发给本人({@code ModNetwork.syncAttachment} →
 * {@code sendToPlayer}),也不能用作此条件。
 *
