 ...
 *
 * <p>语义与 1.21.1 侧 {@code StarCoinWalletButtons} 逐字等价，平台差异只有四处：
 * 事件注解（{@code @Mod.EventBusSubscriber}）、{@link ResourceLocation} 的构造方式、
 * {@code GuiGraphics#blit} 多一个 {@code blitOffset} 参数，以及走 {@link ModNetwork}
 * 的 {@code SimpleChannel} 发 C2S 意图包。
 *
 * <p>不建独立界面：在 {@link ScreenEvent.Init.Post} 时给 {@link InventoryScreen} 与
 * {@link CreativeModeInventoryScreen} 各挂一次；点击只发意图包（C2S），
 * 一切判定在服务端（见 {@link StarCoinWalletActions}）。
 *
 * <p><b>视觉与布局对齐 Magic Coins + SG-Economy 的同类界面</b>（尺寸/坐标逐像素一致），
 * 但**贴图全部自绘**：余额条 96x24 深色圆角面板（右端右对齐显示余额）、钱包按钮 18x20 裸美术、
 * 星币/星币袋按钮 13x13 带原版槽位风格边框（中心 9x9 放货币图标），三个按钮各带一张
 * {@code _highlighted} 悬停贴图（钱包 = 暖金描边光晕，取款按钮 = 描边/填充转蓝），不用白色蒙版。
 *
 * <p>基座坐标**生存与创造各一套**：
 * <pre>
 *   控件              尺寸    生存 (x, y)   创造 (x, y)
 ...
 * </pre>
 * 余额数字**右对齐**，右端固定在「钱包按钮 x + 88」，y 为「钱包按钮 y + 7」。
 * 玩家可用配置项再叠加 x/y 偏移；余额条与数字跟随钱包按钮的偏移一起动。
 *
...
public final class StarCoinWalletButtons {
    ...
    private static final ResourceLocation WALLET_HOVER_TEXTURE =
            new ResourceLocation(AstralDiceMod.MODID, "textures/gui/star_coin_wallet_button_highlighted.png");
    ...
    private static final ResourceLocation COIN_HOVER_TEXTURE =
            new ResourceLocation(AstralDiceMod.MODID, "textures/gui/star_coin_button_highlighted.png");
    /** 星币袋兑换按钮 —— 同上，按袋取。 */
    ...
    private static final ResourceLocation BAG_HOVER_TEXTURE =
            new ResourceLocation(AstralDiceMod.MODID, "textures/gui/star_coin_bag_button_highlighted.png");
    /** 余额条底图（深色圆角面板）。 */
    private static final ResourceLocation BALANCE_BAR_TEXTURE =
            new ResourceLocation(AstralDiceMod.MODID, "textures/gui/star_coin_wallet_bar.png");
    ...
    /**
     * 跟随父界面定位的图标按钮（常态 / 悬停各一张贴图，不用白色蒙版）。
     *
    ...
    private static final class WalletButton extends AbstractButton {
        ...
        protected void renderWidget(GuiGraphics guiGraphics, int mouseX, int mouseY, float partialTick) {
            ...
            if (withBalanceBar) {
                ...
                guiGraphics.blit(BALANCE_BAR_TEXTURE, getX() - BAR_INSET, getY() - BAR_INSET, 0,
            ...
            guiGraphics.blit(hovered ? hoverTexture : texture, getX(), getY(), 0, 0.0F, 0.0F,
