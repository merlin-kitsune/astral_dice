 ...
 *
 * <p>**只有一行**（2026-09-17 用户裁决，与 1.21.1 逐条对等）：当前准星目标的「目标名 + 距离」，
 * 按敌我着色；无有效目标时**不画任何东西**（旧版的状态行「目标选择中」、无目标闪烁行与操作提示行
 * 已全部移到 actionbar，见 {@link TargetSelectionClient} 的每 tick 提示刷新）—— 中央 HUD 不再与
 * actionbar 争夺同一片视线区域。仅在选择激活时渲染；状态切换记录调试日志（禁止每帧日志）。
 */
