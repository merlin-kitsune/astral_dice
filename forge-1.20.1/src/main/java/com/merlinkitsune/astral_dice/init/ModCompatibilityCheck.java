 ...
 * Forge 的装载状态机顺序为「CONSTRUCT（GATHER）→ CONFIG_LOAD（LOAD）→ COMMON_SETUP」，
 * 故构造阶段执行 {@code ConfigValue#get()} 会抛「配置尚未加载」
 * （{@code ModCommonConfig#snapshot()} 的 javadoc 亦据此要求调用者先完成注册）。
 * 改挂在 {@code FMLCommonSetupEvent}（位于 CONFIG_LOAD 之后）后，**提示上屏的链路完全不变**：
 * {@code ModContainer.buildTransitionHandler} 把监听器抛出的异常记入 future，
 * {@code ModList.completableFutureFromExceptionList} 将其挂到汇聚异常的 suppressed 列表上，
 * {@code ModLoader.waitForTransition} 便会挑出所有 {@link ModLoadingException} 并组成
 * {@code LoadingFailedException} ⇒ 逐条上屏。对象仍是本类的 {@link ModLoadingException}，文案不变。
 * ⚠️ 同理**不要**改挂 {@code ModConfigEvent} —— 该事件在 CONFIG_LOAD 阶段触发，而该阶段的转移是
 * {@code InlineTransition}，走 {@code ModLoader.handleInlineTransition}（**没有 try/catch**），
 * 异常会直接冒出去退化成上游的「There was a crash during startup」，本模组的提示就消失了。
 *
 * <p><b>提示为什么能上屏</b>：{@code LoadingErrorScreen} 逐条渲染
 * {@code ModLoadingException#formatToString()}，即对构造时传入的第三个参数（i18n 串）做
 * {@code parseMessage} 后显示；我们的文案里**没有占位符** ⇒ 原样显示
 * （也因此**不要**把这几个串真的写进语言文件，否则会被翻译覆盖）。
 ...
 * 平台差异只有抛出的异常形态（本版为 {@code ModLoadingException(IModInfo, ModLoadingStage, String, Throwable)}；
 * NeoForge 侧为 {@code new ModLoadingException(ModLoadingIssue.error(...))}）。
 */
public final class ModCompatibilityCheck {
    ...
    public static void verifyOrThrow() {
        ...
        IModInfo self = ModList.get().getModContainerById(AstralDiceMod.MODID)
                ...
                .orElse(null);
        throw new ModLoadingException(self, ModLoadingStage.COMMON_SETUP, buildMessage(found), null);
    }
