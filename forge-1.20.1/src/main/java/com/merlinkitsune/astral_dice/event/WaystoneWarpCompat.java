...
/**
 * Waystones 传送可选联动(Forge,反射注册)。
 *
 * <p>Waystones 事件经 Balm 的 fireEvent 最终会转发到 MinecraftForge.EVENT_BUS,
 * 因此这里在 {@code waystones} 模组加载时通过反射向 Forge 事件总线注册:
 * <ul>
...
public final class WaystoneWarpCompat {
    ...
    /** 在 Waystones 模组加载后调用(仅在 Forge 主类中调用)。 */
    ...
    private static void registerWaystoneTeleportEvent(String className, Consumer<Object> handler) throws Exception {
        ...
        addListener.invoke(bus, normalPriority, false, eventClass, handler);
    }
