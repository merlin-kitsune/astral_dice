...
public final class EffectTimerGuard {
    ...
    public static void tick(Player player) {
        ...
        while (it.hasNext()) {
            ...
            if (expectedRemaining <= 0) {
                if (inst != null) {
                    ...
                    forceRemove(player, effect);
                    player.addEffect(new MobEffectInstance(effect, 1, timer.amplifier(),
                ...
                player.addEffect(new MobEffectInstance(effect, (int) expectedRemaining, timer.amplifier(),
                        timer.ambient(), timer.visible(), timer.showIcon()));
            } else {
        ...
        if (dirty) {
            ModAttachments.EFFECT_TIMER_ENDS.set(player, updated);
        }
    }
    ...
    public static void clear(Player player) {
        if (player.level().isClientSide()) return;
        ModAttachments.EFFECT_TIMER_ENDS.set(player, new HashMap<>());
    }

    private static void forceRemove(Player player, MobEffect effect) {
        forcedRemoval = true;
        try {
