 ...
 * 随会话（{@code Session.allowSelf}）与 {@code ModNetwork.TargetSelectStartMessage} 一起下发，
 * 客户端据此在 actionbar 上给出「或按下 鼠标右键 对自身使用」的那一条口径，
 * 并在右键时改为提交对自身的确认。
 *
 * <p><b>实现者与放行口径（2026-09-25 起）</b>：返回 {@code true} 的实现者有两类 —— ① 游戏大师立牌 ren
 * 的主动「熊孩子特权」（{@code ren_privilege}）；② 三张可自用的效果牌动作
 * {@code express_delivery} / {@code luxury_feast} / {@code berserk}（消费方实现类
 * {@code BaseEffectCardItem.SelectorAction}，取值来自各牌
 * {@code registerSelectorAction(actionId, allowSelf)} 的 {@code allowSelf}）。其余动作恒为
