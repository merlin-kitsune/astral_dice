 ...
 *
 * <p>与 1.21.1 侧的 {@code target/HoldToSelect} **接口语义一致**（纯接口，无平台 API；{@code default}
 * 无、方法签名逐字相同）。两线**并非逐字节相同** —— javadoc 里对 tick 事件类的指涉不同
 * （1.20.1 为 {@code TickEvent.PlayerTickEvent}），这是既有惯例。
 */
