...
迁移前源文件：scripts/test/lib/paths.sh 的 mt_load_conf() 与 scripts/test/lib/mt_paths.py 的
_load_conf()（两者均已在 92fbeaf「工具链收敛为纯 pwsh」删除；取回：`git show 92fbeaf^:<路径>`）。两侧语义本就一致，本模块把「同一份 KEY=VALUE 文件被两种语言解析」收敛为一种：
