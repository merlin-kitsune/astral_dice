...
# ══ 多显示器搬移（**已全局移除**，2026-09-17 用户裁决：「全局移除移动游戏窗口到第二屏幕的规则」）══
# 这里原有 Get-MtMonitors / Move-MtWindowToMonitor 两个函数，专供 mt_launch 把测试客户端搬到
# 显示器 #1（第二屏）并固定客户区 1920x1080。该规则已按用户裁决**全局移除**（窗口位置/尺寸由系统
# 与游戏自身决定），两个函数随之删除、导出项一并去掉 —— 它们是这两个函数的**唯一**调用方来源，
# 且仓库内再无其它调用（已核对 mt_capture / mt_inject / mt_ime / mt_preflight / 全部用例）。
# 需要窗口矩形做裁剪/注入的工具仍用保留的 Get-MtWindowRect / Get-MtClientRect / 
# Find-MtMinecraftWindow，与搬移规则无关。
