...
- **按用户要求新增「光影开启」验证项 `SHADER-VISION-26.1.2`（视觉识别 + 游戏内手动开启 + 崩溃判定）**：用例 = `screenshot(crop SH_OFF)` → **vision**「是否已开启光影」→ `inject_key k`（Iris `iris.keybind.toggleShaders`，与界面 Apply 同一入口 `IrisApiV0ConfigImpl.setShadersEnabledAndApply → Iris.reload → createPipeline`）→ `wait 5s` → `assert log "Using shaderpack:.*ComplementaryUnbound"` → `screenshot(SH_ON)` → **vision**「游戏窗口是否还在/是否已呈现光影」→ `assert absent "Missing sampler Sampler1|Unreported exception thrown"` → `assert crash` → `assert kubejs`。**实测结论（2026-09-17）**：`Using shaderpack:` 命中（开启动作确实走到建管线）→ 立即 `IllegalStateException: Missing sampler Sampler1` → 新增崩溃报告 `crash-2026-09-17_10.53.08-client.txt` ⇒ **用例 = FAIL（= 上游缺陷仍在，非工具链故障）**；视觉读数：`SH_OFF`（裁剪窗口）为**无光影原版观感**（星空/月亮/平面地形均匀受光，无方向性阴影或反射，标题栏 `Minecraft NeoForge* 26.1.2 - 单人游戏`）确认前置成立，`SH_ON`（崩溃后抓拍，窗口已不存在 ⇒ `mt_capture` 退化为抓整屏并只 WARN）**画面里已无游戏窗口**，与崩溃报告一致。
...
- **测试环境渲染栈（26.1.2，按用户裁决全部改走 Modrinth Maven）**：`mt_env.ps1 mods --version 26.1.2` 新增 `Install-MtRenderStack`，从 `https://api.modrinth.com/maven/maven/modrinth/<slug>/<version>/<file>` 拉取并**体积 + SHA1 双校验**（缓存 `temp/probe_mods/26.1.2/`）：Sodium `mc26.1.2-0.9.2-neoforge`、Iris `1.11.4+26.1-neoforge`、光影包 Complementary Shaders - Unbound `r5.9.3`（落到 `run/26.1.2/shaderpacks/`），并写 `run/26.1.2/config/iris.properties`。**`enableShaders=false` 是刻意默认**：26.1.2 上启用光影必崩 `IllegalStateException: Missing sampler Sampler1`（`GlCommandEncoder.trySetup`），两个互不相关的光影包均可复现 ⇒ 上游 Iris/Sodium 缺陷（Iris #3182 / #2719）。阶段 L 新增回显 `SODIUM_LOADED` / `IRIS_LOADED` / `SHADERS=… pack=…` / `SHADERPACK_LOADED=…`（仅启用时才校验 `Using shaderpack:`，关闭时报 `n/a` 而非 WARN）。
...
- **光影崩溃的上游定性复核（2026-09-17，用户问「为何启用光影立即崩、是否用了最新 Sodium/Iris」）**：结论 = **已是最新版本，且这是上游 Iris/Sodium 在 26.1.x 上的未修复缺陷**，与本模组、与测试工具链无关。① **版本核对（Modrinth API 实测）**：Sodium 在 `26.1.2` 的最新发布就是 `mc26.1.2-0.9.2-neoforge`（release，2026-09-11，`sodium-neoforge-0.9.2+mc26.1.2.jar`，1260562 B）；Iris 最新为 `1.11.4+26.1-neoforge`（release，2026-09-13，`iris-neoforge-1.11.4+mc26.1.2.jar`，2756643 B），其声明的前置依赖 `version_id=zg4YQ9EL` **正是** Sodium `mc26.1.2-0.9.2-neoforge` ⇒ 本环境装的这一对就是当前唯一的「最新且互相匹配」组合（无更新的 beta/alpha 可换）。② **崩点源码（本地 `minecraft-patched-26.1.2.109-sources.jar` 实读）**：`GlCommandEncoder#trySetup` 第 526~532 行遍历 `renderPass.pipeline.program().getUniforms()`，凡 `Uniform.Sampler` 都必须能在 `renderPass.samplers` 里找到同名绑定，否则 `throw new IllegalStateException("Missing sampler " + name)`——即**程序声明的采样器多于本次 draw 绑定的采样器**（`Sampler1` 是原版管线里的自动命名）。Iris 会用自家程序**替换**原版管线程序，替换后采样器集合与原版 pass 的绑定集合不再一一对应即崩。③ **为什么「立即」崩**：崩点不在管线创建，而在**启用后的第一帧 draw**——本机两次实测的调用链分别是 `GuiRenderer.executeDraw`（点 Apply 后渲染该界面/世界那帧）与 `LevelRenderer.addMainPass → MultiBufferSource.endBatch → RenderType.draw`（进入世界后的正常帧）；上游 Iris 同刻还自报 `[Iris/FATAL]: Missing program minecraft:pipeline/gui_text in override list. This is likely an Iris bug!!!`（`gui`/`gui_textured`/`panorama`/`blur/0..5` 同样缺失）⇒ 26.1.2 新增/改名的这些原版管线不在 1.11.4 的替代表里。④ **上游状态**：Iris [#3314「26.1.2 Game Crash」](https://github.com/IrisShaders/Iris/issues/3314)（open，2026-08-31，Iris 1.11.3 + Sodium 0.9.1，**堆栈与本机逐帧相同**）、[#3182](https://github.com/IrisShaders/Iris/issues/3182)（open）、[#2719](https://github.com/IrisShaders/Iris/issues/2719)（open，1.21.5 同类）；`26.1` 分支在 1.11.4 之后的最新提交为 2026-09-15 的 `Fix normalMatrix in core profile (#3310)`，**没有任何 sampler/`GlCommandEncoder` 相关修复**。⑤ 次要发现（**不是**崩溃原因，仅 WARN）：`Failed to resolve uniform inSulfurCaves, reason: Unknown variable: BIOME_SULFUR_CAVES` —— Complementary Unbound r5.9.3 的自定义 uniform 引用了 Iris 1.11.4 不认识的地物常量（光影包侧超前/版本不匹配）。⑥ 处置：维持 `enableShaders=false`（刻意默认），光影兼容性测试**阻塞于上游**；若要继续追，可选动作 = 向 #3314 补一份本机复现、或改试 Iris 自家 nightly（`maven.irisshaders.net`，Modrinth 无 beta）。⚠️ **测试期间请勿在光影界面点 Apply**：那会写回 `config/iris.properties` 的 `enableShaders=true`，下一次 `--phase launch` 会在进入世界后的第一帧崩掉（客户端进程死亡 → OP 前置读数失败，表现为「探针未加载」的假工具链故障）；实测已发生一次，需手工把该键改回 `false`（或重跑 `mt_env.ps1 mods`）才能继续。
