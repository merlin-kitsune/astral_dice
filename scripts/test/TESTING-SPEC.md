...
2. `RAILGUN-AOE-SCOPE` 行内的 `bolt_delta` 期望**相对现行探针已过期**（原为 `1`），**2026-09-15 裁决走「方案 A」**：直接把 `RAILGUN-AOE-SCOPE-{1.21.1,1.20.1}.json` 的断言改为 `bolt_delta=3`（现行 `railgunfriendly` 探针摆 5 靶并激怒中立/宠物，其中可命中者为 3 个 → 3 道雷击），用例语义保持「只生成预期数量、没有多生成」。**「方案 B」记为待办**：下次因其它原因动探针输出格式时，顺手把 `bolt_delta` 改成**按目标类型分桶**（如 `extra_neutral` / `extra_pet` / `extra_override`），届时把期望逐项锁死——不要为这一条次要计数断言单独付「改探针格式 + 冷启动 + 读数失去可比性」的代价。该用例的鉴别力来自 `VERDICT` 的命中范围/防误伤（`turtle=0:villager=0:friendly=0` 等），`bolt_delta` 只是旁证，故方案 A 不降低鉴别力。
   **2026-09-15 结论（已复跑；1.21.1 已回填，1.20.1 不回填）**：推导值 `3` 被实测否证 —— 推导把「已激怒的已驯服狼」误计为可命中者，与 `RAILGUN-PET-EXCLUDE` 要求狼被排除的口径**直接冲突**。修正探针激怒步（改用 Java API `NeutralMob#setRemainingPersistentAngerTime`，替代「按类型选全世界最近实体」的选择器；实测两版 `ANGER_POST:mergeN=api:mergeF=api`）后冷启动复跑：**1.21.1 实测 `bolt_delta=2`（两次 read 均 2）、`charge=0`、`SCOPE_OK:1`、`VERDICT:self=0:enemy=1:neutral=1:friendly=0:turtle=0:villager=0:valive=1:talive=1` ⇒ 已回填 `2`**；同批 `HOSTILE-TARGET-NEUTRAL`(24/24 PASS)、`RAILGUN-PET-EXCLUDE`(24/24 PASS)、`RAILGUN-OVERRIDE-CLASS`(20/20 PASS) 三例全绿，同为 `bolt_delta=2`。**1.20.1 维持 `3`、不回填**：该版复跑 `bolt_delta=3`（两次 read 均 3），但**同一轮命中范围读数异常**（见 §8.2-1），而该版用例断言集不含防误伤项、察觉不到 —— 异常定性前不把可疑行为固化成基线。
...
1. **1.20.1 雷击命中范围异常（原「施放者间歇自伤」升级；2026-09-15 新增证据，未定根因）**：既有记录是 `HOSTILE-TARGET-NEUTRAL` 1.20.1 首轮 `self=2.67/4.67`、复跑 `self=0`。本轮 `RAILGUN-AOE-SCOPE-1.20.1`（冷启动、修正后的探针）复现得更完整：同一轮 `AP_RG_AFTER:self=2 → 6`、`friendly=12`（**施放者自己的已驯服狼**）、`turtle=12`、`villager=12`、`enemy=16`、`neutral=21`、`bolt_delta=3`、`SCOPE_OK:0`、`VERDICT` 六项全 1 —— 即「落点箱内所有实体」都被命中；与 1.21.1 同批冷启动的 `self=0:friendly=0:turtle=0:villager=0:SCOPE_OK:1` 形成对照。**已排除的静态解释**：① `astral_dice.mixins.json` 两版都列了 `LightningBoltStrikeScopeMixin`；② 1.20.1 `debug.log` 有 `Mixing LightningBoltStrikeScopeMixin … into net.minecraft.world.entity.LightningBolt` 且**无任何 mixin 报错/警告**（`injectors.defaultRequire=1` 未触发 ⇒ 注入点已找到）；③ `RailgunBolts#isValidLightningTarget`、`HostileTargets#isHostile`、`LightningBoltStrikeScopeMixin` 三个源文件在**两版逐字节相同**；④ `RailgunChipItem#strike` 两版都是先 `RailgunBolts.mark(bolt)` 再 `addFreshEntity(bolt)`（标记顺序符合注释要求）。**故不是配置或代码差异能解释的确定性缺陷**，更像「该道雷击未被识别为 railgun bolt → 白名单整段跳过」的**间歇**情形（或 1.20.1 侧另有环境伤害叠加，尚未区分）。**待办**：在 1.20.1 上复跑 `RAILGUN-OVERRIDE-CLASS` / `HOSTILE-TARGET-NEUTRAL` / `RAILGUN-PET-EXCLUDE`（这三例断言集**含**防误伤项，可判定），必要时在探针里加一条 railgun bolt 识别现场读数以定性。**未改任何产品代码**。
