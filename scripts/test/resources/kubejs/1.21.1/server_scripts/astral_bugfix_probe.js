// ════════════════════════════════════════════════════════════════════════════
//  astral_bugfix_probe.js —— 1.21.1 (NeoForge) 五 bug 冒烟取证探针
//
//  取证通道(唯一权威 = 聊天):
...
//     —— doAttack / doHud / diag 三条链全部在断言前失败。2026-09-12 已全部改回属性形式。
...
function doBlessingClear(ctx, tag) {
    var p = ctx.source.getPlayerOrException();
    ModEffectRemoval.remove(p, ModEffects.DICE_BLESSING);
...
function doHud(ctx, ticks, tag) {
    var p = ctx.source.getPlayerOrException();
    var type = BuiltInRegistries.ENTITY_TYPE.get(ResourceLocation.parse("minecraft:zombie"));
    var mob = type.create(p.level);
    if (mob == null) { send(ctx, "AP_" + tag + "_ERR:spawn_failed"); return 0; }
