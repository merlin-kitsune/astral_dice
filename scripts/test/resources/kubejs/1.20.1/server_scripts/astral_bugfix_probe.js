...
// B（2026-09-15 B2）：与 1.21.1 侧同步改用 `Vec3` 重载 —— 1.20.1 的
// `Entity#setDeltaMovement(double,double,double)` 虽然存在，但两版本探针必须**保持同一写法**，
// 否则 1.21.1 上 Rhino 报的
// `Can't find method net.minecraft.world.entity.Entity.setDeltaMovement(number,number,number)`
// 会在下次移植时被重新引入。`Vec3(double,double,double)` 两版本都存在且唯一。
...
function ensureChipSlot(player, need) {
    ...
    // 不要用 handler.grow(n):Curios 15 已移除 ICurioStacksHandler#grow ⇒ 抛错会让整条
    // `/astralprobe equipslot chip …` 一行读数都不出(静默故障,2026-09-20 实测)。
    // 改走产品公开入口:内部是 removeModifier → addPermanentModifier → update(),
    // 且用产品自己的修饰符 id ⇒ 不与产品每 tick 的尺寸重算叠加(用探针自有 id 会让尺寸翻倍)。
    try {
...
// ANVIL 追加（2026-09-15 B2，与 1.21.1 侧同步）：block 路径必须携带**真实方块访问**。
...
/**
 * 关界面前后的**只读**诊断读数（ANVIL 追加，2026-09-15 B2）。
...
function doAnvilBags(ctx, tag) {
    ...
    // ANVIL 追加修复（2026-09-15 B2）：旧写法把「物品栏里找不到骰子」直接判成
 ...
 *
 * ⚠️ 2026-09-15 B2 加固(与 1.21.1 侧同步;1.21.1 实测免疫相位抛
 * `IllegalStateException: … damage_type … minecraft:5.0` 并逃出命令执行体):
 * KubeJS 的 `DamageSource` 类型包装器把某个 **JS number(5.0)** 当成伤害类型 id 解析了。
 * 属**测试探针侧**的 Rhino/KubeJS 调用形态问题,不是产品行为。加固三层:
 * ① `src` 获取本身也进 try;② 三次尝试各自独立 try;③ 全部失败回退原版 `/damage`。
...
function doDecayFlash(ctx, tag) {
    ...
    var blessedBefore = "?";
    try { blessedBefore = p.hasEffect(ModEffects.DICE_BLESSING.get()) ? 1 : 0; } catch (e10) { blessedBefore = "ERR:" + exText(e10); }
    try { ModEffectRemoval.remove(p, ModEffects.DICE_BLESSING.get()); } catch (e11) { /* 忽略 */ }
...
ServerEvents.tick(event => {
    // 外层兜底(B2 加固):任何逃出内层 try 的异常此前只进 KubeJS server.log,
    // 既看不到是哪一步、也不进 latest.log ⇒ 用例只能看到「读数整段缺失」。
    // 这里把异常额外落到一条 AP_ 行(独立标记 TICKDIAG,不污染各用例的 absent 断言)。
    try {
        pearlTickBody();
    } catch (eOuter) {
...
function pearlTickBody() {
    ...
    try {
        ...
        // 2026-09-15 B2(与 1.21.1 侧同步):免疫窗口的**峰值**必须每 tick 采样一次。
        // 此前只在「珍珠消失」那一 tick 读一次 ⇒ 1.21.1 实测读到 window=0(珍珠落地后仍存活约
        // 20 tick,20 tick 的窗口在读取时已耗尽),断言 `window∈[18,20]` 时序上不可达。
        var maxWin = 0;
        try {
...
function doFenSplash(ctx, tag) {
    ...
    try { ModEffectRemoval.remove(p, ModEffects.DICE_BLESSING.get()); } catch (e2) { /* 忽略 */ }
...
function doFenSplashHit(ctx, tag) {
    ...
    var blessed = "?";
    try { blessed = "" + st.player.hasEffect(ModEffects.DICE_BLESSING.get()); } catch (e1) { blessed = "ERR:" + exText(e1); }
    send(ctx, "AP_" + tag + "_MELEE:" + hit.api + ":dealt=" + hit.dealt
...
function doFenSplashRead(ctx, tag) {
    ...
    try { ModEffectRemoval.remove(st.player, ModEffects.DICE_BLESSING.get()); } catch (e2) { /* 忽略 */ }
...
function doSpellTdHit(ctx, tag) {
    ...
    var laser = "ok";
    try { p.addEffect(new MobEffectInstanceClass(ModEffects.MONSTER_LASER.get(), 2400, 0)); }
...
// ══════════════════════════════════════════════════════════════════════════════
// === 游戏大师立牌(ren)「鼠鼠护盾」:REN-SHIELD-1.20.1 ===
...
// 平台差异:1.20.1 的 ModEffects 是 RegistryObject ⇒ 取效果一律 `.get()`。
// ══════════════════════════════════════════════════════════════════════════════
...
function renEffect(p) {
    return ModEffects.REN_SHIELD.get();
}

/** 「反击」层数图标(ren_counter):层数附件的可见镜像,用于断言图标与层数严格同步 */
function renCounterEffect(p) {
    return ModEffects.REN_COUNTER.get();
}
...
/**
 * 施加一次伤害(本地自足版:1.20.1 的 sourceOf/applyDamage 都是 doTrueDmg 内部的嵌套函数,不可跨命令复用)。
 * 两线共同可见的入口 = `LivingEntity#attack(DamageSource,float)`(`damage(amount, source)` 为备选)。
 */
...
function renState(p) {
    var shield = -1;
    try { shield = p.hasEffect(renEffect(p)) ? 1 : 0; } catch (e) { shield = -1; }
    var res = -1;
    try {
    ...
    try { ctrfx = p.hasEffect(renCounterEffect(p)) ? 1 : 0; } catch (e) { ctrfx = -1; }
 ...
 * `[TargetSelection] start ... action=ren_privilege ... allowSelf=true` 断言。
 */
...
/** 骰神赐福效果(1.20.1:ModEffects 是 RegistryObject,取 .get() 得到 MobEffect) */
function teruBlessing() { return ModEffects.DICE_BLESSING.get(); }
...
function doSignSlot(ctx, tag) {
    ...
    try {
        ...
        if (opt != null && opt.isPresent()) {
            var h = opt.resolve().get().getStacksHandler("stand");
...
//  ⚠️ 1.20.1 平台差异(与 1.21.1 段逐条对等):
//    · ModEffects 常量是 RegistryObject ⇒ hasEffect 传 .get()
//    · CuriosApi.getCuriosInventory 返回 LazyOptional(本段未直接使用)
//    · Attributes.MAX_HEALTH 为 Attribute(非 Holder),getAttribute 重载两版本都存在
...
function doSherryRead(ctx, tag) {
    ...
    try { mirror = p.hasEffect(ModEffects.SHERRY_REASONING.get()) ? 1 : 0; } catch (e2) { err = err + "|eff:" + exText(e2); }
...
function doHannaRead(ctx, tag) {
    ...
    try { floatOn = p.hasEffect(ModEffects.HANNA_FLOAT.get()) ? 1 : 0; } catch (e3) { err = err + "|float:" + exText(e3); }
...
.then(Commands.literal("dumpstate")
    ...
    .then(Commands.argument("tag", StringArg.word())
        ...
        .executes(ctx => guard(ctx, StringArg.getString(ctx, "tag"), function () {
            ...
            // ── 2026-09-25：游戏大师立牌(ren)「鼠鼠护盾」读数（REN-SHIELD-1.20.1）──
