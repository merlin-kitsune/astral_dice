// ════════════════════════════════════════════════════════════════════════════
//  astral_bugfix_probe.js —— 26.1.2 (NeoForge) 回归取证探针
//  ⚠️ 本文件由 temp/port_probe_26.py 从 1.21.1 版机械移植生成;请勿手工分叉修改。
//
//  取证通道(唯一权威 = 聊天):
...
//
//  ── 26.1.2 版本差异(2026-09-22 移植;全部由双反编译源码 jar 逐条核对,非记忆) ──
//  事实来源:neoforge-1.21.1/build/moddev/artifacts/neoforge-21.1.235-sources.jar
//            neoforge-26.1.2/build/moddev/artifacts/minecraft-patched-26.1.2.109-sources.jar
//   · **ResourceLocation → Identifier**(Mojang 官方改名,非 Yarn)。静态工厂
//     parse/fromNamespaceAndPath/tryParse 全部同名保留 ⇒ 仅改 FQN。
//   · 三个子包下沉:monster.Spider → monster.spider.Spider;
//     projectile.ThrownEnderpearl → projectile.throwableitemprojectile.*;
//     level.GameRules → level.gamerules.*。
//   · **inventory.ClickType → inventory.ContainerInput**(枚举值
//     PICKUP/QUICK_MOVE/SWAP/CLONE/THROW/QUICK_CRAFT/PICKUP_ALL 同名同序)。
//     另新增 `ClickAction`(PRIMARY/SECONDARY),那是 `AbstractContainerMenu#clicked`
//     **内部**派生出的点击语义,与 ContainerInput 不是同层概念,不要混用。
//   · ⚠️ **最高危:Registry#get(id) 语义已变**。1.21.1 = `T get(ResourceLocation)`;
//     26.1.2 = `Optional<Holder.Reference<T>> get(Identifier)` **且**新增
//     `T getValue(Identifier)`。旧写法在 26.1.2 会**静默**拿到一个 Optional(不抛错),
//     随后所有 typeIdOf()/instanceof/属性读取全部失效,而读数看起来"有值" ⇒
//     本文件所有 `BuiltInRegistries.<X>.get(Identifier.parse(...))` 一律改 **getValue(...)**。
//     `getKey(T)` / `wrapAsHolder(T)` 未变,保持原样。
//   · `Entity#getCommandSenderWorld()` 删除,统一为 `level()`。
//   · `Level#getDayTime()` 删除 ⇒ 用 `getOverworldClockTime()`。**语义不完全等价**:
//     1.21.1 = levelData 的白昼时刻(0..24000);26.1.2 走
//     `getClockTimeTicks(registryAccess().get(WorldClocks.OVERWORLD))`。
//     本文件仅在 diag 用它做**时间基准能力自检**,不参与任何差值断言 ⇒ 可接受。
//     **时间差值的唯一基准仍是** `level.getLevelData().getGameTime()`(两版都没变)。
//   · `NeutralMob#setRemainingPersistentAngerTime(int)` → `setTimeToRemainAngry(long)`
//     (语义一致:内部即 setPersistentAngerEndTime(level.gameTime + remain))。
//   · `Entity#kill()` → `kill(ServerLevel)`。两处调用点都显式传 level,否则会落进
//     下面的「显式伤害源」回退分支 —— 那会让 airbag / mamu 用例不再走**与 /kill 同一条
//     原版代码路径**,断言虽仍通过但验的东西变了。
//   · `Entity#hurt(DamageSource,float)` **保留**(26.1.2 标 `@Deprecated public final void`,
//     内部委托 `hurtServer(ServerLevel,DamageSource,float)`)⇒ 所有 hurt 调用行为不变。
//     ⚠️ 返回类型由 boolean 变 void,而本文件从未用其返回值 ⇒ 无影响。
//     维护提示:上游若后续删除该兼容方法,改为 `ent.hurtServer(ent.level, src, amt)`。
//   · `ItemEntity#setPickupDelay(int)` **已彻底删除**(字段 private、无 setter)。该调用
//     本就落空(delay_set 恒 0,见 teruAttackDrop 注释),依赖构造器自带的 10 tick
//     拾取延迟 ⇒ 行为不变。
//   · ⚠️ **权限 API 换代**:`CommandSourceStack#hasPermission(int)` 在 26.1.2 **已删除**,
//     数字权限等级被 `PermissionSet`/`Permission` 取代(源码只剩 `PermissionSet permissions()`)。
//     数字级 2 = `PermissionLevel.GAMEMASTERS` = **`Permissions.COMMANDS_GAMEMASTER`**
//     (与模组侧 `AstralPartyCommand#REQUIRED_PERMISSION` 用同一常量,口径一致)。
//     本文件全部 `x.hasPermission(2)` 已改为
//     `x.permissions().hasPermission(Java.loadClass(
//          "net.minecraft.server.permissions.Permissions").COMMANDS_GAMEMASTER)`。
//     ⚠️ 这是**实测踩过的坑**:未改时客户端在玩家登录阶段抛
//     `TypeError: Cannot find function hasPermission`(server_scripts:astral_dice_curios_check.js#24)
//     ⇒ 原版报 `Couldn't place player in world` + `Dev lost connection: 无效的玩家数据`
//     ⇒ 玩家被踢、单人服务器停机、客户端退出,表现为「进世界后 4 秒就断开、且没有任何崩溃报告」。
//   · ⚠️ **Curios 15:`CurioChangeEvent` 成 abstract**。1.21.1(Curios 9.x)可直接订阅,
//     Curios 12.0.0 起抛 `Cannot register listeners for abstract class ...`。
//     具体子类是 `CurioChangeEvent$Item`(物品变了)/ `CurioChangeEvent$State`(数量/组件变了),
//     两线语义要对等就得**各订阅一次** —— 见整文件覆盖的 astral_dice_curio_watch.js。
//     依据 curios-dRnRThvD-sources.jar 的 api/event/CurioChangeEvent.java。
//   · ⚠️ **全部 gamerule 改名**:26.1.2 把 camelCase + `do` 前缀改成 snake_case、去前缀
//     (`GameRules.java:24-92` 逐条 registerBoolean/registerInteger 可证)。用旧名会得到
//     原版回显「错误的命令参数 / Incorrect argument for command」,**整条命令不执行**。
//     本文件用到的两处已改:`keepInventory`→`keep_inventory`;
//     `doFireTick false`→`fire_spread_radius_around_player 0`(**语义近似**,见下)。
//     ⚠️ `doFireTick` 在 26.1.2 **没有同名规则** —— 其职责被拆成 `fire_damage`(玩家着火
//     伤害,Boolean)与 `fire_spread_radius_around_player`(整数)。探针只用它来「防止落雷
//     点燃地面污染读数」,最近的等价物是把蔓延半径设为 **0**
//     (`ServerLevel#canSpreadFireAround`:`spreadRadius == -1 || anyPlayerCloseEnoughTo(pos, spreadRadius)`
//      ⇒ 0 = 仅当玩家恰好同格才蔓延 ≈ 不蔓延;而 -1 反而是「无条件蔓延」)。
//     这是本移植里**唯一一处语义近似**,已在测试报告里单独登记。
//   · 未变(已核对):Identifier.parse、LevelData/LevelAccessor.getGameTime、
//     Level.getServer、Commands.performPrefixedCommand(两版同为 void)、
//     CommandSourceStack.sendFailure、ServerPlayer.sendSystemMessage、
//     BuiltInRegistries 各字段名、Registry.getKey/wrapAsHolder、EntityType.ARROW.create(Level)、
//     damageSources().*、Attributes、Player.attack(Entity)、Player.drop(ItemStack,boolean)、
//     ItemStack.set/getOrDefault(后者签名放宽为 `DataComponentType<? extends T>`,擦除后同形)、
//     AbstractContainerMenu.clicked(int,int,ContainerInput,Player)。
//   · 待**运行时**验证(静态不可判,diag 会给出读数):① `p.level` **属性**访问是否仍被
//     KubeJS 8 的 Rhino 支持(26.1.2 的 `Entity#level()` 与 1.21.1 一样是 public 方法、
//     字段 private;1.21.1 侧实证属性形式可用。domLevel() 已内建 level() 降级链,故即使
//     失效也不会静默);② KubeJS 注入的 `kjs$damage(float,DamageSource)` /
//     `kjs$attack(DamageSource,float)` 是否仍在(调用点均有 try/catch 链式回退,
//     回退结果写进读数 api=...)。
//   · KubeJS 8.0.6 侧(已用 javap 核对 run/26.1.2/mods/kubejs-neoforge-26.1.2-8.0.6.jar):
//     `ServerEvents.commandRegistry` / `ServerEvents.tick` / `EntityEvents.spawned` /
//     `event.commands` / `event.arguments` **JS 可见名全部未变**(Java 字段名改成了
//     SCREAMING_SNAKE,但注册用的字符串仍是 camelCase)⇒ 命令注册与 tick 回调原样可用。
//
...
// 26.1.2:EntityType#create(Level) 单参重载已删除,只剩 create(Level, EntitySpawnReason)。
// 本文件所有生成实体的地方都带上这个枚举(详见文件头「26.1.2 版本差异」)。
var EntitySpawnReasonClass = Java.loadClass("net.minecraft.world.entity.EntitySpawnReason");
...
function spawnDummy(p, typeId, dist) {
    var type = BuiltInRegistries.ENTITY_TYPE.getValue(IdentifierClass.parse(typeId));
    var mob = type.create(p.level, EntitySpawnReasonClass.MOB_SUMMONED);
...
function doDiag(ctx, tag) {
    ...
    probe("getDayTime", function () { return p.level.getOverworldClockTime(); });
    ...
    // ── 26.1.2 专属 API 自检(2026-09-22 移植时新增;只读,不改变世界状态) ──────────
    // ① Level 访问器形态:本文件全篇用**属性** `p.level`(104 处)。这两条读数**唯一**
    //    能证实/证伪「KubeJS 8 的 Rhino 仍把零参方法属性化」:期望两者皆为 "object";
    //    若 lvlProp 是 "function" ⇒ 拿到的是方法对象,必须全篇改 `p.level()`。
    probe("lvlProp", function () { return typeof p.level; });
    probe("lvlCall", function () { return typeof p.level(); });
    // ② 注册表取值语义(最高危静默变更):regGetValue 期望物品名(如 "1 stone"),
    //    regGetOld 期望 Optional 形态 —— 两者对比即该变更的直接证据。
    probe("regGetValue", function () {
        return "" + BuiltInRegistries.ITEM.getValue(IdentifierClass.parse("minecraft:stone"));
    });
    probe("regGetOld", function () {
        return "" + BuiltInRegistries.ITEM.get(IdentifierClass.parse("minecraft:stone"));
    });
    // ③ Identifier 静态工厂(原 ResourceLocation.parse 的同名等价物)
    probe("identParse", function () { return "" + IdentifierClass.parse("astral_dice:blank_chip"); });
    // ④ Entity#kill(ServerLevel) —— D4/D5 两处调用点的形态前提
    probe("killArity", function () {
        var mob = spawnDummy(p, "minecraft:pig", 4);
        if (mob == null) return "spawn_failed";
        try { mob.kill(mob.level); return mob.isAlive() ? "still_alive" : "ok"; }
        catch (e) { return "ERR:" + exText(e); }
    });
    // ⑤ Entity#hurt 兼容层是否仍在(26.1.2 = @Deprecated final void,委托 hurtServer)
    probe("hurtCompat", function () {
        var mob = spawnDummy(p, "minecraft:pig", 5);
        if (mob == null) return "spawn_failed";
        try { mob.hurt(p.level.damageSources().genericKill(), 1.0); return "ok"; }
        catch (e) { return "ERR:" + exText(e); }
    });
    // ⑥ ContainerInput 枚举可见性(clicked 第三参类型,原 ClickType)
    probe("containerInput", function () { return "" + ContainerInputClass.PICKUP; });
...
function spawnVisualBoltAt(p, pos) {
    try {
        var type = BuiltInRegistries.ENTITY_TYPE.getValue(IdentifierClass.parse(BOLT_TYPE_ID));
        var bolt = type.create(p.level, EntitySpawnReasonClass.MOB_SUMMONED);
...
function spawnTrader(p) {
    var type = BuiltInRegistries.ENTITY_TYPE.getValue(IdentifierClass.parse("minecraft:wandering_trader"));
    var trader = type.create(p.level, EntitySpawnReasonClass.MOB_SUMMONED);
...
//      结束后还原(测试脚手架,不改被测逻辑)。
// ════════════════════════════════════════════════════════════════════════════
var ContainerInputClass = Java.loadClass("net.minecraft.world.inventory.ContainerInput");
...
function anvilClick(p, menu, slot, button) {
    menu.clicked(slot, button, ContainerInputClass.PICKUP, p);
...
var ThrownEnderpearlClass = Java.loadClass("net.minecraft.world.entity.projectile.throwableitemprojectile.ThrownEnderpearl");
...
function doRailgunFriendly(ctx, tag) {
    ...
    function angerCmd(e, typeId, ticks) {
        ...
        try { e.setTimeToRemainAngry(ticks); return "api"; }
        catch (e9) {
...
function opprobe(ctx) {
    ...
    try {
        ...
        has2 = cs.permissions().hasPermission(Java.loadClass("net.minecraft.server.permissions.Permissions").COMMANDS_GAMEMASTER) ? 1 : 0;
        src = "cmdsource";
    } catch (e1) { /* 落到下面的数值级 */ }
    try {
...
function doTrueDmg(ctx, tag) {
    var p = ctx.source.getPlayerOrException();
    var weather = "skip";
    try { p.level.getServer().setWeatherParameters(6000, 0, false, false); weather = "clear"; } catch (e0) { weather = "err"; }
...
function spellSource(p) {
    var arrow = null;
    try { arrow = Java.loadClass("net.minecraft.world.entity.EntityType").ARROW.create(p.level, EntitySpawnReasonClass.MOB_SUMMONED); }
    catch (e1) { arrow = null; }
...
function airbagApplyKill(entity) {
    try { entity.kill(entity.level); return "kill"; } catch (e0) { /* 退化到显式伤害源 */ }
    try {
        var ResourceKey = Java.loadClass("net.minecraft.resources.ResourceKey");
        var Registries = Java.loadClass("net.minecraft.core.registries.Registries");
        var src = entity.level.damageSources().source(ResourceKey.create(Registries.DAMAGE_TYPE,
            IdentifierClass.parse("minecraft:generic_kill")));
...
function doGloveBase(ctx, tag) {
    ...
    try { p.setGameMode(GameTypeClass.SURVIVAL); mode = "forced_survival"; } catch (e0) { /* 忽略 */ }
    var weather = "skip";
    try { p.level.getServer().setWeatherParameters(6000, 0, false, false); weather = "clear"; } catch (e1) { weather = "err"; }
    try { p.setHealth(p.getMaxHealth()); } catch (e2) { /* 忽略 */ }
        ...
        try {
            ...
            if (st.kind === "self") {
                ...
                emitTo(p, "AP_" + tag + "_AFTER:delay=" + LULU_SETTLE_TICKS
                    ...
                    + ":spider_by_bot=" + (st.spiderByBot == null ? "?" : luluFx(st.spiderByBot, MobEffectsClass.SLOWNESS))
...
function luluSpawnAt(anchor, typeId, dx, dz) {
    var type = BuiltInRegistries.ENTITY_TYPE.getValue(IdentifierClass.parse(typeId));
    var mob = type.create(anchor.level, EntitySpawnReasonClass.MOB_SUMMONED);
    if (mob == null) return null;
...
function luluRunTargetSegment(ctx, tag, t, which) {
    ...
    send(ctx, "AP_" + tag + "_ANCHOR:which=" + which + ":target=" + botName
        ...
        + ":spider_by_bot=" + (spiderByBot == null ? "?" : luluFx(spiderByBot, MobEffectsClass.SLOWNESS))
...
function lpFillBox(p, x1, y1, z1, x2, y2, z2, blockId) {
    ...
    try {
        ...
        var state = BuiltInRegistries.BLOCK.getValue(IdentifierClass.parse(blockId)).defaultBlockState();
        for (var x = x1; x <= x2; x++) {
            for (var y = y1; y <= y2; y++) {
                for (var z = z1; z <= z2; z++) {
...
// ===== 单人替身：FakePlayer（2026-09-22 用户裁决「移除双人测试项、改为单人测试」）=====
// 26.1.2 拿不到第二玩家（Carpet 无 26.1.x / Connector 转发致命失败 / 原版无 bot 命令，
// 见 scripts/test/cases/blocked-26.1.2/README.md）；而产品多处 API 形参类型是 `Player`
// （`MamushiSignItem.onCardGivenToOther(Player,Player)`、`DragonCardUtil.giveExclusiveCard
// (Player,Player,…)`、`TeruSignItem.castDescent(Player,Player)`）⇒ **非玩家实体在类型
// 层面就走不通**，只能用 `Player` 子类当替身。NeoForge 自带 `FakePlayer extends Player`，
// 且 `FakePlayerFactory.get(ServerLevel, GameProfile)` 允许**自定义 UUID** ⇒ 可造多名
// 互不相同的替身，从而保住「按受益人去重 / 单次封顶 / 多名受益人」这类判据。
var FpFactoryClass = teruLoadCls("net.neoforged.neoforge.common.util.FakePlayerFactory");
var GameProfileClass = teruLoadCls("com.mojang.authlib.GameProfile");
var UuidClass = teruLoadCls("java.util.UUID");

/**
 * 替身名 → **固定 UUID**：固定值保证跨用例、跨重跑身份稳定（按 UUID 去重的判据才可预期）。
 * 段位 `4a737472-616c-…` = ASCII `Jstr`+`al`，避开真实玩家 UUID 空间。
 */
var FP_PROFILES = {
    "FP1": "4a737472-616c-0001-0001-000000000001",
    "FP2": "4a737472-616c-0001-0001-000000000002",
    "FP3": "4a737472-616c-0001-0001-000000000003",
    "FP4": "4a737472-616c-0001-0001-000000000004"
};
...
function teruArmorModifier(p, attr) {
    var mod = null;
    try { mod = attr.getModifier(IdentifierClass.parse("astral_dice:" + TERU_DEF_ARMOR_KEY)); } catch (e1) { mod = null; }
    if (mod == null) {
        try {
 ...
 */
/**
 * 单人替身解析：`FP1`..`FP4` ⇒ NeoForge `FakePlayer`（`Player` 子类、独立 UUID）。
 * 名不是 FP 名、或三个类在 26.1.2 不可加载时返回 null（调用方照旧给可归因读数）。
 *
 * <p>⚠️ **已知局限（调用方必须据此选判据）**：FakePlayer **不在 `PlayerList`**、
 * 也不在 `level` 的实体列表里 ⇒ 一切「由**产品自己**从在线玩家/世界实体中挑目标」的
 * 路径（范围发牌、按距离过滤、按 UUID 每 tick 重解析）**都看不见它**；只有
 * **直调产品方法**的探针命令（把替身当形参传进去）才有效。
 * 另：它不参与 tick ⇒ 跨 tick 存续类断言在替身上不成立（读数须取在同一次调用内）。
 */
function fpResolveSafe(ctx, nameText) {
    var want = "" + nameText;
    if (FP_PROFILES[want] == null) return null;
    if (FpFactoryClass == null || GameProfileClass == null || UuidClass == null) return null;
    try {
        var prof = new GameProfileClass(UuidClass.fromString(FP_PROFILES[want]), want);
        var lvl = ctx.source.getPlayerOrException().level;
        var fp = FpFactoryClass.get(lvl, prof);
...
function teruFindPlayer(ctx, nameText) {
    ...
    // 单人化回退（2026-09-22）：在线列表里没有 ⇒ 试 FP1..FP4 替身（见 fpResolveSafe 注释）。
    // 放在**最后**：有真玩家时优先真玩家 ⇒ 1.21.1 / 1.20.1 两线行为完全不变。
    var fpFallback = fpResolveSafe(ctx, want);
    if (fpFallback != null) return fpFallback;
    return null;
}
...
/**
 * 单人替身自检 —— 「26.1.2 的双人能力缺口能否用单人替身补上」的**唯一**技术前提。
 *
 * <p>读数 `AP_<tag>_FPTEST:` 字段：
 *   · `cls`      三件套（FakePlayerFactory / GameProfile / UUID）在 26.1.2 是否可加载（1/0）；
 *   · `fpN`      `FakePlayerFactory.get(level, profile)` 是否真返回非 null（1=有 / 0=空 / -2=抛异常）；
 *   · `uuidN`    各替身 UUID（判据：**互不相同** —— 「按受益人去重 / 封顶」成立的基础）；
 *   · `isPN`     是否 `instanceof Player`（产品 API 形参类型的硬要求）；
 *   · `listN`    是否出现在**在线玩家列表**（期望 0：FakePlayer 不进 PlayerList ⇒
 *                「产品自选目标」类路径看不见它；这是替身的已知局限，用例据此降级）；
 *   · `uuid_dup` 四个 UUID 是否重复（0=全不同）；`self` 主玩家自己是否 `instanceof Player`（对照，恒 1）。
 */
/**
 * 物品标签读取路径诊断。
 *
 * <p>背景（2026-09-22）：26.1.2 的 `AP_*_REG_TAG` 六项**恒为 0**，而 `data/astral_dice/tags/item/signs.json`
 * 与 `data/curios/tags/item/stand.json` 与 1.21.1 **逐字节一致**、`AP_*_REG_SRC` 也报物品 ok
 * ⇒ 既不是产品缺标签，也不是物品取不到，而是 `mamuHasTag` 的 `Holder#tags()` 链在 Rhino 下没走通
 * （该函数把「item==null」「arr==null」「未匹配」三种情况分别写成 0/-1/0，所以**返回 0 无法区分**）。
 * 本命令把中间量逐个打出来定位。
 */
function doTagProbe(ctx, tag) {
    var item = resolveItem(MAMU_SIGN_ID);
    var out = "item=" + ((item == null) ? "null" : "ok");
    try { out += ":reg=" + ((BuiltInRegistries.ITEM == null) ? "null" : "ok"); } catch (e0) { out += ":regERR=" + domExText(e0); }
    // 路径 1：BuiltInRegistries.ITEM.wrapAsHolder(item).tags()
    try {
        var h = BuiltInRegistries.ITEM.wrapAsHolder(item);
        out += ":p1h=" + ((h == null) ? "null" : "ok");
        var st = h.tags();
        out += ":p1stream=" + ((st == null) ? "null" : "ok");
        var arr = st.toArray();
        out += ":p1len=" + arr.length;
        if (arr.length > 0) {
            try { out += ":p1a0=" + arr[0]; } catch (eA) { out += ":p1a0ERR=" + domExText(eA); }
            try { out += ":p1loc=" + arr[0].location(); } catch (eB) { out += ":p1locERR=" + domExText(eB); }
        }
    } catch (e1) { out += ":p1ERR=" + domExText(e1); }
    // 路径 2：item.builtInRegistryHolder().tags()
    try {
        var h2 = item.builtInRegistryHolder();
        out += ":p2h=" + ((h2 == null) ? "null" : "ok");
        var a2 = h2.tags().toArray();
        out += ":p2len=" + a2.length;
        if (a2.length > 0) { try { out += ":p2loc=" + a2[0].location(); } catch (eC) { out += ":p2locERR=" + domExText(eC); } }
    } catch (e2) { out += ":p2ERR=" + domExText(e2); }
    // 路径 3：栈上的 ItemStack 走 is(TagKey)（探针注释说 Rhino 下有重载歧义，这里实测确认）
    try {
        var tkCls = Java.loadClass("net.minecraft.tags.TagKey");
        var rgCls = Java.loadClass("net.minecraft.core.registries.Registries");
        var tagKey = tkCls.create(rgCls.ITEM, IdentifierClass.parse("astral_dice:signs"));
        out += ":p3key=" + ((tagKey == null) ? "null" : "ok");
        var stack = new ItemStack(item);
        out += ":p3is=" + (stack.is(tagKey) ? 1 : 0);
    } catch (e3) { out += ":p3ERR=" + domExText(e3); }
    send(ctx, "AP_" + tag + "_TAGPROBE:" + out);
    return 1;
}
function doFpTest(ctx, tag) {
    var p = ctx.source.getPlayerOrException();
    var cls = (FpFactoryClass != null && GameProfileClass != null && UuidClass != null) ? 1 : 0;
    var playerCls = teruLoadCls("net.minecraft.world.entity.player.Player");
    var names = ["FP1", "FP2", "FP3", "FP4"];
    var acc = [];
    var uuids = [];
    for (var i = 0; i < names.length; i++) {
        var nm = names[i];
        var got = 0, uu = "-", usrc = "-", isP = -1, inList = -1;
        try {
            var fp = fpResolveSafe(ctx, nm);
            if (fp != null) {
                got = 1;
                // ⚠️ `fp.getUUID()` 在 Rhino 下**不可见**（本文件既有踩坑：ServerPlayer#getUUID 不可用）
                // ⇒ 复用既有的三级回退读取器 playerUuid()（getUUID → uuid 字段 → NBT），
                //   并把命中路径一起报出来（src=1/2/3，none=全不可达）。
                var ui = playerUuid(fp);
                uu = ui.ok ? ("" + ui.value) : "?";
                usrc = "" + ui.src;
                try { isP = (playerCls != null && fp instanceof playerCls) ? 1 : 0; } catch (e2) { isP = -1; }
                try {
                    var l = p.level.getServer().getPlayerList().getPlayerByName(nm);
                    inList = (l == null) ? 0 : 1;
                } catch (e3) { inList = -1; }
                uuids.push(uu);
            }
        } catch (e5) { got = -2; }
        acc.push(nm.toLowerCase() + "=" + got + ":uuid" + (i + 1) + "=" + uu + ":usrc" + (i + 1) + "=" + usrc
            + ":isP" + (i + 1) + "=" + isP + ":list" + (i + 1) + "=" + inList);
    }
    var seen = {}, dup = 0;
    for (var k = 0; k < uuids.length; k++) {
        if (seen[uuids[k]] === 1) { dup = 1; } else { seen[uuids[k]] = 1; }
    }
    var selfP = -1;
    try { selfP = (playerCls != null && p instanceof playerCls) ? 1 : 0; } catch (e6) { selfP = -1; }
    send(ctx, "AP_" + tag + "_FPTEST:cls=" + cls + ":" + acc.join(":")
        + ":uuid_dup=" + dup + ":uuid_n=" + uuids.length + ":self=" + selfP);
    return 1;
}

/**
 * 把单人替身摆到「主玩家 + (dx,dy,dz)」。
 * 只影响**探针自己读到的** `position()`（`lulu`/`teru` 的「锚点距离」类判据需要目标有位移）；
 * 产品侧「从在线列表挑目标」的路径照旧看不见替身（见 fpResolveSafe 的局限说明）。
 */
function doFpMove(ctx, tag, nameText, dxText, dyText, dzText) {
    var p = ctx.source.getPlayerOrException();
    var fp = fpResolveSafe(ctx, nameText);
    if (fp == null) { send(ctx, "AP_" + tag + "_FPMOVE:found=0:want=" + nameText); return 1; }
    function num(v) { var q = parseFloat("" + v); return isNaN(q) ? 0 : q; }
    var err = "";
    try {
        var pos = p.position();
        placeAt(fp, pos.x() + num(dxText), pos.y() + num(dyText), pos.z() + num(dzText));
    } catch (e1) { err = "|" + domExText(e1); }
    var q = "?", dist = -1;
    try {
        var pp = fp.position();
        q = teruR1(pp.x()) + "," + teruR1(pp.y()) + "," + teruR1(pp.z());
        dist = teruR1(pp.distanceTo(p.position()));
    } catch (e2) { q = "?"; }
    send(ctx, "AP_" + tag + "_FPMOVE:found=1:want=" + nameText
        + ":pos=" + q + ":dist=" + dist + ":err=" + err);
    return 1;
}
...
function teruResolveType(typeText) {
    ...
    var type = null;
    try { type = BuiltInRegistries.ENTITY_TYPE.getValue(IdentifierClass.parse(raw)); }
...
function doTeruDummy(ctx, tag, whichText, typeText, distText, hpText) {
    ...
    try { isSpider = (mob instanceof Java.loadClass("net.minecraft.world.entity.monster.spider.Spider")) ? 1 : 0; }
...
function domCountTemp(p) {
    ...
    try {
        ...
        // 26.1.2:Inventory#offhand 字段已移出(只剩 private items + EntityEquipment),
        // 改用 LivingEntity#getOffhandItem() —— 原语义就是「副手那 1 格」,一一对应。
        var st2 = p.getOffhandItem();
...
function domLevel(p) {
    ...
    if (lv != null) return lv;
    try { lv = p.level(); } catch (e3) { lv = null; }
    return lv;
}
...
function domTempBreakdown(p) {
    ...
    try {
        ...
        // 26.1.2:同 domCountTemp —— Inventory#offhand 已移出,改走 getOffhandItem()。
        var offItem = p.getOffhandItem();
        if (offItem != null) scan(offItem);
...
function signGateHolders(signId) {
    ...
    try {
        ...
        if (id === "astral_dice:fanny_sign") {
            return signLagCompact([signLagHold("REGENERATION"), signLagHold("STRENGTH"),
                signLagHold("SPEED"), signLagHold("INSTANT_DAMAGE"), signLagHold("POISON"),
                signLagHold("HUNGER"), signLagHold("NAUSEA"), signLagHold("WITHER"),
                signLagHold("DARKNESS"), signLagHold("WEAKNESS"),
                signLagHold("MINING_FATIGUE"), signLagHold("SATURATION")]);
        }
    } catch (eT) { return []; }
...
function mamuKeepInvRule(p) {
    try {
        var GR = Java.loadClass("net.minecraft.world.level.gamerules.GameRules");
        var lvl = p.level;
...
function mamuKill(p) {
    ...
    try { p.kill(p.level); if (dead()) return "kill"; } catch (e1) { /* 回退 */ }
    var src = null;
 ...
 * 与本文件既有的 `luluFx(…, MobEffectsClass.SLOWNESS)` 同源),避免再猜描述 id。
 */
function mamuSlowFx(ent) {
    var inst = null;
    try { inst = ent.getEffect(MobEffectsClass.SLOWNESS); } catch (e1) { inst = null; }
    var amp = -1, dur = -1;
...
function doMamuDie(ctx, tag, keepText) {
    ...
    var kiSet = "-";
    if (!keepInv) {
        try { kiSet = "" + runCmdP(p, "gamerule keep_inventory false"); }
...
function ssCountTargets(p, radius) {
    ...
    try {
        ...
        for (var i = 0; i < list.size(); i++) {
            var e = list.get(i);
            try { if (e.entityTags() != null && e.entityTags().contains(SS_TARGET_TAG)) n = n + 1; } catch (t1) { }
        }
    } catch (e0) { }
    return n;
}
...
function ssFindTarget(p, radius) {
    ...
    try {
        ...
        for (var i = 0; i < list.size(); i++) {
            var e = list.get(i);
            var hit = false;
            try { hit = (e.entityTags() != null) && e.entityTags().contains(SS_TARGET_TAG); }
            catch (t1) { try { hit = ("" + e.entityTags()).indexOf(SS_TARGET_TAG) >= 0; } catch (t2) { hit = false; } }
            if (!hit) continue;
            var d = 0.0;
...
ServerEvents.commandRegistry(event => {
    ...
    event.register(
        Commands.literal("astralprobe")
            ...
            .then(Commands.literal("tagprobe")
                .then(Commands.argument("tag", StringArg.word())
                    .executes(ctx => guard(ctx, StringArg.getString(ctx, "tag"), function () {
                        return doTagProbe(ctx, StringArg.getString(ctx, "tag"));
                    }))))
            .then(Commands.literal("fptest")
                .then(Commands.argument("tag", StringArg.word())
                    .executes(ctx => guard(ctx, StringArg.getString(ctx, "tag"), function () {
                        return doFpTest(ctx, StringArg.getString(ctx, "tag"));
                    }))))
            .then(Commands.literal("fpmove")
                .then(Commands.argument("tag", StringArg.word())
                    .then(Commands.argument("name", StringArg.word())
                        .then(Commands.argument("dx", StringArg.word())
                            .then(Commands.argument("dy", StringArg.word())
                                .then(Commands.argument("dz", StringArg.word())
                                    .executes(ctx => guard(ctx, StringArg.getString(ctx, "tag"), function () {
                                        return doFpMove(ctx, StringArg.getString(ctx, "tag"),
                                            StringArg.getString(ctx, "name"),
                                            StringArg.getString(ctx, "dx"),
                                            StringArg.getString(ctx, "dy"),
                                            StringArg.getString(ctx, "dz"));
                                    }))))))))
