// ════════════════════════════════════════════════════════════════════════════
//  ⚠️ 本文件是 **MC 26.1.2 / NeoForge 26.1.2.109 / KubeJS 8.x** 版本,
//     由 1.21.1 版(resources/kubejs/1.21.1/server_scripts/astral_bugfix_probe.js)机械迁移而来。
//     迁移脚本:temp/port_probe.ps1(一次性,已随 temp/ 清理策略不入库)。
//
//  ── 与 1.21.1 版的**差异清单**(改动都在下方标记 `26.1.2` 的注释处)────────────
//   1) `net.minecraft.resources.ResourceLocation` → **Identifier**(类变量与 4 处 .parse)
//   2) `net.minecraft.world.inventory.ClickType` → **ContainerInput**(变量名 ClickTypeClass → ContainerInputClass)
//   3) `Registry#get(id)` 现返回 `Optional<Holder.Reference<T>>` ⇒ ITEM/ENTITY_TYPE 读取改走
//      `itemOf(id)` / `entityTypeOf(id)`(缺失返回 null,与旧行为等价)
//   4) `EntityType#create(Level)` → `create(Level, EntitySpawnReason)` ⇒ 改走 `createEntity(type, level)`
//   5) `CommandSourceStack#hasPermission(int)` 已删除 ⇒ 改走 `hasGm(source)`
//      (`permissions().hasPermission(Permissions.COMMANDS_GAMEMASTER)`,与产品侧同一判据)
//   6) `Commands#performPrefixedCommand` 返回 **void** ⇒ `runCmd`/`runCmdP` 改走 `execP`
//      (先 `dispatcher.parse` 预解析,失败落 `ERR:`;成功返回 `rc=ok`)。
//      ⚠️ 用例断言只锚定 `rc=` 前缀,`rc=ok` 与 1.21.1 的 `undefined` 同义(成功)。
//   7) Curios 15:`ICurioStacksHandler` 无 `grow`(移到 `IDynamicStackHandler`)⇒ 改 `handler.getStacks().grow(...)`
//   8) `ThrownEnderpearl` 移入 `net.minecraft.world.entity.projectile.throwableitemprojectile`
//   9) `opprobe` 的数值级权限读数改用 `getProfilePermissions(NameAndId)`(Rhino 取不到时保持 -1)
//  10) 追加 1.21.1 版没有的 3 条迁移期命令:itemcount / give / readstate
//
//  ── 以下 1.21.1 版原有的实测结论**仍然有效**(Rhino/KubeJS 侧口径未变)──────────
//   · 有命令上下文 → ctx.source.sendFailure(Component);无上下文 → ServerPlayer#sendSystemMessage
//     两条路都落到 logs/latest.log 的 `[CHAT] AP_...` 行;mt_launch 每次启动前删 latest.log
//   · Entity#moveTo(double,double,double) 在 Rhino 下 ambiguous → 一律用 setPos
//   · level.getGameTime() 直接成员查找不可见 → 首选 level.getLevelData().getGameTime()
//   · mob.hurt(DamageSource,float) 不可见 → 首选 Player#attack(Entity)(与真人左键同源)
//   · 命令注册必须在 ServerEvents.commandRegistry 回调内;执行体提取为顶层命名函数,函数内一律 var
//   · 所有 id 参数用 StringArgumentType.string()(word() 会吞冒号)
//   · 执行体一律包 guard(),异常落成 AP_<TAG>_EX:...
//     ⚠️ 例外:KubeJS 自身的 `KubeRuntimeException`(如 ItemWrapper 解析失败)**不会被 guard 捕获**,
//        只能在写探针时避开那些调用形态(见 hasDiceEquipped26 的注释)
// ════════════════════════════════════════════════════════════════════════════// ════════════════════════════════════════════════════════════════════════════
...
//   · Identifier.parse(...)(1.20.1 为 new ResourceLocation(...))
...
// ════════════════════════════════════════════════════════════════════════════
//  26.1.2 兼容层(shim) —— 由 scripts/temp 的迁移脚本从 1.21.1 版生成时插入。
//  下列 helper 承担 1.21.2+ 的 API 形态差异,正文调用点一律改走它们,
//  以便与 1.21.1 版逐条对读时差异最小:
//    · Registry#get 返回 Optional<Holder.Reference<T>>(1.21.1 直接返回对象)
//    · EntityType#create 需要 EntitySpawnReason
//    · CommandSourceStack#hasPermission(int) 删除 → permissions().hasPermission(Permission)
//    · Commands#performPrefixedCommand 返回 void
// ════════════════════════════════════════════════════════════════════════════
...
// Curios 15:尺寸改写必须走「槽位修饰符 → update() → resize()」(见 applyChipSlotCount)
var AttributeModifierClass = Java.loadClass("net.minecraft.world.entity.ai.attributes.AttributeModifier");
var AttributeModifierOperationClass = Java.loadClass("net.minecraft.world.entity.ai.attributes.AttributeModifier$Operation");

/** Optional<Holder.Reference<T>> → T(缺失返回 null) */
function holderValue(opt) {
    try { return (opt != null && opt.isPresent()) ? opt.get().value() : null; }
    catch (e) { return null; }
}

/** 物品 id → Item(26.1.2:注册表返回 Optional<Holder>) */
function itemOf(id) {
    try { return holderValue(BuiltInRegistries.ITEM.get(Identifier.parse(id))); }
    catch (e) { return null; }
}

/** 实体类型 id → EntityType(26.1.2:注册表返回 Optional<Holder>) */
function entityTypeOf(id) {
    try { return holderValue(BuiltInRegistries.ENTITY_TYPE.get(Identifier.parse(id))); }
    catch (e) { return null; }
}

/** 创建实体(26.1.2 需要 EntitySpawnReason;失败回退旧签名) */
function createEntity(type, level) {
    if (type == null) return null;
    try { return type.create(level, EntitySpawnReasonClass.COMMAND); }
    catch (e) { return type.create(level); }
}

/** 命令源是否具备 OP 级 2(与产品侧 AstralPartyCommand 同一判据) */
function hasGm(source) {
    try { return source.permissions().hasPermission(PermissionsClass.COMMANDS_GAMEMASTER); }
 ...
 * 跑一条命令并返回 `rc=…` / `ERR:…`。
 *
 * ⚠️ 26.1.2 的 `Commands#performPrefixedCommand` 返回 **void**(1.21.1 返回 int),
 * 无法再用返回值区分「未知命令 / 语法错误」。为保住原有契约(失败必须可判定),
 * 这里**先经 dispatcher.parse 预解析** —— 未知命令/语法错误会抛 CommandSyntaxException,
 * 落到 `ERR:` 分支;成功才执行并返回 `rc=ok`。
 * 注意 dumpState 的判据是 `rc=0` 或 `ERR:` 前缀,`rc=ok` 视为成功(与 1.21.1 的 undefined 同义)。
 */
function execP(p, cmd) {
    try {
        var src = p.createCommandSourceStack();
        var commands = p.level.getServer().getCommands();
        commands.getDispatcher().parse(cmd, src);
        commands.performPrefixedCommand(src, cmd);
        return "rc=ok";
    } catch (e) { return "ERR:" + exText(e); }
}
var Identifier = Java.loadClass("net.minecraft.resources.Identifier");
...
// ── 关卡类型判定(26.1.2 移植差异)────────────────────────────────────────────
// 1.21.1 的 doDiag 直接 `p.level.getClass().getName()`;26.1.2 的 Rhino 沙箱**不暴露
// `getClass()`**(实测 `ERR:TypeError: Cannot find function getClass in object
// ServerLevel[testworld]`)⇒ 改成两级判定,但**断言文本与 1.21.1 完全一致**:
//   ① `instanceof`(Rhino 经 NativeJavaClass#hasInstance → Class#isInstance);
//   ② 退化到对象 toString 前缀(`ServerLevel[testworld]`,即上条报错里的形态)。
// 目的不变:证明命令上下文取到的 `p.level` 确实是**服务端**关卡,而不是客户端/代理对象。
var ServerLevelClass = null;
try { ServerLevelClass = Java.loadClass("net.minecraft.server.level.ServerLevel"); } catch (e) { ServerLevelClass = null; }
function serverLevelKind(l) {
    if (ServerLevelClass != null) {
        try { if (l instanceof ServerLevelClass) { return "net.minecraft.server.level.ServerLevel"; } } catch (e) { /* 落到 toString */ }
    }
    var s = "" + l;
    return s.indexOf("ServerLevel[") === 0 ? "net.minecraft.server.level.ServerLevel" : s;
}
 ...
 * 按骰子星级把尺寸调到目标值。测试里骰子可能只有 0 星 → 槽数为 0 → 筹码装不进去
 * (会落成 slot_overflow:chip:0)。此处按测试需要直接调尺寸,属**测试脚手架**,
 * 不改变被测逻辑(电磁炮的延迟与冷却判定与槽位如何获得无关)。
 *
 * ⚠️ Curios 15 下**必须**走「槽位修饰符 + update()」,与产品代码 DiceCurioItem#applySlotCount 同一路径:
 * 直接 `handler.getStacks().grow(...)` 只改 stackHandler,而 Curios 自身 tick 循环以 getSlots() 为界、
 * 无保护地读 getCosmeticStacks().getStackInSlot(i) ⇒ 抛 `Slot 0 not in valid range - [0,0)` 并崩服
 * (2026-09-17 实测)。
 */
...
function spawnDummy(p, typeId, dist) {
    ...
    var mob = createEntity(type, p.level);
...
function spawnVisualBoltAt(p, pos) {
    try {
        var type = entityTypeOf(BOLT_TYPE_ID);
        var bolt = createEntity(type, p.level);
...
function spawnTrader(p) {
    var type = entityTypeOf("minecraft:wandering_trader");
    var trader = createEntity(type, p.level);
...
function runCmdP(p, cmd) {
    // 26.1.2:performPrefixedCommand 返回 void ⇒ 统一走 execP(预解析 + rc=ok/ERR)
    return execP(p, cmd);
}
...
function runCmd(ctx, cmd) {
    // 26.1.2:同上,统一走 execP(预解析 + rc=ok/ERR)
    var p = ctx.source.getPlayerOrException();
    return execP(p, cmd);
}
...
function dumpState(ctx, tag) {
    ...
    // ⚠️ 26.1.2:Brigadier 的 executes 处理器必须返回 **int** —— 返回字符串会抛
    //    EvaluatorException: Cannot convert rc=ok to int(实测,且该异常**逃出 guard**,
    //    只留一行 Brigadier 	hrew an exception)。故这里只把 rc 用于消息,返回值固定 1。
    return 1;
}
...
function opprobe(ctx) {
    ...
    try {
        // 26.1.2:`MinecraftServer#getProfilePermissions(NameAndId)`(记录 = UUID + name);
        // 旧写法直接传 GameProfile 已无该重载。PermissionLevel 是枚举
        // (ALL/MODERATORS/GAMEMASTERS/ADMINS/OWNERS),ordinal 与原版 0..4 数值级同序
        // (GAMEMASTERS→2),故报 ordinal 即可与 1.21.1 的读数对照。
        // ⚠️ 该读数在 Rhino 下取不到时保持 -1 —— mt_launch 的闸门只判 has2,不受影响。
        var prof = p.getGameProfile();
        level = p.level.getServer().getProfilePermissions(new NameAndIdClass(p.getUUID(), prof.name())).level().ordinal();
...
function spellSource(p) {
    var arrow = null;
    try { arrow = createEntity(Java.loadClass("net.minecraft.world.entity.EntityType").ARROW, p.level); }
    catch (e1) { arrow = null; }
...
// ════════════════════════════════════════════════════════════════════════════
//  26.1.2 迁移期附加命令(1.21.1 版没有;供 MIGRATION-SMOKE-26.1.2 与日常排查用)
//    /astralprobe itemcount <tag>  只读:astral_dice 命名空间注册物品数(应为 123)
//    /astralprobe give <itemId> <count> <tag>
//    /astralprobe readstate <tag>  只读:关键附件 + 骰子饰品在位 + 槽位规模
//  三者都只做「注册表 / 附件 / Curios 槽位」读取,不碰任何产品状态。
// ════════════════════════════════════════════════════════════════════════════

/** astral_dice 命名空间的注册物品数 */
function countModItems() {
    var n = 0;
    try {
        var it = BuiltInRegistries.ITEM.keySet().iterator();
        while (it.hasNext()) {
            var k = it.next();
            if (("" + k.getNamespace()) === "astral_dice") n++;
        }
    } catch (e) { return -1; }
    return n;
}
...
function doItemCount(ctx, tag) {
    ...
    send(ctx, "AP_" + tag + "_ITEMCOUNT:astral_dice=" + countModItems() + ":total=" + total);
    send(ctx, "AP_" + tag + "_DONE");
    return 1;
}
...
function doGive(ctx, itemId, count, tag) {
    ...
    var item = resolveItem(itemId);
    if (item == null) { send(ctx, "AP_" + tag + "_ERR:unknown_item:" + itemId); return 0; }
    try { p.getInventory().add(new ItemStack(item, count)); }
    catch (e) { send(ctx, "AP_" + tag + "_ERR:give:" + exText(e)); return 0; }
...
/** 骰子槽里是否有本模组的饰品(只读)。
 *  ⚠️ **禁止**改用 `ICuriosItemHandler#findFirstCurio(...)`:Curios 15 多出
 *  `findFirstCurio(Item)` 重载,KubeJS 8 的 ItemWrapper 会按该重载解析 JS 函数并抛
 *  KubeRuntimeException —— **连 guard 都捕不到**(读数全无)。故直接遍历槽位。 */
function hasDiceEquipped26(p) {
    ...
    try {
        ...
        for (var i = 0; i < stacks.getSlots(); i++) {
            ...
            if (itemIdOf(s).indexOf("astral_dice:") === 0) return 1;
        }
        return 0;
    } catch (e) { return -1; }
}
...
ServerEvents.commandRegistry(event => {
    ...
    event.register(
        Commands.literal("astralprobe")
            ...
            // ── 26.1.2 迁移期附加(1.21.1 版没有;MIGRATION-SMOKE-26.1.2 依赖它们)──
