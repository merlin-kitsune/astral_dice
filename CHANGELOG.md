...
- 新增骰子「诡异骰子」:与钻石骰子同阶(`astral_dice:dice_t2`),可由钻石骰子升级(WWW/WDW/WWW,W=灵魂沙,D=钻石骰子);立牌主动技能冷却时间 -50%(所有开始冷却入口统一经 `WeirdDiceHandler.signCooldownTicks` 取值),但战斗骰低点数(1-3)出现概率提升 50%(权重法:1/2/3 权重 ×1.5,P(低点数) 50%→60%,对攻击骰与玩家防御骰均生效)(仅 1.21.1)。 / New "Weird Die": same tier as the Diamond Die (`astral_dice:dice_t2`), upgradeable from it (WWW/WDW/WWW, W=Soul Sand, D=Diamond Die); sign active-skill cooldowns are halved, but low combat-die rolls (1-3) are 50%% more likely (weighted: faces 1-3 ×1.5, P(low) 50%%→60%%, applies to attack rolls and player defense rolls) (1.21.1 only).
