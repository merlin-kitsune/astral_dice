...
- 新增充能流派基础设施:图标取自 `images/充能.png`(效果纹理 `mob_effect/charge.png`);最大层数写入 `GameplayConstants.CHARGE_MAX_STACKS = 20`;拥有至少 1 层充能时固定获得防御力 +1(折算为真实护甲)并使立牌主动/效果牌冷却时间 -20%(经 `ChargeManager`,接入立牌冷却入口与效果牌冷却),当前暂未接入获得/消耗来源,后续来源统一调用 `ChargeManager.addStacks/consumeOne/removeAll`(双版本一致)。 / Added the Charge playstyle infrastructure: icon copied from `images/充能.png` (effect texture `mob_effect/charge.png`); max stacks stored in `GameplayConstants.CHARGE_MAX_STACKS = 20`; with at least 1 Charge stack you gain a fixed +1 defense (converted to real armor) and sign-active/effect-card cooldowns are reduced by 20%% (via `ChargeManager`, wired into sign cooldown and effect-card cooldown entries); no gain/spend sources are connected yet — future sources should use `ChargeManager.addStacks/consumeOne/removeAll` (both loaders).
...
- 新增首次加入赠送规则书选项:common 配置新增 `give_guide_book_on_first_join`(默认 `true`),并同步 GameplayConstants 常量;开启时玩家第一次加入世界会获得《恋的规则书》(帕秋莉手册),每个玩家每个世界只发一次;配置文件版本号保持 1 不变(双版本一致)。 / Added first-join rulebook option: new common-config entry `give_guide_book_on_first_join` (default `true`) backed by a GameplayConstants field; when enabled, players receive the Koi's Rulebook (Patchouli guide) on first join, once per player per world; config version remains 1 (both loaders).
...
- 文本/手册整理:合并本地修改至双版本——Moses 手册被动页合并、移除 integration 第 4 页(千咒刻印)、Pandaman tooltip 嘲讽改为蓝色;同时移除肉弹战车立牌 tooltip 两个计数器之间的多余空行;枪匠立牌 tooltip 被动技能标题删除“弱点识破”,仅保留“精密技巧”。 / Text/handbook cleanup merged to both loaders: Moses handbook passive pages merged, integration page 4 (Curse Mark) removed, Pandaman tooltip Taunt recolored blue, the extra blank line between the two Pandaman tooltip counters was removed, and the Gunsmith sign tooltip passive title now shows only “Precision” (removing “Weakness Insight”).
