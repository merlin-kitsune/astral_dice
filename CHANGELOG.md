...
- Fixed and hardened the pwsh automated-test toolchain (all toolchain-side defects, no gameplay impact): (1) `mt.ps1` now decides the env-phase seed package **per version** — the old code hard-coded the existence of `testworld-seed-1.20.1.zip` and reused it as a shared `--seed` for both versions, so 1.21.1 always ended in `MT_WORLD: BLOCKED` and the whole run aborted there; (2) `Find-MtMinecraftWindow` gained a fallback: ModDevGradle/FML DevLaunch starts the client with a short command line plus an args file (measured: the window owner's command line is only 56 characters and no dev-run marker is readable anywhere in the parent chain), so the old criterion always returned 0 and injection was refused — it now falls back to "the single candidate whose title contains that version", still refusing to guess when several candidates exist; (3) case assertions gained `scope: whole` (evaluate the whole file, for lines that only appear during startup such as Mixin apply lines) and the screenshot step gained a `crop` switch (immediate cropped window capture for transient visuals like damage numbers and lightning); (4) the BUG2 case gained a positive **natural decay** check (wait 31 s with no intervention) plus an effect-duration readout assertion; (5) both KubeJS probes' `status` now emits `AP_<tag>_EMPOWER_DUR`; (6) added `scripts/devtools/Start-MtDetached.ps1`, which runs phases that leave long-lived descendants (Gradle daemon / game client) in a separate console with file logging, so a non-interactive caller no longer blocks on the entire process tree until timeout; (7) `mt_env world` now enforces the test-world rules — `Data.allowCommands=1` (TAG_Byte) and `Data.GameRules.keepInventory="true"` (TAG_String, building the `GameRules` compound with an Ordinal comparer when it is absent) — writing them on **both** the freshly generated world and the seed-restored world, and any rule that cannot be written ends in `MT_WORLD: BLOCKED` with exit code 11 (previously only `allowCommands` was written and the seed-restore path dropped the rules altogether, so deaths during testing dropped items and repeatedly interrupted experiments); (8) `mt_case.ps1` now **automatically** records each case's result into the report state (`mt_report mark --case`) right after it finishes — previously this had to be done by hand in the orchestration layer, and missing it once made `mt_report summary` permanently report "未执行 / 0 entries" and fail an all-green run with exit code 1; (9) the BUG5 Railgun case was extended into a full functional check: it now measures the **real delay** (a lightning-bolt baseline is taken before the hit and the tick offset of the first new bolt is recorded → `DELAY_MEASURED:20`) and adds a **cooldown guard** (probe command `cdguard`: hitting a hostile target during the 1:00 cooldown must consume no Charge, schedule no pending strike and not reset the remaining cooldown, plus two negative observation windows (100 and 400 ticks, `LIGHTNING_MAX:0` + `WATCH_TIMEOUT`); the guard is exercised twice, early and mid-cooldown (`CD_BEFORE` 1037 / 687 ticks), and re-checked with only 142 ticks of cooldown left (Charge still 6, zero lightning entities nearby).
...
- Migrated the entire test toolchain (`scripts/test/`, `scripts/verify/`, `scripts/audit/`, `tools/`, `scripts/maintenance/`) from bash + python to PowerShell 7: all 28 original scripts were rewritten as `.ps1` (plus a new shared module layer `lib/Mt.{Conf,Paths,Phase,Proc,Win32}.psm1`), the single entry point is now `pwsh -NoProfile -File scripts/test/mt.ps1`, judgement logic and output stay **byte-equivalent** (the comparison only folds the Python side's CRLF), the CI lang-sync check now runs under `pwsh`, and the interpreter entries in `mt.conf` (`MT_PYTHON`/`MT_NODE`) are obsolete; the 28 original bash/python files were deleted (history stays in git) and the four git self-healing hooks now call the pwsh version.
...
- Completed the Bountiful bounty-board integration data (the 1.2.0 additions had been left entirely unsynced): `astral_objs` gained 7 requirement entries (Glass / Netherrack / Emerald / Obsidian / Weird / Amethyst Dice, Star Coin Bag) and `astral_rews` gained 24 reward entries (those 7 plus the Moses / Pandaman signs, the Fight-Poison-with-Poison effect card, and 13 new chips), bringing the pools to 13 (objs) / 92 (rews) entries. Pool membership is now a closed set by rule (**legendary-rarity dice, chips and signs never enter any pool**, currency and cards excepted), and values are derived from dice tier (T1 gold-tier 3000/2800, T2 diamond-tier 8000/7500; the former for `astral_objs`, the latter for `astral_rews`) and item rarity within the established value bands. Added a read-only gate script `scripts/verify/verify_bountiful_pools.py` (set equality / two-layer rarity mapping / byte-identical dual versions / value-balance formula; exit code 0 = consistent) and expanded the membership rules and value bands in `AGENTS.md` and `docs/bountiful-integration.md` (both loaders).
- Removed the dead event-system chain (zero call sites repo-wide): the event-type registry and trigger entry points (`AstralEvents` / `AstralEventType` / `EventContext` / `EventEffect` / `AstralEventSystem.trigger`), the armour-penalty handler and its `armor_penalty_end` attachment, the orphan `lib/junit-platform-console-standalone-6.1.2.jar`, and 3 unreferenced lang keys (`event.astral_dice.random_buff` / `reduce_armor`, `msg.astral_dice.event_triggered`); the live post-event hooks (Detective's +3 Star Coins / Investigator's Living Page) are kept as-is, so **behaviour is unchanged** (both loaders).
