...
- 新增骰子「下界之星骰子」:全新 T4 奇异品阶(`astral_dice:dice_t4`),可由任意 T3 骰子(下界合金/绯红/末影,经 `dice_t3` 标签)升级——配方为外圈 4 颗下界之星十字环绕(`" N " / "NDN" / " N "`,N=下界之星,D=T3 骰子);筹码栏 = 4+s(0★4/1★5/2★6/3★7,比 T3 多 1 格);卡牌槽与费用点数始终按最高档 3★ 配置(卡牌栏 12 格/攻防费用上限各 6,经 `DiceCurioItem.configStarLevel`,菜单持久化时保留实际星级);骰子每提升 1 星级攻击力 +2、防御力 +2(折算护甲 +4,经 `NetherStarDiceItem` 瞬态属性修饰器实时刷新);物品自带附魔光效(`isFoil` 恒真)(仅 1.21.1)。 / New "Nether Star Die": a brand-new exotic T4 tier (`astral_dice:dice_t4`), upgradeable from any T3 die (Netherite/Crimson/Ender via the `dice_t3` tag) — 4 Nether Stars in a cross around it (`" N " / "NDN" / " N "`, N=Nether Star, D=T3 die); chip slots = 4+s (0★4/1★5/2★6/3★7, one more than T3); card slots and cost points are always configured as the top 3★ tier (12 card slots / cost cap 6 per side, via `DiceCurioItem.configStarLevel`, while the menu persists the actual star); each star level grants +2 attack and +2 defense (+4 armor via transient attribute modifiers refreshed by `NetherStarDiceItem`); the item has a permanent enchanted glint (`isFoil` always true) (1.21.1 only).
- 新增骰子「末影骰子」:与下界合金骰子同阶(`astral_dice:dice_t3`),可由下界合金骰子升级(ZYZ/YDY/ZYZ,Z=末影珍珠,Y=末影之眼,D=下界合金骰子);受到致命伤害时触发一次不死图腾效果(复活至 1 生命、清除全部效果、生命恢复 II 0:45、伤害吸收 II 0:05、火焰抗性 0:40,与原版图腾一致;图腾粒子/音效/手持高亮动画使用末影骰子图标),随后进入 5:00 冷却(经 `EnderDiceHandler` 在 `LivingDeathEvent` 取消死亡并附加到玩家);不死图腾触发后会尝试瞬移到 16 格内安全地面(排除空气/水面/船上,玩家在水中不触发传送),并播放末影粒子与传送音效;但装备期间雨中/水下(`isInWaterRainOrBubble`)受到的伤害 +40%(在 `LivingDamageEvent.Pre` 最终减免后放大)(仅 1.21.1)。 / New "Ender Die": same tier as the Netherite Die (`astral_dice:dice_t3`), upgradeable from it (ZYZ/YDY/ZYZ, Z=Ender Pearl, Y=Eye of Ender, D=Netherite Die); lethal damage triggers one totem-of-undying effect (revive to 1 HP, clear all effects, Regeneration II 0:45, Absorption II 0:05, Fire Resistance 0:40 — matching the vanilla totem, with the totem particles/sound/held-up animation using the Ender Die icon), followed by a 5:00 cooldown (via `EnderDiceHandler`, cancelling `LivingDeathEvent` and attached to the player); after the totem triggers, it attempts to teleport to safe ground within 16 blocks (no air/water-surface/boat destinations, and no teleport while the player is in water), with ender particles and a teleport sound; while equipped, damage taken in rain/underwater (`isInWaterRainOrBubble`) is increased by 40%% (applied after final reductions in `LivingDamageEvent.Pre`) (1.21.1 only).
- 新增骰子「紫晶骰子」:与钻石骰子同阶(`astral_dice:dice_t2`),可由钻石骰子升级(AAA/ADA/AAA,A=紫水晶块,D=钻石骰子);远程/魔法攻击命中时也触发战斗骰(1-6)并追加骰点伤害(经 `SpellDamageRegistry` 法伤修饰器统一结算:作用域沿用白名单——箭矢/投掷物/原版与联动模组魔法,与法伤效果牌同一链路),但不触发骰神赐福、不消耗卡牌耐久(仅 1.21.1)。 / New "Amethyst Die": same tier as the Diamond Die (`astral_dice:dice_t2`), upgradeable from it (AAA/ADA/AAA, A=Block of Amethyst, D=Diamond Die); ranged/magic attacks also roll the combat die (1-6) and add its roll as damage (settled through the `SpellDamageRegistry` spell-damage modifier chain, sharing the whitelisted scope — arrows/throwables/vanilla and linked mod magic — with the spell-damage effect cards), without triggering a Dice Blessing or consuming card durability (1.21.1 only).
- 新增骰子「绯红骰子」:与下界合金骰子同阶(`astral_dice:dice_t3`),可由下界合金骰子升级(YJY/JDJ/YJY,Y=下界疣块,J=绯红菌,D=下界合金骰子);战斗骰高点数(4-6)出现概率提升 50%(权重法:4/5/6 权重 ×1.5,P(高点数) 50%→60%,对攻击骰与玩家防御骰均生效;攻击方掷骰/玩家防御骰/反击伤害掷骰统一经 `rollCombatDie` 分发至 `CrimsonDiceHandler.rollD6`),但掷出 1 点时立即受到 6 点伤害(骰子伤害类型,与王之力自伤同一来源)(仅 1.21.1)。 / New "Crimson Die": same tier as the Netherite Die (`astral_dice:dice_t3`), upgradeable from it (YJY/JDJ/YJY, Y=Nether Wart Block, J=Crimson Fungus, D=Netherite Die); high combat-die rolls (4-6) are 50%% more likely (weighted: faces 4-6 ×1.5, P(high) 50%%→60%%, applies to attack rolls and player defense rolls), but rolling a natural 1 immediately deals 6 damage to you (dice-damage type, the same source as King Power's self-damage) (1.21.1 only).
- 新增骰子「诡异骰子」:与钻石骰子同阶(`astral_dice:dice_t2`),可由钻石骰子升级(CJC/JDJ/CJC,C=垂泪藤,J=诡异菌,D=钻石骰子);立牌主动技能冷却时间 -50%(所有开始冷却入口统一经 `WeirdDiceHandler.signCooldownTicks` 取值),但战斗骰低点数(1-3)出现概率提升 50%(权重法:1/2/3 权重 ×1.5,P(低点数) 50%→60%,对攻击骰与玩家防御骰均生效)(仅 1.21.1)。 / New "Weird Die": same tier as the Diamond Die (`astral_dice:dice_t2`), upgradeable from it (CJC/JDJ/CJC, C=Weeping Vines, J=Warped Fungus, D=Diamond Die); sign active-skill cooldowns are halved, but low combat-die rolls (1-3) are 50%% more likely (weighted: faces 1-3 ×1.5, P(low) 50%%→60%%, applies to attack rolls and player defense rolls) (1.21.1 only).
- 新增骰子「下界岩骰子」:与黄金骰子同阶(`astral_dice:dice_t1`),可由黄金骰子升级(NNN/NTN/NNN,N=下界砖,T=黄金骰子);佩戴后在下界采集矿物(石英矿/下界金矿/远古残骸)时有 30% 概率额外掉落 1 枚星币、5% 概率额外掉落 1 个星盘(同一次采矿最多一种,先判星盘),且猪灵对佩戴者保持中立(经 `PiglinAiMixin` 视同身穿金甲)(仅 1.21.1)。 / New "Netherrack Die": same tier as the Golden Die (`astral_dice:dice_t1`), upgradeable from it (NNN/NTN/NNN, N=Nether Brick, T=Golden Die); mining Nether ores (quartz/gold/ancient debris) has a 30% chance to drop a Star Coin and 5% to drop a Star Plate (at most one per block, plate checked first), and piglins stay neutral to the wearer (via `PiglinAiMixin`, treated as wearing gold armor) (1.21.1 only).
- 新增骰子「黑曜石骰子」:与钻石骰子同阶(`astral_dice:dice_t2`),可由钻石骰子升级(OOO/ODO/OOO,O=黑曜石,D=钻石骰子);佩戴后获得 3 点基础防御力(按 1 防御力 = 2 护甲折算为 +6 护甲属性修饰器),且受到的火焰伤害(原版 `is_fire` 标签)减少 70%;因加入 `dice_t2` 标签,可作为母体合成更高阶的下界合金骰子(仅 1.21.1)。 / New "Obsidian Die": same tier as the Diamond Die (`astral_dice:dice_t2`), upgradeable from it; grants 3 base defense (+6 armor via attribute modifier, 1 defense = 2 armor) and reduces fire damage taken (vanilla `is_fire` tag) by 70%; being in the `dice_t2` tag it can craft the higher-tier Netherite Die (1.21.1 only).
...
- 创造物品栏骰子顺序调整:玻璃骰子、下界岩骰子移到下界合金骰子之后(仅 1.21.1)。 / Creative tab dice order adjusted: Glass and Netherrack dice now appear after the Netherite die (1.21.1 only).
...
- 修复新增骰子 tooltip 缺失:黑曜石/下界岩/诡异/绯红/紫晶/末影/下界之星骰子此前未进入骰子 tooltip 分发入口,导致这些骰子缺少专属说明;现已统一纳入骰子 tooltip 处理(仅 1.21.1)。 / Fixed missing tooltips on newer dice: Obsidian, Netherrack, Weird, Crimson, Amethyst, Ender, and Nether Star dice were not entering the dice tooltip branch, so their unique descriptions were absent — they are now handled as dice (1.21.1 only).
- 修复骰子 tooltip 染色在 `%%`/占位符转译后断裂的问题:含 `%%` 或 `%s` 的骰子 tooltip(如绿宝石骰子的 20%% 折扣)改为先经 `translationString` 展开再放入 `Component.literal`,避免 Minecraft 将 `%%`/`%s` 拆成独立无样式片段,导致 `%` 号与后续文字丢失正确颜色(仅 1.21.1)。 / Fixed dice-tooltip coloring breaking around translated `%%`/placeholder tokens: dice tooltips containing `%%` or `%s` (e.g. the Emerald die's 20%% discount) are now expanded via `translationString` into a single `Component.literal`, so Minecraft no longer splits the format token into an unstyled fragment and the `%` sign and following text keep the intended colors (1.21.1 only).
- 修复所有骰子 tooltip 中「骰神赐福 + 时间」染色中断:通用 `dice_desc` 行不再把时间放进会被翻译/占位符拆断的彩色区间,改为拆成「前缀/赐福效果+时间/中间说明/按键/后缀」独立染色组件,确保「骰神赐福 (60秒)」整体保持蓝色(仅 1.21.1)。 / Fixed the broken coloring of “Dice Blessing + duration” in every die tooltip: the shared `dice_desc` line no longer places the duration inside a legacy-color range that translation/placeholder handling can split; it is now assembled from independently styled fragments (prefix / blessing+duration / middle / key / suffix), so “Dice Blessing (60s)” stays fully blue (1.21.1 only).
- 修复骰子 tooltip 数字高亮后文字被误染为灰色(含新增骰子专属说明与铁砧升星提示):这些 tooltip 原用 `§7`(灰色)作为“恢复”标记,导致金/红/绿/暗绿等条目主色在数字后变成灰色;已改为 `§r`(真正重置回该条目主色),使数字后面的说明文字与条目原本颜色一致(仅 1.21.1)。 / Fixed text after numeric highlights in die tooltips being wrongly colored gray (including newer die unique descriptions and the anvil star-upgrade hint): they used `§7` (gray) as the “restore” marker, so gold/red/green/dark-green lines turned gray after the highlighted number; changed to `§r` (true reset back to the line’s own color), keeping the following text consistent with each tooltip’s intended color (1.21.1 only).
- 删除紫晶骰子 tooltip 中易歧义的「不会触发骰神赐福」条目,仅保留“远程/魔法攻击也触发战斗骰并追加骰点伤害”(仅 1.21.1)。 / Removed the potentially misleading “Does not trigger a Dice Blessing” line from the Amethyst die tooltip, keeping only the ranged/magic combat-die description (1.21.1 only).
- 修复部分战斗牌 tooltip 的耐久度数字未染黄:中/大/特大/暗影突袭/名刀/蓄力/防御中/防御大/防御特大仍走 `Component.translatable`,使 `%s` 处的耐久度变成无样式片段;已统一改为 `tt()` 先展开再以单字面量渲染,剩余次数数字保持黄色(仅 1.21.1)。 / Fixed durability numbers on some combat-card tooltips not being colored yellow: medium/large/epic/Shadow Strike/Meito/Charge/defense medium/large/epic still used `Component.translatable`, so the `%s` durability value became an unstyled fragment; they now use `tt()` to expand first and render as one literal, keeping the remaining-uses number yellow (1.21.1 only).
