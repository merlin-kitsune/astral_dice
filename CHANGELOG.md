...
- 手册「基本介绍」分类图标为「头号玩家立牌」;「材料」分类图标为星币;「卡牌」分类图标为攻击-特大(双版本一致)。 / Handbook icons (both versions): "Getting Started" uses the No.1 Player Sign, "Materials" uses the Star Coin, and "Cards" uses the Epic Attack Card.
...
### 已修复BUG / Bug Fixes
- **双版本同步**:立牌/筹码/骰子的右键行为统一为仅装备或替换装备——移除帕鲁南(经商)立牌手持右键触发主动技能的意外路径(主动技能统一由快捷键/立牌栏触发);同步修复 1.21.1 骇客立牌 `onCurioTick` 隐身到期判断恒真问题(附件默认 0 时每 tick 误删其他来源的隐身,与 1.20.1 已修复行为一致)。 / **Both versions synced**: right-click on signs/chips/dice now only equips or replaces equipment — removed the Parunan sign's in-hand right-click active-skill path (actives are now triggered solely via the keybind/equipped path); synced the 1.21.1 Nancy Lu `onCurioTick` always-true invisibility-expiry check to the 1.20.1 fix (no longer strips invisibility from other sources every tick).
...
### 工程 / Project
- 版本号更新为 `1.1.3-pre1+forge_1.20.1`(仅 1.20.1;1.21.1 保持 `1.1.3-rc1+neoforge_1.21.1`),并重新编译发布至 1.20.1 整合包(自动清理旧版本产物)。 / Version bumped to `1.1.3-pre1+forge_1.20.1` (1.20.1 only; 1.21.1 stays `1.1.3-rc1+neoforge_1.21.1`) and rebuilt/published to the 1.20.1 modpack (old artifacts auto-cleaned).
