...
- 新增骰子「下界之星骰子」:全新 T4 奇异品阶(`astral_dice:dice_t4`),可由任意 T3 骰子(下界合金/绯红/末影,经 `dice_t3` 标签)升级——配方为外圈 4 颗下界之星十字环绕(`" N " / "NDN" / " N "`,N=下界之星,D=T3 骰子);筹码栏 = 4+s(0★4/1★5/2★6/3★7,比 T3 多 1 格);卡牌槽与费用点数始终按最高档 3★ 配置(卡牌栏 12 格/攻防费用上限各 6,经 `DiceCurioItem.configStarLevel`,菜单持久化时保留实际星级);骰子每提升 1 星级攻击力 +2、防御力 +2(折算护甲 +4,经 `NetherStarDiceItem` 瞬态属性修饰器实时刷新)(仅 1.21.1)。 / New "Nether Star Die": a brand-new exotic T4 tier (`astral_dice:dice_t4`), upgradeable from any T3 die (Netherite/Crimson/Ender via the `dice_t3` tag) — 4 Nether Stars in a cross around it (`" N " / "NDN" / " N "`, N=Nether Star, D=T3 die); chip slots = 4+s (0★4/1★5/2★6/3★7, one more than T3); card slots and cost points are always configured as the top 3★ tier (12 card slots / cost cap 6 per side, via `DiceCurioItem.configStarLevel`, while the menu persists the actual star); each star level grants +2 attack and +2 defense (+4 armor via transient attribute modifiers refreshed by `NetherStarDiceItem`) (1.21.1 only).
