...
- Added first-join rulebook option: new config entry `giveGuideBookOnFirstJoin` (default `true`) backed by a GameplayConstants field; when enabled, players receive the Koi's Rulebook (Patchouli guide) on first join, once per player per world (both loaders).
...
- Configuration decoupled, then **rewritten on Cloth Config**: the shared `GameplayConstants` no longer reads a loader-specific config (`ModConfigSpec` / `ForgeConfigSpec` are not interchangeable), and the library provides the platform-neutral value snapshot `GameplayConfigValues` (a record) plus `GameplayConstants.applyConfig(...)`. The config module itself now lives in the library as `StarEngineCommonConfig` — a Cloth **AutoConfig** POJO (`@Config(name = "astral_dice")`, 13 entries covering every former option) — and Cloth's `AutoConfig` owns both file IO and the config GUI: `StarEngineConfigs.register()` calls `AutoConfig.register(...)` with `GsonConfigSerializer` and wires load/save listeners that feed the snapshot into `GameplayConstants.applyConfig(...)`, while the in-game screen is reached through each loader's own extension point (`IConfigScreenFactory` on NeoForge, `ConfigScreenHandler.ConfigScreenFactory` on Forge; the `Screen`-touching factory is isolated in `StarEngineConfigScreen` so the server-side classpath never links a client-only class). The former `ModCommonConfig` / `GameplayConfigBinder` classes were deleted from both platforms. **Breaking — the config file changes from `config/astral_dice-common.toml` (TOML, flat snake_case) to `config/astral_dice.json` (JSON, nested camelCase): the `.json` suffix is hard-coded by `GsonConfigSerializer`, and the `astral_dice` namespace fixes both the file name and the lang-key prefix `text.autoconfig.astral_dice.*`.** A one-shot `LegacyCommonTomlImporter` seeds the new JSON from the old TOML (only when the JSON does not yet exist) and renames the old file to `astral_dice-common.toml.bak`; any parse failure falls back to defaults instead of failing startup. Values hand-edited into the JSON are clamped in `validatePostLoad()`, because `@ConfigEntry.BoundedDiscrete` only constrains the GUI (both loaders).
- Added **Cloth Config** (`cloth_config`) as a required prerequisite, for the library and for this mod alike. The library compiles against `cloth-config-neoforge` 15.0.140 / `cloth-config-forge` 11.1.136; the two were byte-diffed and their `ConfigBuilder` / `ConfigEntryBuilder` / `autoconfig.*` surfaces are identical, which is what makes one shared schema viable. The Forge jar carries SRG method names (819 `m_xxxxx_` names), so it is pulled in with `modImplementation` and remapped by MDG LegacyForge — the same mechanism as Curios; the NeoForge jar is official-mapped and needs no remapping (`implementation`). Both `mods.toml` files declare `cloth_config` as required alongside `starengine_lib` (`type="required"` / `mandatory=true`, `versionRange="[15,)"` / `"[11,)"`, `ordering="AFTER"`), and 31 `text.autoconfig.astral_dice.*` lang keys (title + 4 categories + 13 options + 13 tooltips) were added to all four lang files. Because a published library jar does not expose its own dependencies transitively, both consumer `build.gradle` files also declare the Cloth Config dependency explicitly. The library was renumbered to `1.0.0-SNAPSHOT.1` (the build that first ships the config module and the first version published outside `mavenLocal`) and then to **`1.0.0-SNAPSHOT.2`** by the package rename below; both consumer `gradle.properties` were synced (both loaders).
...
- The tag rule now skips pre-releases/snapshots: the auto-tag step in both repositories' `build.yml` aborts when the version contains `-`. Previously `mod_version=2.0.0-SNAPSHOT.5` would have been truncated to `2.0.0` and silently tagged as a release on a `main` push. The mod version was bumped to `2.0.0-SNAPSHOT.5` and the library to `1.0.0-SNAPSHOT.2` (both loaders).
