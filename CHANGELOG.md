...
- 新增「反击」流派(玩家效果,图标 `temp/反击.png`):拥有反击层数时被近战敌对生物攻击触发一次反击——视为玩家近战攻击,按 手持最高近战武器基础伤害 + 1d6 骰点 + 攻击牌加成(未赐福时自动触发骰神赐福并消耗攻击牌耐久)+ 攻击力加成 计算总伤害并对攻击目标造成伤害,随后移除 1 层;魔法伤害(唤魔者尖牙/守卫者光束等)不算近战,不触发;首个层数来源:夹心饼干-美味(生命值低于最大生命值一半时,每 1:00 获得 1 层「反击」)(仅 1.21.1)。
- 新增筹码「肾上腺素-一般/高效」(史诗/传奇):生命值低于最大生命值一半时,攻击力/防御力 +3/+8(防御按 1 点 = 2 点护甲折算为护甲 +6/+16);高效额外:触发加成时被敌方攻击,掷 1d6——骰点 4-5 → 50% 概率闪避本次伤害、骰点 6 → 100% 闪避。配方:一般 = ZXZ/DCD/PPP(Z=再生药水,X=下界之星,D=凋零玫瑰,C=空白筹码,P=星盘);高效 = RZR/ZOZ/PPP(R=红石粉,Z=钻石,O=肾上腺素-一般,P=黄金星盘,紫→金升级式)(仅 1.21.1)。
- 修复骰子 tooltip 显示攻击牌时加成误标为「骰子」及染色问题:攻击牌(中/大/特大/名刀)统一显示为「攻击」,范围不再带「+」前缀,与防御牌/独立牌 tooltip 格式一致(仅 1.21.1)。
- 占星师/秘密侦探立牌主动技能迁移至目标选择器(仅 1.21.1):「虚弱印记」「隐匿行动」由「下次攻击释放」改为使用目标选择器选择目标施加;目标规则=敌对生物或非队友玩家(选择者无队伍时对所有玩家生效),不符合规则的目标无法被选择;确认目标后施加并开始玩家级冷却,取消/超时不冷却;清理旧「等待器」机制(占星师=1/秘密侦探=2 的 SIGN_READY_TYPE/EXPIRE 废弃,`haiqing_ready`/`bonnie_ready` 待命效果与 ReadyEffect 移除,攻击不再自动释放印记)。 / The Astrologer and Secret Detective signs' active skills migrated to the Target Selector (1.21.1 only): "Weak Mark" and "Undercover Action" are now applied via the target selector instead of on the next attack; the target rule is hostile mobs or non-team players (all players if the selector has no team), invalid targets cannot be selected; applying starts the player-level sign cooldown, while cancel/timeout does not; the legacy "waiting" system was removed (SIGN_READY_TYPE/EXPIRE deprecated, `haiqing_ready`/`bonnie_ready` ready effects and ReadyEffect removed, marks are no longer applied automatically on attack).
- 新增「目标选择器」通用框架(仅 1.21.1):立牌主动技能/效果牌可通过注册 `TargetSelectionAction` 并调用 `TargetSelectionManager.start` 进入第一人称目标选择模式——准星射线瞄准(按动作限定 玩家/敌对/任意活体),目标高亮描边仅选择者可见(友方绿/敌对红/中立黄,经 `RenderLevelStageEvent.AFTER_ENTITIES` 渲染,Sodium/Iris 兼容);确认键=鼠标右键或 Enter(可改绑),取消=Esc 或再次按主动技能键;选择期间接管攻击/使用、拦截滚轮与第三方模组按键(键盘经 `KeyboardHandler` Mixin,含 F3/E/T/H、Xaero/FTB 等),保留移动;选择半径由配置 `target_select_radius` 控制(默认 16 格,上限 32);调试命令 `/astral_dice targetselect <player|enemy|living>`(OP,演示动作,暂不接入现有立牌/效果牌)。 / A generic Target Selector framework was added (1.21.1 only): sign active skills / effect cards can register a `TargetSelectionAction` and call `TargetSelectionManager.start` to enter first-person target-selection mode — crosshair raycast aiming (restricted per action to players / hostiles / any living), a target outline visible only to the selector (friendly green / hostile red / neutral yellow, rendered at `RenderLevelStageEvent.AFTER_ENTITIES`, Sodium/Iris compatible); confirm with right-click or Enter (rebindable), cancel with Esc or by pressing the sign-active key again; while selecting, attack/use and scrolling are taken over and third-party keybinds are blocked (keyboard via a `KeyboardHandler` Mixin, incl. F3/E/T/H, Xaero/FTB, etc.) while movement is preserved; selection radius is controlled by the `target_select_radius` config (default 16, capped at 32); a debug command `/astral_dice targetselect <player|enemy|living>` (OP, demo action, no existing signs/effect cards wired yet).
...
- 目标选择器相关代码全部增加调试 LOGGER(`[Astral Dice][TargetSelection*]` 前缀,dev run 可见);新增 Mixin 基建(`astral_dice.mixins.json` + mods.toml `[[mixins]]` + compileOnly `org.spongepowered:mixin:0.8.5`,NeoForge 运行时 Mojmap 无需 refmap);公共配置版本 12→13(新增 `target_select_radius`);自动化测试流程(scripts/test + Sodium/Iris 测试环境 + 本地 MCP computer-control-mcp)详见 AGENTS「自动化测试流程」子配置。 / Debug LOGGERs were added to all target-selector code (`[Astral Dice][TargetSelection*]` prefix, visible in the dev run); Mixin infrastructure was added (`astral_dice.mixins.json` + `[[mixins]]` in mods.toml + compileOnly `org.spongepowered:mixin:0.8.5`, no refmap needed since NeoForge runs Mojmap); common config version bumped 12→13 (new `target_select_radius`); an automated testing flow (scripts/test + Sodium/Iris test environment + local MCP computer-control-mcp) is documented in the AGENTS "Automated Testing" sub-config.
...
- New Counterattack playstyle (player effect, icon `temp/反击.png`): with Counterattack layers, being hit by a hostile melee attack triggers a counter strike treated as your own melee attack — total damage = highest held melee weapon base damage + a 1d6 dice roll + attack-card bonus (auto-triggering the Dice Blessing and consuming card durability when you are not blessed) + all attack bonuses, dealt to the attacker, then 1 layer is removed; magic damage (evoker fangs, guardian beams, etc.) does not count as melee and does not trigger; first layer source: Sandwich (Deluxe) grants 1 Counterattack layer per 1:00 while below half max HP (1.21.1 only).
- New chips "Adrenaline - Common/High-Grade" (epic/legendary): while below half max HP, attack/defense +3/+8 (defense converts to armor +6/+16 at 1 point = 2 armor); High-Grade extra: while the bonus is active, being attacked by a hostile rolls 1d6 — 4-5 gives a 50%% dodge chance, 6 gives a 100%% dodge for this damage. Recipes: Common = ZXZ/DCD/PPP (Z = Potion of Regeneration, X = Nether Star, D = Wither Rose, C = Blank Chip, P = Star Plate); High-Grade = RZR/ZOZ/PPP (R = Redstone Dust, Z = Diamond, O = Adrenaline - Common, P = Golden Star Plate, the purple-to-gold upgrade form) (1.21.1 only).
...
The Astrologer and Secret Detective signs' active skills migrated to the Target Selector (1.21.1 only): "Weak Mark" and "Undercover Action" are now applied via the target selector instead of on the next attack; the target rule is hostile mobs or non-team players (all players if the selector has no team), invalid targets cannot be selected; applying starts the player-level sign cooldown, while cancel/timeout does not; the legacy "waiting" system was removed (SIGN_READY_TYPE/EXPIRE deprecated, `haiqing_ready`/`bonnie_ready` ready effects and ReadyEffect removed, marks are no longer applied automatically on attack).
- A generic Target Selector framework was added (1.21.1 only): sign active skills / effect cards can register a `TargetSelectionAction` and call `TargetSelectionManager.start` to enter first-person target-selection mode — crosshair raycast aiming (restricted per action to players / hostiles / any living), a target outline visible only to the selector (friendly green / hostile red / neutral yellow, rendered at `RenderLevelStageEvent.AFTER_ENTITIES`, Sodium/Iris compatible); confirm with right-click or Enter (rebindable), cancel with Esc or by pressing the sign-active key again; while selecting, attack/use and scrolling are taken over and third-party keybinds are blocked (keyboard via a `KeyboardHandler` Mixin, incl. F3/E/T/H, Xaero/FTB, etc.) while movement is preserved; selection radius is controlled by the `target_select_radius` config (default 16, capped at 32); a debug command `/astral_dice targetselect <player|enemy|living>` (OP, demo action, no existing signs/effect cards wired yet).
...
- Debug LOGGERs were added to all target-selector code (`[Astral Dice][TargetSelection*]` prefix, visible in the dev run); Mixin infrastructure was added (`astral_dice.mixins.json` + `[[mixins]]` in mods.toml + compileOnly `org.spongepowered:mixin:0.8.5`, no refmap needed since NeoForge runs Mojmap); common config version bumped 12→13 (new `target_select_radius`); an automated testing flow (scripts/test + Sodium/Iris test environment + local MCP computer-control-mcp) is documented in the AGENTS "Automated Testing" sub-config.
