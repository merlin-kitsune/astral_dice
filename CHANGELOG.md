...
- 新增骰子「下界岩骰子」:与黄金骰子同阶(`astral_dice:dice_t1`),可由黄金骰子升级(RRR/RTR/RRR,R=下界岩,T=黄金骰子);佩戴后在下界采集矿物(石英矿/下界金矿/远古残骸)时有 30% 概率额外掉落 1 枚星币、5% 概率额外掉落 1 个星盘(同一次采矿最多一种,先判星盘),且猪灵对佩戴者保持中立(经 `PiglinAiMixin` 视同身穿金甲)(仅 1.21.1)。 / New "Netherrack Die": same tier as the Golden Die (`astral_dice:dice_t1`), upgradeable from it; mining Nether ores (quartz/gold/ancient debris) has a 30% chance to drop a Star Coin and 5% to drop a Star Plate (at most one per block, plate checked first), and piglins stay neutral to the wearer (via `PiglinAiMixin`, treated as wearing gold armor) (1.21.1 only).
- 新增骰子「黑曜石骰子」:与钻石骰子同阶(`astral_dice:dice_t2`),可由钻石骰子升级(GPG/PDP/GPG,G=黑曜石,P=星盘,D=钻石骰子);佩戴后获得 3 点基础防御力(按 1 防御力 = 2 护甲折算为 +6 护甲属性修饰器),且受到的火焰伤害(原版 `is_fire` 标签)减少 70%;因加入 `dice_t2` 标签,可作为母体合成更高阶的下界合金骰子(仅 1.21.1)。 / New "Obsidian Die": same tier as the Diamond Die (`astral_dice:dice_t2`), upgradeable from it; grants 3 base defense (+6 armor via attribute modifier, 1 defense = 2 armor) and reduces fire damage taken (vanilla `is_fire` tag) by 70%; being in the `dice_t2` tag it can craft the higher-tier Netherite Die (1.21.1 only).
