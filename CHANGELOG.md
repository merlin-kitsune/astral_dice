...
- Added a unified tooltip coloring rule (non-time numbers coloured yellow `§e` including adjacent signs; time values coloured blue `§9`, covering both `M:SS` and `N seconds`/`Ns`; entries shaped "<effect name> (<duration>)" fully blue; negative values stay red `§c`, reset codes `§r`, key hints `§f` and inline semantic colours are preserved; precedence 3 > 2 > 1). Both loaders' language files and hard-coded tooltips have been processed in full (audit script and usage: `scripts/test/TESTING-SPEC.md` §9).
