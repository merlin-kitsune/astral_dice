...
- Configuration decoupled: the shared `GameplayConstants` no longer reads the loader-specific `ModCommonConfig` (`ModConfigSpec` / `ForgeConfigSpec` are not interchangeable); the library now provides a platform-neutral value snapshot `GameplayConfigValues` (a record) plus `GameplayConstants.applyConfig(...)`, and each side gained a `GameplayConfigBinder.refresh()` that reads its own config and feeds the snapshot back. The **config file name and path are unchanged** (`config/astral_dice-common.toml`), so existing configs are not reset (both loaders).
