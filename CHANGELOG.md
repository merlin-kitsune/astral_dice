...
- Balance: the Flashlight chip's "max 1 Starlight per target" is now actually implemented (the description promised it but the code never did, letting players farm Starlight off one target). Granted hostile UUIDs are recorded in the attachment `flashlight_granted_targets` (comma-separated, capped at 256, FIFO) and a target already recorded grants nothing; the record is cleared when the chip is unequipped or the player dies (both loaders).
