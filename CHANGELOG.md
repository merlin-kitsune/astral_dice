...
- 新增骰子「末影骰子」:与下界合金骰子同阶(`astral_dice:dice_t3`),可由下界合金骰子升级(ZYZ/YDY/ZYZ,Z=末影珍珠,Y=末影之眼,D=下界合金骰子);受到致命伤害时触发一次不死图腾效果(复活至 1 生命、清除全部效果、生命恢复 II 0:45、伤害吸收 II 0:05、火焰抗性 0:40,与原版图腾一致;图腾粒子/音效/手持高亮动画使用末影骰子图标),随后进入 5:00 冷却(经 `EnderDiceHandler` 在 `LivingDeathEvent` 取消死亡并附加到玩家);但装备期间雨中/水下(`isInWaterRainOrBubble`)受到的伤害 +40%(在 `LivingDamageEvent.Pre` 最终减免后放大)(仅 1.21.1)。 / New "Ender Die": same tier as the Netherite Die (`astral_dice:dice_t3`), upgradeable from it (ZYZ/YDY/ZYZ, Z=Ender Pearl, Y=Eye of Ender, D=Netherite Die); lethal damage triggers one totem-of-undying effect (revive to 1 HP, clear all effects, Regeneration II 0:45, Absorption II 0:05, Fire Resistance 0:40 — matching the vanilla totem, with the totem particles/sound/held-up animation using the Ender Die icon), followed by a 5:00 cooldown (via `EnderDiceHandler`, cancelling `LivingDeathEvent` and attached to the player); while equipped, damage taken in rain/underwater (`isInWaterRainOrBubble`) is increased by 40%% (applied after final reductions in `LivingDamageEvent.Pre`) (1.21.1 only).
