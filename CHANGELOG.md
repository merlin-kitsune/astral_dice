...
### 工程 / Project
- 移除从未触发的 AstralEvents 事件管线(random_buff/reduce_armor 注册、EventContext/EventEffect/AstralEventType、ArmorPenaltyHandler、事件目标收集中的范围/女仆分支与相关配置)与写而未读的 PlayerResourceRegistry(PlayerResource/ResourceType 与治愈/星光注册实现)(仅 1.21.1)。 / Removed the never-fired AstralEvents pipeline (random_buff/reduce_armor registrations, EventContext/EventEffect/AstralEventType, ArmorPenaltyHandler, range/maid target collection and their config options) and the write-only PlayerResourceRegistry (PlayerResource/ResourceType and the healing/starlight registration stubs) (1.21.1 only).
