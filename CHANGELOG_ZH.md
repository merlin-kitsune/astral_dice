...
- 新增治愈类筹码「大碗炖肉」(`big_bowl_stew_chip`,史诗):骰神赐福效果结束后,对 16 格范围内所有友方目标生效——玩家 +1 点治愈并恢复 2 点生命值;驯服/可骑乘友方生物(宠物、马、猪、炽足兽、骆驼)仅恢复 2 点生命值(治愈点为玩家级资源);图标取自 images/大碗炖肉.png(双版本一致)。
- 新增无流派筹码「会员推荐信」(`member_recommendation_chip`,史诗):每次触发骰神赐福时获得一张随机卡牌(卡池为普通随机池,不含专属牌);图标取自 images/会员推荐信.png(双版本一致)。
...
- 新增无流派筹码「小猪存钱罐」(`piggy_bank_chip`,史诗):每使用 2 张效果牌获得 3 星币(计数与魔法秘典/忍者立牌独立,卸下清除);图标取自 images/小猪存钱罐.png(双版本一致)。
- 新增无流派筹码「智能手表」(`smart_watch_chip`,稀有):物品栏中卡牌数量不足 10 张时,每击杀 1 个敌对目标获得一张随机卡牌(无冷却、无计数,达到阈值后不再发放);图标取自 images/智能手表.png(双版本一致)。
