...
- 新增充能流派基础设施:图标取自 `images/充能.png`(效果纹理 `mob_effect/charge.png`);最大层数写入 `GameplayConstants.CHARGE_MAX_STACKS = 20`;拥有至少 1 层充能时固定获得防御力 +1(折算为真实护甲)并使立牌主动/效果牌冷却时间 -20%(经 `ChargeManager`,接入立牌冷却入口与效果牌冷却),当前暂未接入获得/消耗来源,后续来源统一调用 `ChargeManager.addStacks/consumeOne/removeAll`(双版本一致)。
