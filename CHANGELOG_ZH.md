...
- 新增充能筹码「电流核心」(`current_core_chip`,史诗):①使用主动技能时充能 +1(需指定目标的立牌在其等待释放成功时计入,等待超时未释放不计);②主动技能冷却中按下主动技能键时,按剩余冷却时长占 180 秒的比例消耗充能(每 1/6 一档,最低 1 点、最高 6 点,向上取整)并**立即使主动技能冷却完成**——完成后不直接释放技能,需再次按键使用;充能不足则不生效并提示。tooltip 追加「当前充能」计数器;图标取自 images/电流核心.png。暂未创建配方(双版本一致)。
