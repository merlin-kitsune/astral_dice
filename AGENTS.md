...
| 怪力侦探 sherry | 怪力投掷(2026-09-21 新增,**非选择器类**):把 **12 格内所有敌对目标**按抛物线扔到**玩家面前 2 格内**,**落地之后**对被投掷目标造成 **2 点伤害**并施加 **1 层「标记」**;「推理时间」达 **5 层**时额外造成 **5 点伤害**;落点由玩家**水平视线方向**决定、多目标沿垂直方向均匀铺开(仍在「面前 2 格内」);**冷却基础 120 秒(与枪匠同款链:先按充能上限封顶、再按诡异骰子减半 ⇒ 仍受既有减免)**;投掷飞行期间目标 `noAi` 且 `fallDistance` 归零(`SherryThrowManager.FLIGHT_TICKS` = 10 tick);范围内没有可投掷目标时**零消耗**(不进冷却,只提示 `msg.astral_dice.sherry_no_target`);**不起锁定(生效中)态** | 侦探出击:每攻击一个**最大生命值 ≥ 20** 的**敌对目标**获得 1 层「推理时间」(上限 5);每层攻击力 **+1**、受到伤害 **−1**;**每次骰神赐福结束扣除 1 层**;**跨死亡保留(`.copyOnDeath()` / 白名单 + `DeathPreservedBonuses` 第 4 槽位)、仅卸下立牌归零**;计层只认玩家**直接近战**(`getDirectEntity()` 是本人才计;弹射物/法术/AOE/环境伤害不计)。挚友守护:同队伍内若有玩家装备**人偶师立牌**(按注册 id `astral_dice:hanna_sign` 字符串匹配),则**该玩家**受到的伤害 **−1**(无队伍不生效) |
...
- **文件**：三线 `lang/ja_jp.json`（各 **802 键**；2026-09-22 交付清理后基线，原 805），**以 `zh_cn.json` 为基线**（键集必须完全一致）。`key.categories.astral_dice`（1.20.1/1.21.1）与 `key.category.astral_dice.main`（26.1.2）是**预期平台差异键**，各线各留其一。
