...
⛔ **2026-09-19 用户裁决（覆盖上段）：暂停 26.1.2 版本更新** —— 原话「暂停26.1.2版本更新」。即：在用户另行通知前，**不得**再向 `neoforge-26.1.2` 落地任何内容/修复/迁移（含该线代码、资源、数据、探针与用例），该线**冻结在当前已提交状态**（`127fabd5`，三线功能对等已按 `TESTING-SPEC.md` 附录 A 续 20/22 的实测表达成）。本文其余各处「三线同批」「三线同规则」的要求对 26.1.2 **暂不适用**；`neoforge-1.21.1` + `forge-1.20.1` 两个发布线照常推进（仍按「先 1.21.1、再同步 1.20.1」）。**恢复需用户明确指示**，不得以「顺手对齐」为由自行恢复。⚠️ 冻结期间对 26.1.2 的**只读**取证（构建、冒烟、结构比对）不受限；受限的是**写改动**。📌 **2026-09-21 用户裁决（解除条件明确化）**：冻结**保持到「新功能增加完毕」为止**，届时**解除冻结并执行功能移植** —— 即按下方冻结清单（第 1–18+ 条，含每次发布线新增内容追加的行）与 `docs/compat-26.1.2-neoforge.md` 逐项把发布线已落地内容迁到该线，并补齐三侧读数 diff 一致性测试（`scripts/test/TESTING-SPEC.md` §13.2）。⚠️ **「新功能增加完毕」由用户判定**，代理不得自行解冻；冻结期间仍须**每批在下方清单追加一行**（否则解冻时漏项）。
...
> **实施进展（2026-09-21 补记；26.1.2 侧仍冻结，待迁移项见上方「26.1.2 冻结清单」第 13 / 17 / 18 / 19 条）**：
