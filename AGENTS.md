...
3. 按需覆写:
   - `selectorActionId()`:**是否走目标选择器释放**（缺省 `null` = 自身牌,按下即对自己生效;返回非 null = **手持（主手）即自动开会话**、确认后才生效并消耗;2026-09-25 起四张可对他人使用的牌全部走这条路,**旧的 `canUseOnOtherPlayers()` 已删除**;触发/收官/抑制闩口径见目标选择器规范的 `### 现行口径` 第 14 条);
...
- **效果牌操作方式文案统一(必须遵守,2026-09-25 改口径)**:凡**目标选择器类**效果牌(加急加快 `express_delivery` / 奢华大餐 `luxury_feast` / 狂暴 `berserk` / 你有我有 `you_have_i_have`),tooltip 与手册首句一律写「手持此牌（主手）自动开启目标选择器（可选择 16 格内的玩家，无倒计时），确认后…」,并**按 `allowSelf` 分别写**「按下鼠标右键可对自身使用」(前三张)或「不能对自己使用」(你有我有),句尾统一「移出手持即关闭、未确认不消耗卡牌」。⚠️ **旧写法「使用后进入目标选择模式（30 秒内可选择 16 格内的玩家），确认后…」已随 2026-09-25 手持触发作废**(立牌文案不受影响:立牌仍是「激活后进入目标选择模式（30 秒内…）」)。⚠️ **更旧的「右键-自身使用，下蹲右键-对其他玩家使用」写法同样作废** —— 旧的两段式释放路径(`canUseOnOtherPlayers()` / `findPlayerInFront()`)已随迁移删除,新牌一律走选择器;自身牌(王之力/岿然不动/以毒攻毒/对怪三张/定向爆破/巧克力蛋糕/汉堡/活体书页/命运的指引)tooltip 不写释放手势。
