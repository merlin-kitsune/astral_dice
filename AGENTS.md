...
| 磨刀石 | `whetstone_chip` | 紫 | 生命值为 50% 或更低时:攻击力 +4、受到伤害 -2;生命值 > 1 点时:使受到的伤害不超过剩余生命值(至多扣到剩 1 点,不会因一次伤害被击倒;**该保命能力每次生效后进入 1:00 冷却**(附件 `whetstone_guard_cooldown_end`,只在 cap 实际削减了伤害时写入),冷却中该保护不生效、-2 减伤与攻击力 +4 不受影响;**保命优先级低于安全气囊** —— 致命一击先由气囊接,气囊不可用(未佩戴 / 冷却中 / 充能不足)才轮到本条) |
