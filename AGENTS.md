...
13. **绿洲女王立牌(nardis)+ 临时牌机制(2026-09-27 本批;26.1.2 侧**未改动任何文件**,以下为解冻后待迁移项)**:① **物品** `nardis_sign`(稀有 = `Rarity.RARE`)在 `item/ModItems` + `init/ModCreativeTabs`(RARE 段)+ `datagen/ModItemModelProvider` + `datagen/ModRecipeProvider`(**黄金骰子无星盘档**,shape 照 `padman_sign` 的 `WYW/TET/TDT`,立牌骰子固定中下)+ 生成物 `recipe/nardis_sign.json` 与 `advancement/recipes/misc/nardis_sign.json`(26.1.2 侧目录名为 `recipe/`+`advancement/` 且**另需** `src/generated/clientResources/assets/astral_dice/items/nardis_sign.json` 物品模型定义);② **效果** `nardis_privilege`(女王特权,`effect/NardisPrivilegeEffect`,BENEFICIAL、**有限时长 3600 tick**;注册后该线效果数 34 → **35**;图标 `textures/mob_effect/nardis_privilege.png` 从立牌贴图**逐字节复制**);③ **新数据组件** `temporary_card`(Boolean;1.20.1 走 `ItemDataKey`、1.21.1 走 `DataComponentType`)+ **`component/AppliedStone` 增第 3 分量 `boolean temporary`**(CODEC 用 `optionalFieldOf("temporary", false)` 保老存档兼容;1.21.1 另有 STREAM_CODEC 追加列);④ **临时牌机制的行为代码**(两线各一套,语义必须一致):`item/card/TemporaryCardUtil`(标记读写/`grantRandom`/两类计数/`purgeAll`/tick 自检/唯一容器拦截判据)、`item/card/TemporaryCardPermissiveSlot`(标记接口,卡牌栏槽实现)、`CardItem`(`isFoil` 单层光效 + `onDroppedByPlayer` 拒绝丢弃 + 1.21.1 的 stack-aware `canFitInsideContainerItems(ItemStack)`)、`event/TemporaryCardEvents`(`ItemTossEvent` 取消 + 尽力退还)、`event/PlayerTickEvents`(在 `% 20` 早退**前**调 `temporaryCardTick`)、`event/PlayerLifecycleHandler`(死亡段 `purgeAll` + 移除效果,**必须在掉落之前**)、`screen/CardInventoryMenu`(装配双向透传 `temporary`)、`combat/DiceCombatModifiers`(攻击修饰器 + 防御折算调用)、**`combat/DiceCombatEvents` 的耐久重建两处必须透传 `stone.temporary()`**(否则临时牌扣一次耐久就被"洗"成永久牌);⑤ **容器拦截 mixin**:两线都要 `Slot#mayPlace` + `AbstractContainerMenu#moveItemStackTo`;1.20.1 **额外**需要 `ShulkerBoxSlot#mayPlace`、`ShulkerBoxBlockEntity#canPlaceItemThroughFace`、`BundleItem#add`(该线没有 stack-aware `canFitInsideContainerItems`,禁覆写类型级版本),26.1.2 需按该线 API 重新确认(若已有 stack-aware 钩子则只需前两个);⑥ **玩家附件**:**不新增任何键**(HUD 计时器 = 原生效果实例;临时性 = 物品标记 + `AppliedStone.temporary`)⇒ 附件计数 78/80 与 1.20.1 的 `SYNCED_KEYS` **均不变**;⑦ **文案**:lang 9 键(`item.astral_dice.nardis_sign`、`effect.astral_dice.nardis_privilege`(+`.description`)、`msg.astral_dice.nardis_active`、`tooltip.astral_dice.sign.nardis_active` / `nardis_passive`、`astral_dice.guide.entry.nardis_sign.{1,2,3}`)+ 手册 `entries/signs/nardis_sign.json`;⑧ **测试资产**:探针 `nardi*` 段(两发布线**逐字节相同**;其中骰子组件读写走**双形态容错助手** `nardiEnhOf`/`nardiSetEnh`,以兼容 1.20.1 的 `ItemDataKey` 形态)+ 用例 `NARDIS-SIGN-{1.21.1,1.20.1}.json`;迁移后按 §13.2 做三侧读数 diff。
...
| 绿洲女王 nardis | 女王特权(2026-09-27 新增,**非选择器类**):立即获得 **3 张随机临时牌**(池 = `CardCategory.ALL`,专属牌由既有池逻辑排除;走 `VitaminPillChipItem.giveCard` 发牌漏斗 ⇒ 维生素药丸治愈与教主狐光计层照常触发),并获得「女王特权」效果 **3:00**(HUD 计时器 + 图标 = 立牌贴图;**该效果实例同时是「临时牌是否仍在有效期」的唯一真值**);**临时牌只能装备与使用** —— 带**单层**附魔光效、**不可丢弃**、**不可移入任何其它容器**(白名单 = 玩家物品栏 0..35 / 手中 / 骰子卡牌栏),到期(或效果被移除 / 死亡 / 离线到期后重登)时连同**已装配到骰子里的**一并清除(**骰子上的临时性存在 `AppliedStone.temporary`,物品标记由 `CardInventoryMenu` 双向透传**);背包放不下时只发能放下的张数、**绝不落地**;**不覆写 `startActiveLockOnUse`(刻意不起锁定态)** ⇒ 冷却被减免后可在 3:00 内再次释放,此时**先清空全部旧临时牌再发 3 张新的**;触发后立即进入玩家级冷却(默认 180 秒) | 威压:骰子**已装配**的每张攻击牌使攻击力 +1、每张防御牌使防御力 +1(读时现算:攻击走 `DiceCombatModifiers` 修饰器链 ⇒ 实战与 tooltip/GUI 同源,防御走 `setDefenseArmorBonus` 每 tick 现算、1 防御 = 2 护甲;卸下立牌或卸下卡牌立即失效;临时牌同样计入) |
