...
| 绿洲女王 nardis | 女王特权(2026-09-27 新增,**非选择器类**):立即获得 **2 张战斗牌 + 1 张效果牌**(战斗池 = `CardCategory.BATTLE` 的「攻击+防御」**混合随机 2 张**、效果池 = `CardCategory.EFFECT` 随机 1 张;专属牌由既有池逻辑排除;走 `VitaminPillChipItem.giveCard` 发牌漏斗 ⇒ 维生素药丸治愈与教主狐光计层照常触发),并获得「女王特权」效果 **3:00**(HUD 计时器 + 图标 = 立牌贴图;**该效果实例同时是「临时牌是否仍在有效期」的唯一真值**);**临时牌只能装备与使用** —— 带**单层**附魔光效、**不可丢弃**、**不可移入任何其它容器**(白名单 = 玩家物品栏 0..35 / 手中 / 骰子卡牌栏),到期(或效果被移除 / 死亡 / 离线到期后重登)时连同**已装配到骰子里的**一并清除(**骰子上的临时性存在 `AppliedStone.temporary`,物品标记由 `CardInventoryMenu` 双向透传**);背包放不下时只发能放下的张数、**绝不落地**;**起立牌统一锁定态(冻结,2026-09-27 用户裁决)** —— 临时牌**被全部用光**或 **3:00 到期**之前,主动技能处于「使用中」**不可再释放**,重复释放一律被拒绝,反馈照搬其它立牌(提示「使用中」+ 剩余时间);**释放当刻即开始玩家级冷却(默认 180 秒)并与冻结并行流逝,解冻时不再追加新冷却** ⇒ 默认参数下 3:00 > 180 秒,冻结结束时冷却通常已好、可立即再释放;**牌被全部用光 ⇒ 立刻解冻并同时移除「女王特权」效果**(HUD 计时器消失;语义 = 该效果实例只表示「还有临时牌在有效期内」),3:00 到期则清空剩余临时牌后解冻;「先清空旧临时牌再发新的」保留为**安全网**(正常玩法被冻结挡住,仅在效果被外力移除等异常情形生效) | 威压:骰子**已装配**的每张攻击牌使攻击力 +1、每张防御牌使防御力 +1(读时现算:攻击走 `DiceCombatModifiers` 修饰器链 ⇒ 实战与 tooltip/GUI 同源,防御走 `setDefenseArmorBonus` 每 tick 现算、1 防御 = 2 护甲;卸下立牌或卸下卡牌立即失效;临时牌同样计入) |
