...
7. **数据生成(runData)前必须移开 run 目录中的旧产物 jar**:`run/1.21.1/mods/astral_dice-*.jar`(forge 同理 `run/1.20.1/mods/`,26.1.2 同理 `run/26.1.2/mods/`)会与 `build/classes/java/main` 同时被加载,**遮蔽刚编译的 dev 类**,导致 `runData` 用旧代码生成资源却**不报任何错误**(表现为"改了 Provider 但生成结果没变")。执行 `:neoforge-1.21.1:runData` / `:forge-1.20.1:runData`(26.1.2 为**两段式**:`:neoforge-26.1.2:runClientData :neoforge-26.1.2:runServerData`,且 `run/26.1.2/mods` 里的探针运行时 KubeJS/Rhino/BAT 也要一并移出,见下方 26.1.2 速记第 6 条)前先把该 jar 移出(如 `temp/shadow_jars/`),生成后再 `gradlew build` 重新推回。判定是否被遮蔽:比对 `build/classes/.../ModRecipeProvider.class` 中是否含新增字面量(如新筹码模式串),同时确认生成文件时间戳已更新(不要只看日志的 `written: N`)。
   ...
   - **datagen 增量缓存 `src/generated/**/.cache/` 不得进产物(2026-09-18 实测,三条线统一口径)**:`sourceSets.main.resources` 的 `exclude` 模式是**相对每个 srcDir 根**解析的,写成 `exclude("src/generated/**/.cache")`(仓库路径写法)**永远不命中** ⇒ 本地 forge 发货 jar 的根目录里带着 `.cache/`(3 条目 / 33.5 KB 未压缩 / +12.7 KB 压缩),而 CI 新鲜检出时该目录被 `.gitignore` 排除、产物干净 ⇒ **同一版本本地与 CI 的 jar 字节不一致**(2026-09-18 核查 CurseForge「1.20.1 产物与 26.1.2 内容一致」报告时发现并修复;1.21.1 侧同一错误写法此前只是恰好没有 `.cache` 目录才未暴露)。**必须**统一写成相对模式 **`exclude("**/.cache")`**(与 `neoforge-26.1.2` 既有写法一致)。**判据**:`jar tf <产物>.jar` 中不得出现任何 `.cache/` 条目(forge 的 `build/libs` 发货 jar 与 `build/devlibs` dev jar 都要查)。
...
6. **26.1.2 的游戏内验证必须走本仓库 `scripts/test` 工具链,且已按 26.1.2 适配(2026-09-17 实跑 PASS)**:`computer_control` 的合成点击**进不去 GLFW 窗口**(实测主菜单点不动),键鼠注入只能用 `mt_inject`。五条已固化的事实:
   ...
   - **命令注入的端到端延迟实测 > 2 秒(2026-09-22 实测定量,必须遵守)**:`inject_command` 走的是真实的「按 T 开聊天 → 输入 → 回车」,且探针命令自身也要耗时 ⇒ **「施放之后立刻再注入一条读数」这种用例,两条命令的真实间隔远大于 `wait` 里写的毫秒数**(实测:怪力侦探的落地保险冻结 = 40 tick = 2 秒,而 `signcast` 后写 `wait 600ms` 的 noAi 读数**已经是解冻后**的状态 ⇒ 真实间隔 > 2.5 秒)。⚠️ **凡是「窗口短于约 3 秒」的瞬态行为(冻结 / 无敌帧 / 短冷却 / 闪烁 / 刚落地那一帧),一律不能靠注入式读数取证**,必须让**服务端自己**在 tick 里采样并落盘:KubeJS `ServerEvents.tick` + `console.info`(输出进 `latest.log`,可被 `type=log,source=latest` 断言),持续时间用**采样器自持的 tick 计数器**计算 —— ⚠️ KubeJS 下 `entity.getUUID()` 与 `entity.level.getGameTime()` **都取不到**(实测分别退化成占位符 `idx0` 与 `0` ⇒ `dur` 恒 0)。范本 = `scripts/test/resources/kubejs/26.1.2/server_scripts/astral_bugfix_probe.js` 末尾的 `AP_FREEZE` 采样器(`ssspawn` 时登记被跟踪实体,读数 `dur=50` = 飞行 10 + 冻结 40)。
...
- **文档去第三方名(2026-09-22 用户裁决,必须遵守)**:玩家侧发布说明与两份 CHANGELOG 的**内容描述里不得出现其它模组名**(范围 = **本版本小节**;历史小节是已发布版本的记录,**不回溯改写**,但**新增/修改条目时必须遵守**) —— 含:① 操作类比(把本模组的交互写成「某模组的 XX 式」);② 同类模组点名对比;③ 第三方模组清单(军火 / 经济类只写类别);④ 以他方系统命名本模组内容(`XX 手册` / `XX 奖励池` / `XX 标签` / 配方查看器类的他方名)。中性词基准:`强力胶式 → 点选式`、`帕秋莉手册 → 手册`、`赏金板(奖励池) → 赏金奖励池`、`JEI → 配方查看器`、`饰品栏标签 → 饰品槽标签`。**唯一允许出现第三方模组名的位置 = 安装要求 / 前置 / 联动声明**(`📌 安装要求`、`### 安装要求`:StarEngine Lib / Curios API / Mixin Booster / 可选的 Patchouli、Bountiful 等),因为玩家必须照名安装 —— 该节禁止去名化。⚠️ 服务端与仓库内的技术文档(`AGENTS.md`、`TESTING-SPEC.md`、代码注释)不受此约束,该写 API 名就写(如 `ICurioItem`、`Curios handleDrops`)。
...
**26.1.2 迁移清单**（发布线每次落地新内容都在此追加一行；迁移完成后标注「已迁移 + 日期」。⚠️ 自 2026-09-19 起该线**纳入主线、三线同批**，故本清单只记录「确有例外未落该线」的项；各项是否已落该线**以该项正文的「现状 / 复核」小节为准**；第 22 条记录 2026-09-25 的整批接线补齐，其余各批的迁移情况见该项自身的复核行）：
...
12. **未对齐项一并带上**:t4 的 VER-01(两线英文卡名分歧:冻结值 = `Blessing / Misfortune Talisman`,1.20.1 现为 `Fortune / Misfortune Talisman Card`)、VER-02(1.20.1 手册缺半句)、VER-03(`countFu` 是否含副手)、VER-04(1.20.1 丢弃被拒无 actionbar 反馈)在迁移前若未在发布线统一,**迁移时必须按统一后的口径落线**;若仍不统一,则按平台差异逐条登记理由(见 `AGENTS.md` 实现契约第 12 条与附录 A 续 25)。
13. **绿洲女王立牌(nardis)+ 临时牌机制(2026-09-27 本批;26.1.2 侧**未改动任何文件**,以下为待迁移项)**:① **物品** `nardis_sign`(稀有 = `ASTRAL_DICE_RARE`)在 `item/ModItems` + `init/ModCreativeTabs`(RARE 段)+ `datagen/ModItemModelProvider` + `datagen/ModRecipeProvider`(**黄金骰子无星盘档**,shape 照 `padman_sign` 的 `WYW/TET/TDT`,立牌骰子固定中下)+ 生成物 `recipe/nardis_sign.json` 与 `advancement/recipes/misc/nardis_sign.json`(26.1.2 侧目录名为 `recipe/`+`advancement/` 且**另需** `src/generated/clientResources/assets/astral_dice/items/nardis_sign.json` 物品模型定义);② **效果** `nardis_privilege`(女王特权,`effect/NardisPrivilegeEffect`,BENEFICIAL、**有限时长 3600 tick**;注册后该线效果数 **36 → 37**;图标 `textures/mob_effect/nardis_privilege.png` 从立牌贴图**逐字节复制**);③ **新数据组件** `temporary_card`(Boolean;1.20.1 走 `ItemDataKey`、1.21.1 走 `DataComponentType`)+ **`component/AppliedStone` 增第 3 分量 `boolean temporary`**(CODEC 用 `optionalFieldOf("temporary", false)` 保老存档兼容;1.21.1 另有 STREAM_CODEC 追加列);④ **临时牌机制的行为代码**(两线各一套,语义必须一致):`item/card/TemporaryCardUtil`(标记读写/`grantRandom`/两类计数/`purgeAll`/tick 自检/唯一容器拦截判据 `isPlacementBlocked`,以及**物品面唯一判据** `glint(stack, vanillaGlint)` 与 `fitsInsideContainer(stack, vanillaAllows)`)、`item/card/TemporaryCardPermissiveSlot`(标记接口,卡牌栏槽实现)、**牌的两个根类各覆写一次同一对方法**(牌池含战斗牌与效果牌,而 `CardItem extends Item` 与 `BaseEffectCardItem abstract extends Item` **无共同父类** ⇒ 接口 default 会被类方法压住、无法只写一处):`CardItem`(`isFoil` 单层光效 + `onDroppedByPlayer` 拒绝丢弃 + 1.21.1 的 stack-aware `canFitInsideContainerItems(ItemStack)`)与 `BaseEffectCardItem`(`isFoil` + 1.21.1 的 `canFitInsideContainerItems(ItemStack)`;**2026-09-27 R1/R2 修复**:此前两者都缺 ⇒ 临时**效果牌**既无光效、又能经潜影盒 GUI 槽/收纳袋/组件容器进箱并永久留存,而 1.20.1 因五个 mixin 判据与物品类别无关故无此缺口;覆写体只允许委托上面两个方法,禁止在覆写里另写条件以免再次漂移)、`event/TemporaryCardEvents`(`ItemTossEvent` 取消 + 尽力退还)、`event/PlayerTickEvents`(在 `% 20` 早退**前**调 `TemporaryCardUtil.tick`)、`event/PlayerLifecycleHandler`(死亡段 `purgeAll` + 移除效果,**必须在掉落之前**)、`screen/CardInventoryMenu`(装配双向透传 `temporary`)、`combat/DiceCombatModifiers`(攻击修饰器 + 防御折算调用)、**`combat/DiceCombatEvents` 的耐久重建两处必须透传 `stone.temporary()`**(否则临时牌扣一次耐久就被"洗"成永久牌);⚠️ **2026-09-24 补(三线)**:上述「不可丢弃」原先只做在**服务端** ⇒ 客户端 Q 键会把牌"吃掉"(本地移除 + 服务端静默拒绝 + 无槽位校正包 ⇒ 表现为「牌消失、无掉落物」),容器界面内 Q(`THROW`)则先摘出槽位再「尽力退还」(满包即销毁)。现补 `mixin/client/LocalPlayerDropGuardMixin`(客户端 `LocalPlayer#drop(Z)Z` HEAD,判据同源)、`mixin/container/ContainerClickGuardMixin`(`AbstractContainerMenu#clicked` HEAD,含 `PICKUP`+`-999` 拖出)、`BaseEffectCardItem#onDroppedByPlayer`(与 `CardItem` 对称);`purgeAll` 增补「打开的卡牌栏容器 + 光标栈」两段。⚠️ **26.1.2 半移植缺口已一并补齐**:`CardInventoryMenu` 两个卡牌槽此前**未实现** `TemporaryCardPermissiveSlot`(⇒ 该线临时牌根本无法装备)+ 装配双向**未透传** `temporary` + 该线 `DiceCombatEvents` 耐久重建两处**未透传** `stone.temporary()`。⚠️ **2026-09-24 第二批(自毁机制 + tooltip,三线)**:第三方容器拦不住(判据见下条⑤之后的实测)→ 新增组件 `temporary_card_expires` + `TemporaryCardUtil#purgeOutOfPlace`(每 20 tick 清「非白名单槽位」与「附近 16 格地面掉落物」)+ `realignExpiry`(发放 / 再次释放 / 登录三处对齐);tooltip 增黄字提示行 `tooltip.astral_dice.temporary_card`(lang 811 → **812**)。⑤ **容器拦截 mixin**:两线都要 `Slot#mayPlace` + `AbstractContainerMenu#moveItemStackTo`;1.20.1 **额外**需要 `ShulkerBoxSlot#mayPlace`、`ShulkerBoxBlockEntity#canPlaceItemThroughFace`、`BundleItem#add`(该线没有 stack-aware `canFitInsideContainerItems`,禁覆写类型级版本),26.1.2 需按该线 API 重新确认(若已有 stack-aware 钩子则只需前两个);⑥ **玩家附件**:**不新增任何键**(HUD 计时器 = 原生效果实例;临时性 = 物品标记 + `AppliedStone.temporary`)⇒ 附件计数 78/80 与 1.20.1 的 `SYNCED_KEYS` **均不变**;⑦ **文案**:lang 9 键(`item.astral_dice.nardis_sign`、`effect.astral_dice.nardis_privilege`(+`.description`)、`msg.astral_dice.nardis_active`、`tooltip.astral_dice.sign.nardis_active` / `nardis_passive`、`astral_dice.guide.entry.nardis_sign.{1,2,3}`)+ 手册 `entries/signs/nardis_sign.json`;⑧ **测试资产**(2026-09-27 两线实机 **4/4 用例 PASS**):探针 `nardi*` 段(两发布线**逐字节相同**;其中骰子组件读写走**双形态容错助手** `nardiEnhOf`/`nardiSetEnh`,以兼容 1.20.1 的 `ItemDataKey` 形态;**写组件必须走 `dice.set(key.get(), jsObject)` 且值先转 KubeJS 可 JSON 化的 JS 对象** —— 详见 `TESTING-SPEC.md` 附录 A 本条)+ 命令面 `nardiarm`/`nardiarmclear`(被动装配栏)、`nardiguard <tag> <effect|attack>`(R1/R2 回归:光效 + 栈级/行为面容器的只读取证)、`nardiuseup`/`nardiinv`(把临时牌消耗到 0 / 精确控制主物品栏可用格数,供冻结-解冻与安全门用例)、`nardiequip <tag> <chip|teru>`(筹码/教主立牌脚手架)、`nardilockback <tag> [keep|noeffect]`(F1 回归专用:只把 `sign_active_lock_end` 写到**过去**(`max(1, now-200)`,其余锁定键与冷却一概不动)再显式驱动产品的 `tickSignActiveLock`,`keep`=效果与牌保持/`noeffect`=先经内部通道移除效果作对照 —— 单机无法自然复现「多人在线离线跨越 gameTime」;**冻结机械撤回后本命令无用例使用,保留为通用锁定硬上界脚手架**)、`nardicd <tag>`(只写 `sign_active_cooldown_end = 0`,用于「复位冷却后再释放」,避免用作用域过大的 `reset`)+ 用例 `NARDIS-SIGN-{1.21.1,1.20.1}.json`(各 **177 步/68 断言**,M1–M8;历史:首轮 126/49 → 冻结轮 155/60 → 本轮重写)与 `NARDIS-TEMP-GUARD-{1.21.1,1.20.1}.json`(各 19 步/9 断言);迁移后按 §13.2 做三侧读数 diff。
14. **客户端输入放行修复(手持效果牌时 ESC 菜单按键全部失效,2026-09-27 本批;26.1.2 侧**未改动文件**,但同一 BUG 在该线**仍然存在** ⇒ 迁移时**必须**补同一行守卫)**:根因 = 1.21.1 `client/TargetSelectionClient#onMouseButton(InputEvent.MouseButton.Pre)` 在会话激活时**无条件** `event.setCanceled(true)`,而原版 `MouseHandler#onPress` 把该 hook 放在 `if (screen == null) … else screen.mouseClicked(…)` **之前且不看 screen 是否为空**(实测 1.21.1 `MouseHandler.java:87` → `ClientHooks.java:667-669`;1.20.1 `MouseHandler.java:81` → `ForgeHooksClient.java:721-723`)⇒ 取消即 `return`,界面收不到点击;平时不暴露是因为 `onScreenOpening` 开界面时会取消会话,但**「手持即选择」(`holdToSelect`,效果牌走这条)被刻意豁免**、`ChatScreen` 也豁免 ⇒ 会话存活时暂停菜单点不动。修复 = `onMouseButton` **首行**加 `if (Minecraft.getInstance().screen != null) return;`(两线同位置各 +1 行;界面打开时鼠标/滚轮交回界面、不再执行会话动作、也不再弹 `effect_card_burst_full`;关界面后会话照旧继续)。同类隐患已逐条排查为「无需修」:`onMouseScroll` 只在 `screen == null` 分支派发、`startUseItem` 注入点被 `overlay == null && screen == null` 包住、渲染/KeyBinding 类不碰输入、全仓只有该文件订阅输入事件。26.1.2 的同构代码在该线 `client/TargetSelectionClient.java`(约 `:680`/`:719` 的 `event.setCanceled(true)`)。
15. **绿洲女王 nardis 语义变更(2026-09-27 用户裁决,晚于提交 `8e59e923` 的实现;迁移时**必须按新语义**,勿照旧实现)**:⚠️ **本条第 ①–⑦ 与紧随其后的「实现落点」「F1 修复」整段描述的是当日更早的「冻结(使用中)」方案 —— 该方案已于同日晚些时候被用户推翻;一切以本条末尾的【最终语义(2026-09-27 终版)】为准。** 原文保留仅为追溯:① **给牌构成**由「3 张随机(`CardCategory.ALL`)」改为 **2 张战斗牌 + 1 张效果牌**(战斗池 `CardCategory.BATTLE` 攻击+防御**混合随机 2 张**、效果池 `CardCategory.EFFECT` 随机 1 张;专属牌继续排除;仍走 `VitaminPillChipItem.giveCard` 漏斗;放不下就少发、绝不落地);② **新增冻结**:恢复立牌**统一锁定态** —— 临时牌被全部用光或 3:00 到期之前主动技能处于「使用中」**不可再释放**,反馈照搬其它立牌(提示「使用中」+ 剩余时间);③ **解冻先到者**:牌被全部用光 ⇒ 立刻解冻**并同时移除 `nardis_privilege` 效果**(HUD 计时器消失;防抖要求见下),3:00 到期 ⇒ 清空剩余临时牌后解冻;④ **冷却与冻结并行**:释放当刻即开始玩家级冷却(默认 180 秒,沿用既有减免机制),解冻时**不再追加**新冷却 ⇒ 解冻后冷却已过即可立即再释放;⑤「先清空旧临时牌再发新的」降级为**安全网**(正常玩法被冻结挡住,仅在效果被外力移除等异常情形生效),**不得删除**;⑥ **防抖硬约束**:`TemporaryCardUtil.tick` 的「无效果⇒清临时牌」与新增的「无临时牌⇒移除效果并解冻」互为逆操作,必须保证不会每 tick 反复增删效果、也不会在释放当刻发牌未完时被误判为 0 张而自我解冻。**实现落点(2026-09-27 两线同批落地)**:`TemporaryCardUtil` 增 `GRANT_BATTLE_COUNT=2`/`GRANT_EFFECT_COUNT=1` 与 `grantNardisPrivilege(Player)`(战斗牌**先发**、两类各自重算空槽)+通用漏斗 `grantRandom(Player, CardCategory, int)`(**2 参重载 `grantRandom(Player,int)`=`ALL` 必须保留** —— 探针 `nardigive` 直接调它,删了会打断测试资产);解冻(a) 写在 `TemporaryCardUtil.tick` 尾段(`fx==null && !hasCards ⇒ return` / `hasCards ⇒ return` / `fx.getDuration() >= DURATION_TICKS ⇒ return` 三重防抖后才 `ModEffectRemoval.remove` —— **直接 `player.removeEffect(...)` 不可用**,`astral_dice:*` 会被拦截);安全门写在 `NardisSignItem#handleUse` 客户端早退之后、`purgeAll` 之前(`countFreeSlots(player) < TemporaryCardUtil.MIN_FREE_SLOTS_TO_CAST` ⇒ `sendSignActionBar(player, "msg.astral_dice.nardis_inventory_full")` + `InteractionResultHolder.fail` ⇒ 不冷却/不施效果/不冻结/不发牌/不清旧牌;**阈值 `MIN_FREE_SLOTS_TO_CAST = 2`** —— 2026-09-27 用户裁决由 3 放宽为 2:卡牌物品 `stacksTo(64)` 可堆叠、发牌前先清空临时牌 ⇒ 恰好 2 格时两张战斗牌常并进一格,效果牌仍可能放得下;少于 2 格必然只能发 ≤1 张;**发放顺序保持战斗牌优先、各张独立重算空槽**(放不满就少发、绝不落地);冻结三件套 = `NardisSignItem` 覆写 `startActiveLockOnUse`(读回 `instance.getDuration()` 写 `beginActiveLock(player, SIGN_ID, now + dur)`)/`isGateEffectActive`/`cooldownRunsDuringLock=true`。**`BaseSignItem` 唯一结构性改动** = 新增 `protected boolean cooldownRunsDuringLock(Player)`(**默认 `false` = 既有串行语义,其余立牌逐字不变**)并把 `endLockAndStartCooldown` 拆出 2 参私有重载(`cooldownAlreadyRunning` 为真时解锁**只从剩余冷却里抵扣一次减免池、不追加新冷却`,为假时原实现逐字保留);解锁时经 `lockSignItem(signId)` 反查注册表(与 `isSignActiveLocked` 同一路径,解析失败按 `false` 兜底=旧行为)。**新 lang 键**(两线 `zh_cn`/`en_us` 各 +1,键数 736 → **737**):`msg.astral_dice.nardis_inventory_full` = 「物品栏空间不足（需要 2 个空格），无法获得临时牌」/「Not enough inventory space (2 free slots needed) - cannot gain temporary cards」;提示走既有立牌反馈通道 `sendSignActionBar`(黄字 ActionBarPayload),**不是**裸 `displayClientMessage`。**已登记的三条推论(不修)**:① 卡牌物品其实 `stacksTo(64)` 可堆叠,故门槛最终**放宽为 2**(用户 2026-09-27 裁决,原为 3):恰好 2 格时两张战斗牌同 id(约 `1/9`)会并进一格、余下 1 格收下效果牌 ⇒ 3 张全放得下;随机到**不同**牌时就只有 `grantRandom` 的「少发、绝不落地」截断 ⇒ **恰好 2 格时常见结果是只发 2 张(无效果牌)**,这是已接受的代价;② 解锁时若冷却已过完,冻结期累计的 `sign_active_reduction_pool` **作废**(纯减法口径的必然结果,结转需新附件);③ `cooldown_end` 是绝对 gameTime 而效果时长离线冻结 ⇒ 长时间离线后重登可能出现「冷却已过、冻结尚存」(并行语义的必然推论)。**文案同步(同批,仅 lang 4 文件)**:`effect.astral_dice.nardis_privilege.description`、`tooltip.astral_dice.sign.nardis_active`(第 1 行与第 4 行)、`astral_dice.guide.entry.nardis_sign.1/.3` 里「3 张随机(全部战斗与效果牌)」与「效果生效中再次释放先清空旧牌再发 3 张」的旧描述必须改成「2 战斗 + 1 效果」与「用光或结束后才能再次释放 + 使用中不可释放 + 空位不足(<3)则阻止释放」;patchouli 的 `entries/signs/nardis_sign.json` 只引用 lang 键、**无字面文案**,不用改。**F1 修复(2026-09-27,由 R2 独立只读验证发现)**:锁定硬上界 `sign_active_lock_end` 与 `sign_active_cooldown_end` 都是**绝对 gameTime**,而门控效果 `nardis_privilege` 的时长**只在服务器运行且玩家在线时流逝**(实体不 tick ⇒ 效果时长离线冻结;`EffectTimerGuard` 的绝对记录又在 `PlayerLoggedInEvent` 被 `clear`)⇒ **多人服务器**上「离线时长 > 剩余冻结时长」后重登时,上界与冷却双双已过、而效果与临时牌都还在 ⇒ 冻结提前结束、冷却被归零(`remaining = max(0, cdEnd-now) = 0`)、且该路径**不清剩余临时牌** = 一次免费刷新(单人存档因「停客户端=停服务器」不暴露)。修复 = `BaseSignItem` 新增**默认 `-1`** 的钩子 `protected int gateEffectRemainingTicks(Player)`,并在 `tickSignActiveLock` 的非忍者分支、`isSignActiveLocked` **之前**插入 `realignLockEndToGateEffect(...)`(`lockEnd<=0 || now<lockEnd` 早退 → 解析立牌 → `remaining<=0` 早退(**默认立牌在此短路,写操作永不执行** → 其它立牌含忍者逐字不变,仅解冻那一拍多一次查表)→ `!isGateEffectActive` 早退 → `setSignActiveLockEnd(now + remaining)`);nardis 覆写返回 `nardis_privilege` 的 `getDuration()`。对齐**单调向后、同 tick 幂等**(写完即 `now<lockEnd`,forge START/END 两遍调用第二遍早退;附件不 `.sync()` 故无网络包)。**两条语义后果(契约直接推论)**:① nardis 从此**「外部刷新效果不延长锁定」不再成立**(刷新后下一次对齐会把冻结延到与效果一致),其余立牌仍保留该性质;② 在线自然到期那一拍守卫会按 1 tick 重施加效果 ⇒ 解冻**推迟 1 tick(50ms)**,不抖动。⚠️ **26.1.2 有同一缺陷**(该线 `BaseSignItem.java:257` 同样的 `lockEnd` 早退)⇒ 迁移时必须一并补。**已登记不修(R2 报告 F2–F8,已逐条判定)**:F2 = 1.20.1 只靠 5 个 mixin,对**第三方**模组里覆写 `mayPlace` 且不调 super 的槽无效(平台固有限制、非本批引入,该线 mixin 注释已自认);F3 = `countFreeSlots` 只数主物品栏,而 `Inventory#add` 还能并入副手 ⇒ 门槛只会更保守(少发,不会落地);F4 = 鼠标光标上的临时牌不计入 9 张上限(**2026-09-24 起 `purgeAll` 已覆盖光标栈与「打开的卡牌栏容器」⇒ 死亡/到期不会再漏掉这两处的牌**);F5 = 冻结使 `isSignActiveLocked()` 为真,**翻转**若干冷却减免消费者的分支(`FateGuidanceCardItem`/`JasmineSignItem`/`LuluSignItem` 由「立即扣减运行中冷却」改为「入池」),默认常量下(nardis 冷却基准 ≤ 冻结时长,两者同 3600t)解锁时 `remaining ≡ 0`、池抵扣为空操作 ⇒ **代数等价、无可观察回归**;F7 = 第三方否决效果时可能「吃冷却 + 3 张牌被下一拍清掉」,原版玩家 `canBeAffected` 恒真故不可达;F8 = 防抖那一拍不可达(安全门保证发满)。**F6 裁决(用户 2026-09-27)**:**冻结期「电流核心」整段失效符合本意、不改** —— 它是既有锁定态框架对**所有**立牌的既有行为(`performSkill` 第 0 步锁定拒绝在冷却分支之前;`CurrentCoreChipItem` 另有第二道同判据纵深防御),与立牌种类无关,不为 nardis 开特例。另:jasmine/fen/misaki/nancylu/papara/parunan/fanny 等「效果时长型硬上界」立牌在多人服务器上有**同类**墙钟漂移(冻结提前结束),但影响面不同(无清牌问题、冷却本就串行),按最小改动**未扩展**;新钩子已就位,如需可逐个覆写。**【最终语义(2026-09-27 终版,以本段为准;上文 ①–⑦ 与 F1 段均已作废)】**:① **不进入冻结** —— 主动技能**删掉全部锁定态覆写**(`startActiveLockOnUse`/`isGateEffectActive`/`cooldownRunsDuringLock`/`gateEffectRemainingTicks` 四个一个不留)⇒ 回到普通立牌路径,**释放当刻即开始玩家级冷却(默认 180 秒,可减免)**,效果生效期间可再次释放,只受冷却限制;② **再次释放不清空旧临时牌**(释放路径里的 `purgeAll` 删除;`purgeAll` 仅剩两处合法调用:死亡清理 `PlayerLifecycleHandler` + `TemporaryCardUtil.tick` 的「无效果⇒清牌」安全网),每次再发一整套 **2 战斗 + 1 效果**并**叠加**;③ **每次释放把 `nardis_privilege` 有效期重置为 3600 tick**;④ **牌被用光不再提前结束效果**(`tick` 的「无牌⇒移除效果」分支与其 `getDuration() >= DURATION_TICKS` 防抖一并删除);⑤ **临时牌上限 9 张**:`MAX_TEMPORARY_CARDS = 9`,`current = countTemporaryTotal(player)`(= 物品栏 0..35 含主手 + 副手,按数量 + 骰子 `weapon_enhancement.appliedStones` 中 `temporary()` 条数),`current >= 9` ⇒ 拒绝(新键 `msg.astral_dice.nardis_card_limit`,**零消耗**),否则发放**夹取到上限**(`budget = 9 − current`,战斗牌 `min(2,budget)`、效果牌 `min(min(1), budget−已发战斗牌)`)⇒ **总数永不超 9**;判据顺序 =**冷却 → 上限 → 物品栏空位(`MIN_FREE_SLOTS_TO_CAST = 2`)**;⑥ **产品侧为冻结加的整台机械已全部撤回**:`BaseSignItem` 用 `git checkout c8a5e220~1 -- <两线路径>` 还原,`git diff c8a5e220~1 -- <BaseSignItem>` **必须为空**(即与冻结机制引入前逐字一致,其它立牌零回归面)。**效果时长重置的确定性手段(关键,勿退回旧写法)**:效果已存在 ⇒ `player.forceAddEffect(全新 3600 实例, null)`(**无条件替换实例**,不经 vanilla `update` 合并 ⇒ 剩余必然 3600;与客户端处理 `ClientboundUpdateMobEffectPacket` 同一 API ⇒ HUD 同步)+ **显式 `EffectTimerGuard.record(player, refreshed)`**(因 `forceAddEffect` **不发** `MobEffectEvent.Added`,而记账钩子挂在该事件上);效果不存在 ⇒ 常规 `EffectTimerGuard.apply`。**为什么必须显式 record**:守卫记**绝对结束刻**并取 `max(prev.endTick, now+duration)`,其 `tick` 会把「实际剩余 > 预期 + `CLAMP_TOLERANCE`(=20)」**裁回**预期 ⇒ 只 `forceAddEffect` 不 `record` 会被下一拍裁回旧值(实测 3531,即旧版 `apply` 的失败形态)。全程**不移除**效果 ⇒ `tick` 的「无效果⇒清牌」无窗口;重登时 `EffectTimerGuard.clear` 已清账,效果带存档剩余时长回来。**已登记口径边界(本批不改)**:`current` **不含鼠标光标** `containerMenu.getCarried()`(把临时牌提在鼠标上时不计入 9 张、也不被 `purgeAll` 清理;与 R2 的 F4 同一处),亦不含末影箱/护甲槽/地面掉落物;`current` **含已装配到骰子的**。**26.1.2 迁移提示**:该线仍是旧行为(且无 9 张上限),迁移时按本段**最终语义**落线,并一并补迁移清单第 14 条(ESC 输入放行)。
16. **立牌锁定态「离线时钟漂移」修复(2026-09-27,两线;26.1.2 **未改动**,迁移时必须一并带走)**:① 新玩家附件 `sign_active_lock_last_seen`(long,**服务端专用、不 `.sync()`**;1.21.1 走 `AttachmentType.serialize(Codec.LONG)`、1.20.1 走 `AttachedDataKey`(**不得** `.inMemory()`,且**不进** `SYNCED_KEYS`))+ get/set 包装;② `BaseSignItem.beginActiveLock` 末尾落笔 `lastSeen = gameTime`、`endLockAndStartCooldown` 末尾清零;③ `tickSignActiveLock` **步骤 0**(客户端早退之后、`signId` 读取与任何 `return` 之前):`gap = now − lastSeen`,`gap > 1` 时把 `sign_active_lock_end` 与 `sign_active_lock_grace_end`(各自 `> 0` 时)各 `+= gap`;随后仅在取值变化时刷新 `lastSeen`(无锁定/无宽限 ⇒ 0)。**四条硬约束(改错就会出事故)**:ⓐ **`gap > 1` 才补偿** —— 写成 `gap > 0` 会每拍自我延长、锁定永不解;ⓑ **补偿必须早于 `isSignActiveLocked` 判定与任何早退**;ⓒ **绝不补偿 `sign_active_cooldown_end`**(冷却照旧离线也流逝,这是既有预期);ⓓ **必须确认该附件随玩家存档持久化**(1.21.1 `Entity.saveWithoutId` 的 `neoforge:attachments`;1.20.1 `AstralData.persistentStore`→`ForgeCaps`;与 `sign_active_lock_end` 同链,不持久化则整个方案失效)。⇒ 一次覆盖 `fen`/`jasmine`/`misaki`/`papara`/`parunan`/`fanny`/`nancy_lu` 的硬上界 **与忍者宽限**(忍者从此是「**在线** 1:00」而非「墙钟 1:00」)。④ **`nancy_lu` 同源残留 3 处**(必须一起迁移):`onCurioTick` 到期 sweep 取 `max(nancy_lu_hidden_until, sign_active_lock_end)`(**决定性** —— 不修则重登后 1 tick 内清掉隐身实例,而隐身正是该三态锁定的门控效果,补偿好的锁定会被一起打死)、`onAttackWhileHidden` 与 `isHidden` 改读 `isSignActiveLocked`、`isHiddenClient` 去掉绝对刻比较(改「键非零 + `hasEffect(INVISIBILITY)`」)。⑤ 调试 dump 加只读行 `sign_active_lock_last_seen`(`AstralPartyCommand` 的 `DUMP_GROUP_SIGN`,仅用于实测取证持久化)。⑥ **已登记不修**:`nancy_lu_active_bonus_until`(2400t;离线跨越后会在隐身仍在时提前清掉主动加成与 `NANCY_LU_HACK`,方向为玩家不利、无利用面;其截止刻与锁定上界**不同长** ⇒ 「取 max / 读已补偿锁定态」两法皆无效,通用修法需耦合或加新键,**不值当**)、`nancy_lu_ender_pearl_immune_until`(20t 战斗内短窗口)。⑦ **测试侧待办**:`LOCK-OFFLINE-{1.21.1,1.20.1}`(L1–L7)+ `resetActiveLock` 补第 6 个键 + 所有直接写 `sign_active_lock_end` 的脚手架补写 `lastSeen = now`;历史用例 `NANCY-LU-CLOAK` 的 `doNancyExpire` 若复活需把硬上界对齐到同一过去刻。⚠️ **本修复的游戏内验证按用户指示延后**(代码已两线落地并构建通过,实机读数待补)。
17. **蛟龙立牌(mamushi)资产与注册 + 三组「配方网格重合」修复(2026-09-27,两线;26.1.2 **未改动任何文件**,迁移时必须一并带走)**:① **物品** `mamushi_sign`(蛟龙立牌,传奇 = 物品层 `ASTRAL_DICE_LEGENDARY`;本仓「传奇」用的就是 `UNCOMMON`)、类 `item/sign/MamushiSignItem`(**只有 javadoc + 构造器**:未覆写 `handleUse`、无被动钩子 ⇒ 主动走 `BaseSignItem` 默认实现「WARN + `InteractionResultHolder.fail`」,**技能(主动/被动)与专属卡牌「撕咬」/「龙之咆哮」由同日第二批落地 —— 见下条 18**(本条只覆盖资产与注册,是第一前置;当时本类确实只有 javadoc + 构造器);注册面 = `item/ModItems`(NARDIS_SIGN 之后)、`init/ModCreativeTabs`(传奇段)、`datagen/ModItemModelProvider`、`datagen/ModRecipeProvider`、`data/astral_dice/tags/item/signs.json`(该线为 `tags/items/`)、`data/curios/tags/item/stand.json`(同上)、手册 `entries/signs/mamushi_sign.json`(**只有 `patchouli:crafting` 一页,无 spotlight/text 占位页**;`sortnum`=20)、lang 只加物品名 `item.astral_dice.mamushi_sign`(「蛟龙立牌」/「Mamushi Sign」,两线各 **739/739**)、贴图 `textures/item/mamushi_sign.png`(自 `images/蛟龙立牌.png` 逐字节复制,两线同 sha256 `35711E2F…`,32×32)。② **配方** = 「钻石骰子 + 黄金星盘」传奇档,但**不得与 `fen_sign` 同表** ⇒ `SPS/SES/GDG`(S=**海晶灯**〔2026-09-23 由海晶碎片改〕、P=金块、E=空白立牌、G=黄金星盘×2、D=钻石骰子);空白立牌置中、骰子固定中下。③ **同批修复三组配方网格重合**(2026-09-27 用户裁决「三组一起拆开」):`teru_sign` 改 `GCG/LEL/ZPZ`(**荧石**×2〔2026-09-23 由荧石粉改〕,原与 zhao 同为 `GCG/RER/ZPZ`)、`nardis_sign` 改 `CYC/TET/TDT`(仙人掌×2,原与 padman 同为 `WYW/TET/TDT`)、`mamushi_sign` 用 `SPS/SES/GDG`(原照抄 fen ⇒ 同格);`zhao_sign` / `padman_sign` / `fen_sign` **保持原样**。判据与后果见上文「新增立牌时的要求」第 6 条末段。④ **新增静态守门** `scripts/verify/verify_crafting_recipe_uniqueness.ps1`(已进 `TESTING-SPEC.md` §9;shaped 按「裁剪四周全空行/列后的 3×3 单元格矩阵」判重、shapeless 按材料多重集判重、并做 shaped↔shapeless 交叉判定;**必须在 `runData` 之后跑**);两线 `runData` 后期望读数 `shaped=91 shapeless=27 collisions=0`(2026-09-21 怪力侦探批 87 → 88、人偶师批 88 → 89、飞星批 89 → **91**;26.1.2 当时为 `shaped=83`,该线尚无 zhao/teru/nardis/mamushi/sherry/hanna;⚠️ **2026-09-23 复核:三线均已 `shaped=91 shapeless=27 collisions=0`** —— 26.1.2 已补齐这些立牌,上句的 83 已过时)。⑤ **技能与两张专属战斗牌已于同日落地**(本条只覆盖资产与注册,是其前置;完整语义/落点见下条 18)。⑥ **同批补漏(静态守门发现)**:上一批 nardis 漏登记的**赏金板奖励池**条目已补 —— `nardis_sign`(稀有 RARE)写入 `data/bountiful/bounty_pools/bountiful/astral_rews.json`(`rarity=RARE`、`unitWorth=1400`,与同为黄金骰子档的 `lulu_sign` 一致;条目追加在 `ren_sign` 之后 = `ModItems` 注册序;两线文件**逐字节相同**、MD5 `EA5F23A2…` 改后仍相同);**蛟龙 mamushi(传奇 UNCOMMON)按规则不进任何池**(`teru`/`zhao` 同为传奇亦不入池)。修复前 `verify_bountiful_pools.ps1` 两线各报 `缺失条目(1): ['nardis_sign']`,修复后 `RESULT: ALL OK`。
18. **蛟龙立牌技能 · 觉醒/真龙形态 · 连锁反应 · 撕咬/龙之咆哮(2026-09-27 同日第二批;26.1.2 侧**未改动任何文件**,迁移时必须一并带走)**:① **4 项用户裁决** —— 觉醒按**受益人数**计(同一发牌事件内按受益人去重、单次封顶 3 层;自己给自己发牌不计)/ 多张撕咬**每张各 +1 层**(加成恒 `min(觉醒,4)` 且**实时**)/ 真龙形态的「取消范围限制」= **同维度全体**(仍 32 人上限 + 距离升序)/ 撕咬加成**实时**非快照。② **2 个新效果** `mamushi_dragon`(真龙形态,常驻 `Integer.MAX_VALUE` + tick `refresh`,图标逐字节复制立牌贴图)、`dragon_roar_break`(破防,`ARMOR` 属性修饰器 **-8** = 减 4 点防御,1:00,图标逐字节复制「龙之咆哮」卡牌贴图)。③ **3 个新附件键** `mamushi_awakening`(int,**sync** + `copyOnDeath`)/`mamushi_bite_bonus_active`(bool)/`mamushi_forced_cooldown_until`(long) + `DeathPreservedBonuses` 第 3 槽位 + 1.20.1 `AstralData#onPlayerClone` 白名单(1.20.1 synced 29 → 30)。④ **发牌漏斗加 giver(唯一挂点)**:`VitaminPillChipItem.giveCard(Player giver, Player receiver, ItemStack card)` 重载,成功入包分支调 `MamushiSignItem.onCardGivenToOther`;`RandomCardHandler.giveCards`/`giveCardTo` 与 `YouHaveIHaveCardItem` 改传 giver;`GiveoutScope.maxTargets >= 0` 由 `Collections.shuffle` 随机抽样改为**距离升序取最近 N**,新增 `aroundWithTeam(range,maxTargets)` / `wholeDimensionWithTeam(maxTargets)` 工厂(**保留 API,当前零调用**)。⚠️ **主动技能的目标集合在 R1 复核后被改为「自管」——迁移时必须按修正后的语义实现,勿照 `GiveoutScope` 旧写法**:`MamushiSignItem.collectActiveTargets(Player wearer, boolean dragon)` = 严格**同维度** → 排除自己 → **有队伍只取同队**(`EventTargetCollector.hasAnyTeam` + `collectTeamPlayers`,库侧可能含其它维度队友 ⇒ 仍按维度过滤)/ **未组队不做队伍过滤**(自己取 `ServerLevel.players()`,**不得**使用库的「无队伍 ⇒ 全服在线」回退)→ 距离 ≤ 12(**真龙形态不限**)→ 距离升序(UUID 平局)→ **排序后**截断 32。原写法 `aroundWithTeam(12)` 的语义是「12 格内全部玩家 ∪ 队友不限距离」,会让**非队友**与**超距队友**也拿到牌,与用户原文「12 格内所有**友方队友**」不符。**同批另修**:无主(缺 `owner_uuid`)撕咬 → 龙之咆哮的归属,两线统一为**绑定转换者**(有主时原样保留)。⑤ **强制冷却**:`BaseSignItem` 新增 `forcedActiveCooldownTicks()`(绕过 `WeirdDiceHandler`,直接把 `max_cooldown` 与 `cooldown_end` 写成 1200)与 `forcedCooldownUntil()`(**硬闸门,早于电流核心判定**);`CurrentCoreChipItem` 对 mamushi 返回 `FINISH_NOT_ENOUGH` 并提示 `msg.astral_dice.mamushi_cooldown_locked`(该分支属纵深防御)。⑥ **两张专属战斗牌**:`attack_card_bite`(费用 2/耐久 1/定值 +3)、`attack_card_dragon_roar`(费用 3/耐久 5/定值 +3 + 命中施加缓慢 III 与破防),`CardRegistry` 必须**同时**补 `defaultUses` 与 `minRoll`/`maxRoll` 分支(漏 `defaultUses` 会静默回退成 10 次);⚠️ **专属守门是既有缺口** —— `CardItem` 原本**没有任何** owner 检查,本批在 `CardInventoryMenu` 攻击/防御两个 `mayPlace` 加 `isExclusive && !canUse ⇒ false`,并把两牌加入 `ExclusiveCardUtil.isExclusive` 硬编码列表、`is_exclusive.json` 标签与 `registerExclusiveCard`(**不进 `attackCards()` ⇒ 天然不在随机池**、无配方、不入赏金池)。⑦ **文案**:两份 CHANGELOG 各 +3 条(立牌 1 + 两张牌 2),中英 **372:372**;lang 两线 **760/760** 且跨线键集 diff = 0、本批键取值逐字一致;三份手册条目两线 SHA256 相同。⑧ **本轮验证**:两线 `runData` 生成两张卡模型(`written: 2`);两线 `mt_build` OK;`tooltip_color_audit` 仅剩既有 12 处 teru `50%`;`verify_crafting_recipe_uniqueness` 仍 0 重合;`check_lang_sync` 三线 OK。⑨ **未做(如实登记)**:游戏内行为验证(用户已延后)、探针与用例、26.1.2 迁移。
19. **怪力侦探立牌(sherry)资产 · 技能 · 投掷飞行(2026-09-21;两线已落地,26.1.2 侧**未改动任何文件**,迁移时必须一并带走)**:① **物品** `sherry_sign`(怪力侦探立牌,史诗 = 物品层 `ASTRAL_DICE_EPIC`;英文名 **`Sherry Sign`** 按角色名而非称号直译 —— 用户裁决)、类 `item/sign/SherrySignItem extends BaseSignItem`(覆写 `handleUse` / `activeCooldownBaseTicks` / `onCurioTick` / `clearSignData`,并在本类上挂一个带 `@EventBusSubscriber` 的 `@SubscribeEvent` 订阅器 —— 「侦探出击」计层);注册面 = `item/ModItems`(`MAMUSHI_SIGN` 之后)+ `init/ModCreativeTabs` + `datagen/ModItemModelProvider` + `datagen/ModRecipeProvider` + `data/astral_dice/tags/{item|items}/signs.json` + `data/curios/tags/{item|items}/stand.json` + 手册 `entries/signs/sherry_sign.json`(**spotlight + text×2 + crafting 四页**,`sortnum`=21)+ lang 11 键 + 贴图两张。② **配方** = 「钻石骰子 + 黄金星盘」史诗档 ⇒ `PBP/AEA/IDI`(P=黄金星盘×2、B=书、E=空白立牌、A=铁砧、I=铁块、D=钻石骰子);**⚠️ 2026-09-25 用户裁决:D 由钻石骰子改紫晶骰子(网格不变 ⇒ 与 moses/nancy_lu 的同档关系作废)**;空白立牌置中、骰子固定中下;六个填料的格位与同档 `moses`(`GPG/TCT/PZP`)/`nancy_lu`(`YFY/BLW/PZP`)及全部其它立牌**均不复用**,须保持 `verify_crafting_recipe_uniqueness.ps1` **0 重合**。③ **效果** `sherry_reasoning`(推理时间;层数真值在附件、效果只作 HUD 镜像(层数 = `amplifier + 1`)、BENEFICIAL、`Integer.MAX_VALUE`、图标 `textures/mob_effect/sherry_reasoning.png` **从立牌贴图逐字节复制**;注册后两发布线效果数 **39 → 40**)。④ **附件键 1 个** `sherry_reasoning_layers`(int,**不 `.sync()`**;1.21.1 `.copyOnDeath()`、1.20.1 进 `AstralData#onPlayerClone` 白名单 **+** `DeathPreservedBonuses` **第 4 槽位**(立牌掉落触发的 `onUnequip → clearSignData` 会抢先清零,两层缺一不可)—— 客户端 tooltip 的层数计数器读的是**已同步的效果实例**,不读本键 ⇒ 故本键不进 `SYNCED_KEYS`);玩家级附件计数 1.21.1 **81 → 82**、1.20.1 **83 → 84**,1.20.1 synced 键**不变**(30)。⑤ **三处挂点**:`combat/DiceCombatModifiers` 追加攻击修饰器(实时读层数,每层 +1)、`event/ChipDamageHandler` 在 LOWEST 阶段追加**纯减伤**(层数 + 挚友守护,**刻意不带磨刀石/安全气囊的「不可致死保护」**)、`combat/DiceCombatEvents` 赐福结束段调 `SherrySignItem#onDiceBlessingEnded`。⑥ **`BaseSignItem` 结构性改动**:新增 `protected int activeCooldownBaseTicks(Player)`(**缺省 = `WeirdDiceHandler.signCooldownTicks(player)`,其余立牌逐字不变**),并把冷却段的取值改为 `forced > 0 ? forced : activeCooldownBaseTicks(player)`;与 `forcedActiveCooldownTicks()` 的分工 = 「自定义基础值、仍走减免链」↔ 「强制值、不受任何减免」。⑦ **投掷执行器** `combat/SherryThrowManager`(新增):逐 tick 水平线性插值 + `ARC_HEIGHT × 4t(1−t)` 抛物线;**飞行期 `setNoAi(true)`、每 tick 归零 `fallDistance`**(避免原版坠落伤害)、落点那一 tick 恢复原 AI 并结算 `2 + bonus` 点伤害 + `MarkManager.apply`;作业表按 `ResourceKey<Level>` 取 `ServerLevel`、**服务端内存态不持久化**;平台差异 = 1.21.1 `ServerTickEvent.Post` ↔ 1.20.1 `TickEvent.ServerTickEvent` + `phase == END` 早退(照 `event/LivingPageFlightScheduler`)。⑧ **「挚友守护」按注册 id 字符串匹配 `astral_dice:hanna_sign`**(人偶师立牌尚未落地 ⇒ 不引用 `ModItems.HANNA_SIGN` 以免编译期依赖;hanna 落地后被动**自动生效**,迁移时保持该写法)。⑨ **赏金板**:sherry(史诗 EPIC ⇒ 非传奇)**必须进** `astral_rews.json`(`rarity=EPIC`、`unitWorth=3400`,与同档 `moses`/`nancy_lu` 一致;条目追加在 `nardis_sign` 之后 = `ModItems` 注册序;两线文件**逐字节相同**、sha256 `C3F19D72…`)。⑩ **文案**:lang 两线 **771/771**、跨线键集逐字一致;手册 `entries/signs/sherry_sign.json` 两线 sha256 相同;两份 CHANGELOG 各 +1 条(中英条目数与顺序一致)。⑪ **本轮验证**:两线 `runData` 各 `written: 3`(模型 1 + 配方 1 + 进度 1);两线 `mt_build` OK;`check_lang_sync` 两线 exit=0;`tooltip_color_audit` PASS(0);`verify_crafting_recipe_uniqueness` / `verify_bountiful_pools` 全绿。⑫ **未做(如实登记)**:游戏内行为验证(用户已延后)、探针与用例、26.1.2 迁移。
20. **人偶师立牌(hanna)资产 · 技能 · 路过判定(2026-09-21;两线已落地,26.1.2 侧**未改动任何文件**,迁移时必须一并带走)**:① **物品** `hanna_sign`(人偶师立牌,稀有 = `ASTRAL_DICE_RARE`;英文名 **`Hanna Sign`** 按角色名定 —— 与 `Sherry Sign`/`Mamushi Sign` 同风格,**同时把 sherry 侧两处英文引用由 `Puppeteer Sign` 改为 `Hanna Sign`**:`tooltip.astral_dice.sign.sherry_passive` 与 `astral_dice.guide.entry.sherry_sign.2`)、类 `item/sign/HannaSignItem extends BaseSignItem`(覆写 `handleUse`/`onCurioTick`/`clearSignData` + 两个 `@SubscribeEvent`:掉落免疫与近战闪避走 `LivingIncomingDamageEvent`[neo]/`LivingAttackEvent`[forge],末影珍珠拦截走 `PlayerInteractEvent.RightClickItem`;骰点钩子由 `DiceCombatEvents` 显式调用);注册面 = `ModItems`(`SHERRY_SIGN` 之后)+ `ModCreativeTabs` + 两个 datagen provider + `signs.json` + `curios stand.json` + 手册 `entries/signs/hanna_sign.json`(**spotlight + text×2 + crafting**,`sortnum`=22)+ lang 19 键 + 贴图 4 张(`textures/item/hanna_sign.png` ← `images/人偶师立牌.png`;`textures/mob_effect/hanna_doll_craft.png` ← `images/人偶制作.png`;`hanna_doll_complete.png` ← `images/人偶完成.png`;**`hanna_float.png` 复用立牌图**)。**本批同时解答了「待办交接」§3 第 5 条的待裁决项**:`人偶完成.png`/`人偶制作.png` = 「人偶制作」(层数镜像)与「人偶完成」(完成态)两个**效果的图标**。② **配方** = 「黄金骰子(无星盘)」稀有档 ⇒ `SQS/LEL/TDT`(S=线、Q=白色羊毛、L=皮革、E=空白立牌、T=木棍、D=黄金骰子);**⚠️ 2026-09-25 用户裁决:两个线位改黄金星盘、D 由黄金骰子改紫晶骰子 ⇒ `GQG/LEL/TDT`(原「黄金骰子(无星盘)」档作废)**;空白立牌置中、骰子固定中下;与同档 `lulu`(`SZS/ZCZ/SDS`)/`padman`(`WYW/TET/TDT`)/`nardis`(`CYC/TET/TDT`)及全部其它立牌**格位与材料多重集均不同**,两线 `verify_crafting_recipe_uniqueness` 期望 `shaped=91 collisions=0`(26.1.2 为 83)。③ **3 个新效果**:`hanna_doll_craft`(人偶制作;层数镜像,真值在附件、**0..6 永不为 7**,达 7 时由立牌归零并转挂完成态)、`hanna_doll_complete`(人偶完成;无限时长常驻状态,卸下立牌时移除)、`hanna_float`(魔女漂浮;**有限时长 1200 tick**,`MOVEMENT_SPEED` +20% —— ⚠️ **平台差异**:neo `ADD_MULTIPLIED_TOTAL` + ResourceLocation id ↔ forge `MULTIPLY_TOTAL` + **UUIDv5 字符串** id `aef19f28-557c-564f-bb6c-e72d5f6926d9`(=`uuid5(NAMESPACE_URL,"astral_dice:hanna_float_speed")`,生成口径同 `jasmine_sweep_speed`);注册后两发布线效果数 **40 → 43**)。④ **4 个新附件键**:`hanna_doll_craft_layers`(int)/`hanna_doll_complete`(boolean)/`hanna_fantasy_cooldown_end`(long)/`hanna_blessing_cooldown_end`(long) —— 均 **不 `.sync()`**;**均不跨死亡保留**(技能原文未声明 ⇒ 不占 `DeathPreservedBonuses` 槽位、不进 1.20.1 白名单);玩家级附件计数 1.21.1 **82 → 86**、1.20.1 **84 → 88**。⑤ **两处挂点**:`PlayerTickEvents` 的 `onPlayerTick(Post)` 在 **`% 20` 早退之前**调 `HannaSignItem.tickPassing`(路过判定,两条被动各有独立 1:00 冷却,冷却内只读两个 long 即早退)、`DiceCombatEvents` 骰点定稿处(zhao 之后)调 `HannaSignItem.onDiceRollResult`。⑥ **闪避的平台差异**:neo 走 `applyDodgeCancel(LivingIncomingDamageEvent)`,forge 走 `applyDodgeCancel(LivingAttackEvent)` 且**必须**补 `DiceCombatEvents#isImmuneToDamage` 一步复刻 1.21.1 的事件阶段(照 `onMosesBrokenDodge`);两线都先判 `isInCounterChain()` 防递归。⑦ **赏金板**:hanna(稀有 RARE ⇒ 非传奇)**必须进** `astral_rews.json`(`rarity=RARE`、`unitWorth=1400`,与同为黄金骰子档的 `lulu`/`nardis` 一致;条目追加在 `sherry_sign` 之后;两线逐字节相同)。⑧ **文案**:lang 两线 **790/790**;`+20%`/`-100%` 必须写成 **`+20%%`/`-100%%`**(ⓘ 实测坑:批量把 `%` 转 `%%` 时**会误伤已转义的 `+20%%`** ⇒ 变成 `+20%%%`,替换必须避免匹配已转义形式);手册条目两线 sha256 相同;两份 CHANGELOG 各 +1 条。⑨ **本轮验证**:两线 `runData` 各 `written: 3`;两线 `mt_build` OK;`check_lang_sync` 两线 exit=0;`tooltip_color_audit` PASS(0);`verify_crafting_recipe_uniqueness` `shaped=91 collisions=0`;`verify_bountiful_pools` `rews 94 ALL OK`;开 jar 核对 4 类 + 6 资源 + 配方 + 790 键 + 贴图 sha。⑩ **未做(如实登记)**:游戏内行为验证(用户已延后)、探针与用例、26.1.2 迁移。
21. **紫色飞星 / 金色飞星筹码 + 飞星粒子 FX(2026-09-21;三线均已落地)**:① **物品** `purple_shooting_star_chip`(紫色飞星,史诗 = `ASTRAL_DICE_EPIC`)/`golden_shooting_star_chip`(金色飞星,传奇 = 物品层 `ASTRAL_DICE_LEGENDARY`);两枚**共用同一物品类** `item/chip/ShootingStarChipItem`(只覆写 `curioTick` 转交执行器 —— 二者差异全在执行器,且触发必须共享冷却,**不能拆两份实现**);注册面 = `item/ModItems`(WHETSTONE_CHIP 之后)+`init/ModCreativeTabs`(**星光类**段 = STAR_COIN_HAMMER 之后)+`datagen/ModItemModelProvider`+`datagen/ModRecipeProvider`+`data/astral_dice/tags/{item|items}/chips.json`+`data/curios/tags/{item|items}/chip.json`+手册 `entries/chips_starlight/{purple,golden}_shooting_star_chip.json`(**spotlight+crafting 两页**,`sortnum`=7/8)+lang 7 键+贴图两张(`textures/item/{purple,golden}_shooting_star_chip.png` ← `images/紫色飞星.png`、`images/金色飞星.png`)。② **新执行器** `combat/ShootingStarManager`(两线各一套,**语义逐字等价**):触发检测挂在筹码 `curioTick`(每 tick;共享冷却 200 tick,**首次触发即写入** ⇒ 两枚同时装备时重复调用**幂等**)、粒子下落走服务端 tick 调度(1.21.1 `ServerTickEvent.Post` ↔ 1.20.1 `TickEvent.ServerTickEvent`+`phase == END`,照 `LivingPageFlightScheduler`)。判定:半径 **3 格**取最近合格目标、「敌对」= `HostileTargets.isHostile(viewer, target)` **再收窄掉「未被激怒的中立生物」**（用户 2026-09-24 裁决「飞星应当只对敌对目标生效」；判据 `ShootingStarManager#isStarTarget`，**有意收窄、非绕过入口** —— 详见「敌对目标」纪律段的第二处豁免)。③ **时序**:粒子起点 = 目标**头顶(碰撞箱上沿)**正上方「行程」处(行程 = 旧口径 × **3.6** = 历史 1.2 × 高度 3)、下落 **10 tick(0.5 秒)**、**落到那一 tick 才结算伤害**;两枚同时装备时紫色先落(t=0→10)、金色在**第一束命中后隔 10 tick** 开始(t=20→30)。⚠️ **2026-09-24 用户裁决调整为「高度提升 3 倍、下落速度加快到 6 倍、双飞星间隔加快到 2 倍」**(改前:行程 ×1.2、20 tick、间隔 20 tick;`FALL_DISTANCE_MULTIPLIER` = 3.6、`FALL_TICKS` = 10、`VOLLEY_GAP_TICKS` = 10)。**粒子**复用「活体书页」轨迹参数(间距 0.45 / 每步 2 粒 / `FLASH` 命中爆闪),但把**不可染色**的 `END_ROD` 换成**可染色且自发光**的自有粒子 `astral_dice:glowing_dust`(选项 `particle/GlowingDustOptions`、客户端 `client/GlowingDustParticle`:覆写光照为全亮 `15728880` + 渲染类型改半透明混合;紫 `0.70,0.42,1.00` / 金 `1.00,0.82,0.29`,scale 1.2)。⚠️ 该粒子的接线段 = `init/ModParticles`(三线同构)+`assets/astral_dice/particles/glowing_dust.json`(贴图取原版 `generic_0..generic_7`)+`client/ModClientEvents#registerParticleProviders`(三线)+主入口注册 —— 漏任一处 = 客户端没有 provider(退回原版未发光 dust 或紫黑方块)。④ **新工具类** `combat/EliteTargets`(**精英/Boss 判定唯一入口**,与 `HostileTargets` 正交 —— 前者判立场后者判强度档位):**血量 > 40 或 护甲 > 20**(用户裁决阈值),或 `BossEntityUtil.isBossEntity`(`c:bosses`/boss 血条),或**神化(Apotheosis)持久化 NBT** 的 `apoth.boss`(`Invader.BOSS_KEY`)/`apoth.miniboss`(`Elite.MINIBOSS_KEY`) —— **不硬依赖 Apotheosis**(未安装则键恒不存在 ⇒ 恒 false);两线均经 `Entity#getPersistentData()` 读取(1.20.1 落 `ForgeData`、1.21.1 落 `NeoForgeData`,**调用面一致**故无需分平台)。⑤ **附件 1 键** `shooting_star_cooldown_end`(long,**不 `.sync()`**、**不跨死亡保留** ⇒ 不进 1.20.1 白名单/`DeathPreservedBonuses`);玩家级附件计数 1.21.1 **86 → 87**、1.20.1 **88 → 89**。⑥ **「星光」复用既有** `item/StarLightManager`(+1 层),**不新建效果**(用户明确:计时器不创建效果,只在 tooltip 显示剩余秒数 —— 见 `ModTooltipHandler.addShootingStarCooldown`)。⑦ **配方**(非进阶 / 「星光」流派):紫色 = `XXX/DBD/PPP`(X=紫水晶碎片、D=星币尘、B=空白筹码、P=星盘)、金色 = `XXX/DBD/GGG`(X=荧石粉、G=黄金星盘);两线 `verify_crafting_recipe_uniqueness` 期望 `shaped=91 collisions=0`(飞星批 89 → **91**)。⑧ **赏金板**:紫色飞星(史诗 EPIC)**必须进** `astral_rews.json`(`rarity=EPIC`、`unitWorth=2800`;条目追加在 `whetstone_chip` 之后 = `ModItems` 注册序);金色飞星为**传奇 UNCOMMON ⇒ 不进任何池**(与星币锤/电磁炮同口径)。⑨ **文案**:lang 两线 **797/797**;两份 CHANGELOG 各 +1 条。⑩ **未做(如实登记)**:游戏内行为验证(用户已延后)、探针与用例、26.1.2 迁移。

22. **26.1.2 接线补齐（2026-09-25；用户指示「一点小收尾：sound 素材不入库（排除），26.1.2 缺失的机制补全」）—— 上方迁移清单第 6–21 条的**资产**此前已迁到该线，但**机制接线**整批缺失；本批一次性补齐并做了全量对等审计**：
  ① **本批补齐的接线**（**只改 `neoforge-26.1.2`，两条发布线零改动**）：`combat/CardRegistry` 补 bite/dragon_roar 的四处分支（`defaultUses`/`maxRoll`/`minRoll`/`register` —— **漏 `defaultUses` 会静默回退成 10 次耐久**）；`item/card/ExclusiveCardUtil#isExclusive` 与 `item/card/RandomCardHandler` 补符卡福/祸 + 撕咬/龙之咆哮共 4 张专属牌；`screen/CardInventoryMenu` 两个卡牌槽补专属守门（并补上该文件**缺的 `item.card.ExclusiveCardUtil` import**）；`combat/DiceCombatModifiers` 补 **6 条攻击修饰器**（zhao「白泽赐福」溢出治疗转攻击力 / teru 降神快照缓存 / teru 狐光「新目标追加」/ nardis「威压」/ mamushi 真龙形态+撕咬锁存 / sherry「推理时间」）；`combat/DiceCombatEvents` 补 **5 处**（撕咬赐福触发、zhao 骰点判定、hanna 骰点、龙之咆哮命中减益、mamushi+sherry 赐福结束）；`event/PlayerTickEvents` 补 **5 处 tick**（`zhao.tickBlessing` / `teru.tick` / `TemporaryCardUtil.tick` / `hanna.tickPassing` 四者**必须在 `% 20` 早退之前**，`HuoCardItem.tick` 在之后）；`event/ChipDamageHandler` 补 sherry「推理时间 + 挚友守护」纯减伤（LOWEST，**刻意不带「不可致死保护」**，那只是磨刀石/气囊口径）；`component/DeathPreservedBonuses` 补蛟龙觉醒**第 3 槽位**；`item/chip/CurrentCoreChipItem` 补蛟龙强制冷却硬闸门（纵深防御）；`resource/ResourceConversion#giveItem` 补**星币钱包吸收钩子**；`event/ModTooltipHandler` 补 **10 个物品**的 tooltip 段（6 立牌 + 撕咬/龙之咆哮 + 符卡福/祸，从 1.21.1 **按行范围逐字抽取**，不手抄）。
  ② **同批修掉的三处「静默缺口」**（都是「资产齐了、机制没接」的同一类，且都不会报错）：1) **`TemporaryCardUtil.tick` 在 26.1.2 从未被任何地方调用** ⇒ 临时牌的「无效果⇒清牌」安全网与「自毁巡查」（非白名单槽位 + 附近地面掉落物）整条不跑；2) `ResourceConversion` 缺钱包吸收 ⇒ 钱包开启时星币仍以**物品**形式进背包（该线 `economy/StarCoinCurrency` 本身与 1.21.1 逐字相同，只是没人调）；3) **7 个数据标签缺条目** —— `data/astral_dice/tags/item/{chips,combat_cards,effect_cards,is_exclusive,signs}.json` 与 `data/curios/tags/item/{chip,stand}.json`，分别漏了 2 枚飞星筹码 / 2 张专属战斗牌 / 符卡福祸 / 4 张专属牌 / 6 个立牌；其中 **`curios:stand` 缺失 = 那 6 个立牌根本无法装进饰品槽**，`curios:chip` 同源 = 飞星筹码装不上（标签是**手写**资源，不随 datagen 生成 ⇒ 加物品时必须手工补，这与「新增物品清单」里的标签项是同一处）。
  ③ **可复用的「接线对等」审计法（本批新增；比方法级 diff 灵敏，建议每次迁移后跑一遍）**：对两条线各做 —— ① 收集本模组**全部顶层类型名**（`^class|interface|enum|record`）；② 逐类型求「被哪些文件引用（`\b名字\b`）」的集合；③ 两线取**集合并差**，**阈值必须取 1**（取 2 会漏掉 `TemporaryCardUtil.tick` 这类**单点**缺口 —— 本批第一次跑就是被阈值 2 漏掉的）。再补三类纯资产面判据：**数据标签的内容集**（`data/**/tags/**/*.json` 的 `values`）、**`src/main/resources` 的文件集**、**`src/generated` 的文件集**（26.1.2 是 `resources + clientResources` **两根**）。⚠️ 三个易漏点：类型引用是**文本匹配** ⇒ 注释里的类名也会命中（命中 ≠ 真缺口，必须逐条人工确认，例如 `AstralDiceMod`/`DataGenerators`/`NeoForgeFixesLog` 的「缺失」全是 1.21.1 专有文件的正常差异）；`tags/items` ↔ `tags/item`（forge ↔ neo）目录名差异要先归一；同理 `recipes/` ↔ `recipe/`、`advancements/` ↔ `advancement/`。
  ④ **两处「不可迁移」的平台差异（本批登记；今后别机械照搬）**：1) 1.21.1 / 1.20.1 的 `data/astral_dice/recipe{s}/suspicious_stew.json`（`minecraft:crafting_special_suspiciousstew`）**不迁 26.1.2** —— 实测 26.1.2 的 `RecipeSerializer.class` 里**已无该字符串**（该原版特殊配方类型已被移除），照搬只会得到一条无效配方；2) `astral_dice.neoforge_fixes.mixins.json`（`fixes/DamageStackSanitizer` / `mixin/fixes/LivingEntityDamageContainerMixin` / `fixes/NeoForgeFixesLog`）是 **1.21.1 专有**的 NeoForge 缺陷绕行，26.1.2 不需要（该线另有 `event/ChipSlotMigrationHandler`）。
  ⑤ **判据**：三线 `gradlew build` SUCCESS + 六处部署落盘 + 开 jar 核到新符号（`CardRegistry` 含 bite/dragon_roar、`DiceCombatModifiers` 含 NardisSignItem/MamushiSignItem/ZhaoOverflow、`DiceCombatEvents` 含 MamushiSignItem/ZhaoSignItem、`PlayerTickEvents` 含 TemporaryCardUtil/HuoCardItem）+ jar 内 7 个标签条目数**回读**正确（`chips` 62 / `combat_cards` 12 / `effect_cards` 17 / `is_exclusive` 6 / `signs` 24 / curios`chip` 61 / curios`stand` 23）；九个守门脚本 EXIT=0（`check_lang_sync` 三线 **812 keys** 一致、`tooltip_color_audit` PASS、`verify_bountiful_pools` ALL OK）；tooltip 覆盖面对等（两线同为 **132** 个 `stack.is(ModItems.*)` 分支、差集为空）。⚠️ **未做（如实登记）**：26.1.2 的**游戏内行为验证与三侧读数 diff**（`TESTING-SPEC.md` §13.2）—— 本批只保证「接线对等 + 构建/守门通过」，实机读数待用户安排。
  ⑥ **同批工程项**：仓库根 `sound/` 已加入 `.gitignore`（用户裁决「sound 素材不入库（排除）」）—— 成品 ogg 早已入库三线的 `src/main/resources/assets/astral_dice/sounds/`，根目录那份只是转换前的 wav 源料。
...
注册 id:`attack_card_*` / `defense_card_*`,费用/耐久由 `CardRegistry` 集中定义(耐久:中/大/特大/暗影突袭/防御=10、名刀=5、蓄力=1、全力攻击=5、**撕咬=1、龙之咆哮=5**)。
...
备注说明:法伤牌(激光/板砖/轨道炮/定向爆破/活体书页)的伤害受忍者立牌「效果牌伤害增益」加成;**复制计数范围为全部效果牌**——治疗/伤害/互动/专属牌同样计入忍者立牌、魔法秘典与魔法箭袋的计数(原 `countsForCopy()` 类型过滤已移除);⚠️ **魔法箭袋例外**(2026-09-24 用户裁决第二版):它**只统计伤害效果牌**(激光/板砖/轨道炮/定向爆破/活体书页,判定 `BaseEffectCardItem.isDamageEffectCard`)——非伤害类效果牌不再参与箭袋追踪;治疗类牌(`isHealingCard()`)使用后触发大当家被动;专属牌见 `ExclusiveCardUtil`。
...
| 教主 | teru | teru_sign | TeruSignItem |
| 绿洲女王 | nardis | nardis_sign | NardisSignItem |
| 蛟龙 | mamushi | mamushi_sign | MamushiSignItem |
| 怪力侦探 | sherry | sherry_sign | SherrySignItem |
| 人偶师 | hanna | hanna_sign | HannaSignItem |
...
| 吸血鬼 papara | 嘬你一口:3:00 内攻击时恢复骰神赐福最终伤害的一半、受伤时恢复单次伤害的一半;**所有以血量条件决定是否生效的效果在主动技能期间均视为条件通过**——美工刀 ≥60%、肾上腺素/磨刀石 ≤50%、可口糖果"满血"、磨刀石"生命值 > 1"的不可击杀保护(⚠️ 该保护自 2026-09-24 起带 1:00 冷却,**汲取不绕过冷却**)等;**触发后进入「锁定(生效中)」态,须等本技能施加的全部计时器跑完才进入主动技能冷却**(锁定期内按键无效并提示「主动技能生效中!」) | 可爱即正义:生命值低于一半时攻击力/防御力 +3(汲取期间) |
...
| 绿洲女王 nardis | 女王特权(2026-09-27 新增,**非选择器类**):立即获得 **2 张战斗牌 + 1 张效果牌**(战斗池 = `CardCategory.BATTLE` 的「攻击+防御」**混合随机 2 张**、效果池 = `CardCategory.EFFECT` 随机 1 张;专属牌由既有池逻辑排除;走 `VitaminPillChipItem.giveCard` 发牌漏斗 ⇒ 维生素药丸治愈与教主狐光计层照常触发),并获得「女王特权」效果 **3:00**(HUD 计时器 + 图标 = 立牌贴图;**该效果实例同时是「临时牌是否仍在有效期」的唯一真值**);**临时牌只能装备与使用** —— 带**单层**附魔光效、**不可丢弃**(三处落点,缺一即失效,2026-09-24 补:① 两个牌根类 `CardItem` / `BaseEffectCardItem` 的 `onDroppedByPlayer` 拒绝**服务端**抛出;② `mixin/client/LocalPlayerDropGuardMixin` 在客户端 `LocalPlayer#drop(Z)Z` 的 HEAD 用同一判据拒绝 —— 否则客户端「本地删掉 + 服务端静默拒绝 + 无槽位校正包」⇒ **牌凭空消失、地上无掉落物**;③ `mixin/container/ContainerClickGuardMixin` 在 `AbstractContainerMenu#clicked` 的 HEAD 拦下容器内 Q(`THROW`)与「把光标栈拖出 GUI」(`PICKUP`+`-999`) —— 否则会**先把牌摘出槽位**再走「尽力退还」,满包即**销毁**)、**不可移入任何其它容器**(白名单 = 玩家物品栏 0..35 / 手中 / 骰子卡牌栏);⚠️ **自毁机制 + 提示行(2026-09-24 追加)**:第三方容器(AE2 存储总线 / 机械动力物品舱口)的插入路径**不经槽位校验**,且 NeoForge 自己的 `ItemStackHandler#insertItem` **只**问 `isItemValid`(缺省恒真)、**不**问 `canFitInsideContainerItems`(只有 `ComponentItemHandler` 会问)⇒ **拦不住** ⇒ 改成「看见即销毁」:每 20 tick 的 `TemporaryCardUtil#purgeOutOfPlace` 清掉 ① 当前打开容器菜单里**非白名单槽位**的牌 ② 玩家附近 16 格的**地面掉落物**;有效期由组件 `temporary_card_expires` 承载(发放 + 3:00) ⇒ 存进容器过夜再取回也会自毁(⚠️ 存储**内部**够不着、不主动清;旧存档无戳的牌按「不设限」)。tooltip 另加黄字提示行 `tooltip.astral_dice.temporary_card`(**不显示剩余时间** —— 客户端只同步自己的效果实例),到期(或效果被移除 / 死亡 / 离线到期后重登)时连同**已装配到骰子里的**一并清除(**骰子上的临时性存在 `AppliedStone.temporary`,物品标记由 `CardInventoryMenu` 双向透传**);背包放不下时只发能放下的张数、**绝不落地**;**不进入冻结(2026-09-27 最终裁决,推翻当日更早的冻结方案)** —— 效果生效期间**可以再次释放**,只受**冷却**限制(**释放当刻即开始玩家级冷却,默认 180 秒**,沿用既有减免机制);**再次释放不清空旧临时牌**,每次再发一整套 **2 战斗 + 1 效果**并**叠加**在现有牌上;**每次释放把「女王特权」有效期重置为 3:00(3600 tick)**;**牌被用光不再提前结束效果**(效果只由 **3:00 自然到期**、**玩家死亡**、**内部 `ModEffectRemoval`** 结束 —— ⚠️ **外部 `/effect clear` / 牛奶会被产品拒绝**:`ModEffectEvents#onModEffectRemovalPrevented` 取消对任何 `astral_dice:*` 效果的外部移除,属本模组长期既定设计,`ZHAO-SIGN` 用例早有同形断言;效果消失时清空剩余临时牌含已装配者);**临时牌上限 9 张**(`MAX_TEMPORARY_CARDS`,**已装配到骰子的也计入**)⇒ **已达上限则阻止释放**,否则发放**夹取到上限**(总数永不超 9);**判据顺序 = 冷却 → 上限 → 物品栏空位(<2)** | 威压:骰子**已装配**的每张攻击牌使攻击力 +2、每张防御牌使防御力 +2(**2026-09-21 用户裁决由 +1 加强**;常量 `NardisSignItem.BONUS_PER_CARD = 2`,读时现算:攻击走 `DiceCombatModifiers` 修饰器链 ⇒ 实战与 tooltip/GUI 同源,防御走 `setDefenseArmorBonus` 每 tick 现算、1 防御 = 2 护甲;卸下立牌或卸下卡牌立即失效;临时牌同样计入) |

| 游戏大师 ren | 熊孩子特权(**目标选择器类**,可选任意玩家与**自身**):使目标获得 1 张随机卡牌 + 鼠鼠护盾(选择器里该技能显示为「鼠鼠护盾」;确认前不进冷却,冷却/充能写在动作自己的 `apply` 里) | 鼠鼠救我:佩戴者超过 5:00 没有鼠鼠护盾时自动获得 1 张随机卡牌 + 鼠鼠护盾(作用对象即佩戴者本人;口径全文见下方「游戏大师立牌 ren『鼠鼠护盾』口径」) |
| 教主 teru | 降神(**目标选择器类**,只能选**其它玩家**,不能选生物与自身):锁定目标 50% 攻击力与防御力加给自己(施法瞬间快照),目标每攻击一个新目标消耗 1 层狐光并追加「**施法者快照攻击力(施加时的基础攻击力 + 从降神目标获得的 50%) + 当前剩余层数**」;⚠️ **同一目标的后续攻击照常吃这份加成**(2026-09-25 用户裁决:旧实现「已攻击过的目标 return 0」使加伤只对每个目标**第一击**生效);层数已为 0 时攻击新目标**不消耗、不追加**;持续到**该目标自己的下一次骰神赐福结束**,生效期间无法再次施放 | 狐光:层数资源(上限 20,**跨死亡保留**);自身(以及降神目标)合成/获得/装备攻击牌 +1 层/张,施放降神立即 +3 层;**防刷**:拾取地面上的攻击牌不计层、反复把同一张战斗牌插入骰子卡牌栏再卸除也不计层(按「历史同时装备张数」去重) |
| 蛟龙 mamushi | 连锁反应(2026-09-27 新增,**非选择器类**):自身立刻获得 1 张专属战斗牌「撕咬」(**真龙形态**下改为「龙之咆哮」),并使 **12 格内的所有友方队友**各获得 1 张随机卡牌(**目标当前没有任何卡牌时额外再给 1 张**;未组队时自动对该维度所有其他玩家发牌;**总人数上限 32**,按**距离由近到远**发放;真龙形态下取消 12 格限制);**强制冷却 1:00,不可被充能/电流核心/命运指引等任何减免缩短**(`forcedActiveCooldownTicks()` + `forcedCooldownUntil()` 硬闸门,早于电流核心判定);**不起锁定(生效中)态** | 湖沼之王:佩戴者**使其他角色获得卡牌**时按**受益人数**获得「觉醒」层数(**同一发牌事件内按受益人去重、单次封顶 3 层**;自己给自己发牌不计),**觉醒达 8 层**即进入「**真龙形态**」(锁存):攻击力 **+5**、主动取消范围限制、手中/背包/骰子卡牌栏内的「撕咬」→「龙之咆哮」(骰子内转换后**费用放不下的丢弃**)、此后主动发放的也是「龙之咆哮」,并以立牌图标常驻显示该效果;**觉醒与真龙形态死亡不重置(`.copyOnDeath()` + `DeathPreservedBonuses` 第 3 槽位)、仅卸下立牌归零**(觉醒在 8 层封顶)。触发源收敛到**唯一发牌漏斗** `VitaminPillChipItem.giveCard(giver,receiver,card)` ⇒ 本主动、**「你有我有」**与今后任何给牌功能的卡牌一并生效，⚠️ **计数器失效(2026-09-24 用户裁决)**：进入真龙形态后「觉醒」**不再累计**（加层入口直接早退，值恒为 8），tooltip 也不再显示「觉醒：x / 8」计数器行，只留「真龙形态」标注 |
| 怪力侦探 sherry | 怪力投掷(2026-09-21 新增,**非选择器类**):把 **12 格内所有敌对目标**按抛物线扔到**准星指向的地面或水面**(距玩家 **1–6 格**;2026-09-22 用户裁决,替换原「玩家面前 2 格内」),**落地之后**对被投掷目标造成 **2 点伤害**(走本模组**法伤**类型 `astral_dice:card_spell`、**显示法伤数字**;⚠️ **2026-09-24 用户裁决「为技能类伤害创建单独的伤害标签,避免与法伤混用」** —— 伤害类型已由 `astral_dice:card_spell` 改为 **`astral_dice:skill_damage`**(此前用 card_spell 时,那是法伤白名单的第 4 条 matcher ⇒ 立牌的固定点数会被忍术飞镖 / 贯穿之铳 / 紫晶骰子 / 标记喷罐 / 魔法箭袋 / 效果牌加成层层放大,并触发电击手套的范围波及);不再按玩家近战结算 ⇒ 不吃骰伤、不触发战斗伤类筹码)并施加 **1 层「标记」**;「推理时间」达 **5 层**时额外造成 **5 点伤害**;落点 = **准星射线优先 + 自动回退**(2026-09-22 用户补充裁决;`ClipContext.Block.COLLIDER` + `Fluid.ANY`,射线最长 6 格:命中地面/水面 ⇒ 用该处;射到天空/更远处 ⇒ **基准落点 = 面前 6 格**;基准落点被掩体/高方块阻挡 ⇒ **向内逐 1 格靠近**重试 —— 「可落地面」三条判据 = 该列向下 8 格内有碰撞面且面高 ≤ 玩家脚底 +1 格、该面之上 **3 格净空**(`noCollision`)、自眼睛到落点的路径无方块遮挡;面前 1–6 格**全部不可落** ⇒ 拒绝施放。常量 `DROP_STEP = 1.0` / `DROP_CLEARANCE = 3.0`,入口 `resolveThrowDestination` + `groundBelow`；**水中落点（2026-09-23 实测定量，用户裁决「维持现状」）**：玩家没入水中时，向下射线起点（脚底+1）已在液体内 ⇒ 首个命中面 = **玩家脚底所在格的顶面** ⇒ 落点 = 脚底 + 0.05~1.05 格，**恒在水中、与玩家身体同高，不是水面**。实测 26.1.2：水面在脚底 +3 时落点 `dy=+1.04` / `hd=1.62`；对照「岸上、前方水面低于脚底 2 格」`dy=-1.95`（= **水面**）⇒ 水面路径确实存在，只是被水位高低决定；故 A、B 两段合起来才是完整判据，用例 `SHERRY-WATER-26.1.2` 常驻)、多目标围绕落点沿垂直方向 ±0.9 格铺开;**目标须视线可达**(`hasLineOfSight` —— 隔墙的怪不投掷,用户 2026-09-22 裁决「阻止怪被从墙后拉出来」);**冷却基础 120 秒(与枪匠同款链:先按充能上限封顶、再按诡异骰子减半 ⇒ 仍受既有减免)**;投掷飞行期间目标 `noAi` 且 `fallDistance` 归零(`SherryThrowManager.FLIGHT_TICKS` = 10 tick);**落地后再保留 2 秒 `noAi`**(`LANDING_FREEZE_TICKS` = 40,2026-09-22 用户裁决的保险冻结:落点就在玩家面前,恢复 AI 会被反手秒杀;解冻时同时清掉索敌目标);范围内没有可投掷目标时**零消耗**(提示 `msg.astral_dice.sherry_no_target`);**面前 1–6 格全部不可落(被方块堵死)时同样零消耗**(提示 `msg.astral_dice.sherry_bad_ground`,其文案已同步为「面前 1-6 格内被方块阻挡,无法投掷到地面」;**注意「射到天空/更远处」不再拒绝** ⇒ 该提示只在被堵死时出现;`castThrow` 返回 `THROW_BAD_GROUND`,经 `handleUse` 返回 FAIL ⇒ 不进冷却/不充能/不发牌);**不起锁定(生效中)态** | 侦探出击:每攻击一个**新的**、**最大生命值 ≥ 20** 的**敌对目标**获得 1 层「推理时间」(上限 **4**,2026-09-25 由 5 下调);**每个目标仅提供 1 层** —— 已提供过的目标 UUID 记在附件 `sherry_reasoning_targets`(上限 256、超出按最旧淘汰),首次命中即登记、已登记的直接跳过;**卸下立牌时记录与层数一起清空**(记录刻意**不**跨死亡,与手电筒筹码同款口径);每层攻击力 **+1**、受到伤害 **−1**;**每次骰神赐福结束扣除 1 层**;**层数跨死亡保留(`.copyOnDeath()` / 白名单 + `DeathPreservedBonuses` 第 4 槽位)、仅卸下立牌归零**;计层只认玩家**直接近战**(`getDirectEntity()` 是本人才计;弹射物/法术/AOE/环境伤害不计)。挚友守护:同队伍内若有玩家装备**人偶师立牌**(按注册 id `astral_dice:hanna_sign` 字符串匹配),则**该玩家**受到的伤害 **−1**(无队伍不生效) |
| 人偶师 hanna | 漂浮魔法(2026-09-21 新增,**非选择器类**):使自身获得「魔女漂浮」**1:00**;效果期间四条语义 —— **移动速度 +20%**(`hanna_float` 的 `MOVEMENT_SPEED` 修饰器)、**掉落伤害 -100%**(`LivingIncomingDamageEvent`/`LivingAttackEvent` 对 `DamageTypes.FALL` 直接取消)、**任何近战攻击被闪避**(判定「直接伤害实体是生物」后走 `DiceCombatEvents#applyDodgeCancel` 的**最前置取消**,弹射物/法术/AOE 不算近战)、**无法使用末影珍珠**(取消 `RightClickItem` 里的末影珍珠使用,珍珠不消耗);**冷却走全局默认 180 秒**(用户 2026-09-21 裁决,仍受诡异骰子/充能链减免);**不起锁定(生效中)态** | 幻想千金:① **战斗骰点 = 6** ⇒ 自身 **+1 星币**;② **路过友方玩家**(半径 **3 格**,用户裁决)⇒ 该玩家 **+1 星币** + 自身 **+1 层「人偶制作」**;**自身处于「魔女漂浮」时**该玩家改为 **+3 星币**;③ 「人偶制作」满 **7 层** ⇒ **归零并转为「人偶完成」**(用户裁决),此后路过友方玩家使该玩家**额外**获得 **迅捷 II (1:00) + 3 星币**;④ **本被动整体每 1:00 仅触发 1 次**(骰点 6 与路过**共享**同一冷却附件;一次触发最多结算**一名**玩家 = 距离最近者)。挚友祝福:**路过装备「怪力侦探」立牌的玩家**(按注册 id `astral_dice:sherry_sign` 字符串匹配)⇒ 该玩家获得 **力量 II (1:00) + 抗性提升 (1:00) + 1 层「推理时间」**;**每 1:00 仅触发 1 次**(与幻想千金**各自独立**计时)。层数真值在附件、效果只作 HUD 镜像;**不跨死亡保留**，⚠️ **计数器失效(2026-09-24 用户裁决)**：进入「人偶完成」后「人偶制作」**不再累计**（唯一写入漏斗 `setCraftLayers` 在已完成时直接早退）⇒ 不会在归零后重新爬满 7 层、「人偶制作」图标也不会复现 |
...
| 黄金骰子(无星盘) | 稀有(RARE) | 史莱姆 lulu、上班族 padman(该档模板 `WYW/TET/TDT`)、绿洲女王 nardis(2026-09-27 新增;**不得照抄 padman 整张表** ⇒ 实为 `CYC/TET/TDT`,凋灵骷髅头×2 换成仙人掌×2,见「配方网格必须全局唯一」) |
| 黄金骰子 + 星盘 | 史诗(EPIC) | 大侦探 fanny、吸血鬼 papara |
| 钻石骰子(±星盘) | 史诗(EPIC) | 忍者 komachi、占星师 haiqing、骇客 nancy_lu、枪匠 moses |
| 钻石骰子 + 黄金星盘 | 传奇(UNCOMMON) | 调查员 rin、护法 misaki、大当家 fen(该档模板 `RPR/RER/GDG`)、风水师 zhao(`GCG/RER/ZPZ`;2026-09-26)、教主 teru(2026-09-27;**与 zhao 区分材料** ⇒ `GCG/LEL/ZPZ`,红石块×2 换成**荧石**×2、2026-09-23 由荧石粉改荧石)、蛟龙 mamushi(2026-09-27;**与 fen 区分材料** ⇒ `SPS/SES/GDG`,红石块×4 换成**海晶灯**×4、2026-09-23 由海晶碎片改海晶灯) |
| 下界合金骰子 + 黄金星盘 | 传奇(UNCOMMON) | 秘密侦探 bonnie |
| 紫晶骰子 + 黄金星盘 | 史诗(EPIC) | 怪力侦探 sherry(2026-09-25 用户裁决:骰子由**钻石骰子**改**紫晶骰子**、网格 `PBP/AEA/IDI` 不变) |
| 紫晶骰子 + 黄金星盘 ×2 | 稀有(RARE) | 人偶师 hanna(2026-09-25 用户裁决:两个**线**位改**黄金星盘**、骰子由**黄金骰子**改**紫晶骰子** ⇒ `GQG/LEL/TDT`) |

### 已统一命名的效果/数据（参考）

- 效果注册 id(**两个发布子项目在本树内实测各 43 个**(2026-09-21 人偶师批后复核,取 `register("<id>")` 计数:1.21.1 = 43、1.20.1 = 43;此前记的 40 与更早的 39/37/34/35 均已过时),完整以 `effect/ModEffects.java` 为准;**两侧注册条数与 id 集合一致,但不是逐行相同** —— 平台包装类型不同:neo 用 `DeferredHolder<MobEffect, MobEffect>`、forge 用 `RegistryObject<MobEffect>`(文件行数 159 vs 158;归一包装名后仍有 9 行差异),核对时请按 id 集合而非逐行):`misaki_burst`(爆发)、`papara_bite`(汲取;2026-09-12 由「嘬一口」更名,主动技能名「嘬你一口」不变)、`jasmine_sweep`(清扫)、`magic_tome_count`(魔法秘典计数)、`fen_frenzy`(战斗爽)、`nancy_lu_hack`(远程侵入)、`weak_mark`(虚弱印记)、`weakness_reveal`(弱点识破)、`undercover_investigation`(隐匿调查)、`investigation_bonus`(调查阶段加成)、`fate_guidance`(命运指引)、`healing`(治愈)、`cutter_ready`/`cutter_blade_ready`(美工刀)、`revenge_halberd`(复仇之戟)、`blue_curse`(青之诅咒)、`berserk`/`king_power`/`unwavering`(功能效果牌)、`monster_laser`/`monster_brick`/`orbital_strike`/`directional_blast`(法伤牌加成)、`living_page`(活体书页)、`dice_blessing`(骰神赐福)、`marked`(标记)、`pandaman_taunt`(嘲讽)、`charge`(充能)、`moses_broken`(破绽)、`empower`(赋能)、`ren_shield`(鼠鼠护盾)、`ren_counter`(反击;层数附件的可见镜像,图标 `images/反击.png`)、`zhao_blessing`(白泽赐福;32→34 的两个新效果之一,图标**复用风水师立牌贴图**)、`misfortune`(厄运;层数镜像效果,图标 `images/厄运.png`)、`nardis_privilege`(女王特权;绿洲女王立牌主动,有限时长 3600 tick = 3:00,同时是「临时牌是否仍在有效期」的唯一真值;图标**复用立牌贴图**,见 §「绿洲女王立牌(nardis)实现契约」)、`mamushi_dragon`(真龙形态;蛟龙立牌被动,觉醒 ≥ 8 且佩戴时**常驻** `Integer.MAX_VALUE` + 玩家 tick `refresh`、**不登记 `EffectTimerGuard`**(照 teru 降神/zhao 赐福);图标 `textures/mob_effect/mamushi_dragon.png` 从立牌贴图**逐字节复制**)、`dragon_roar_break`(破防(龙之咆哮);专属战斗牌「龙之咆哮」命中时施加,`ARMOR` **-8** 属性修饰器 = **减 4 点防御**(1 防御 = 2 护甲),HARMFUL 1:00、重复命中只刷新不叠层;图标 `textures/mob_effect/dragon_roar_break.png` 从「龙之咆哮」卡牌贴图**逐字节复制**)、`sherry_reasoning`(推理时间;怪力侦探立牌 sherry 的唯一资源,**层数真值在附件、效果只作 HUD 镜像**(层数 = `amplifier + 1`)、BENEFICIAL、常驻 `Integer.MAX_VALUE`、**不登记 `EffectTimerGuard`**(照 teru 狐光/降神);图标 `textures/mob_effect/sherry_reasoning.png` 从**立牌贴图逐字节复制**)、`hanna_doll_craft`(人偶制作;人偶师立牌 hanna 的**层数镜像效果**,真值在附件 —— **0..6,永不为 7**(达 7 时由立牌归零并转挂 `hanna_doll_complete`);层数 = `amplifier + 1`、BENEFICIAL、`Integer.MAX_VALUE`、**不登记 `EffectTimerGuard`**;图标 `textures/mob_effect/hanna_doll_craft.png` = `images/人偶制作.png`)、`hanna_doll_complete`(人偶完成;hanna 的**完成态常驻状态**,由「人偶制作」满 7 层**归零转换**而来,无限时长、卸下立牌时由 `clearSignData` 移除;图标 `textures/mob_effect/hanna_doll_complete.png` = `images/人偶完成.png`)、`hanna_float`(魔女漂浮;hanna 主动,**有限时长 1200 tick = 1:00**,`MOVEMENT_SPEED` +20% 属性修饰器 —— ⚠️ neo 用 `ADD_MULTIPLIED_TOTAL` + ResourceLocation id、forge 用 `MULTIPLY_TOTAL` + **UUIDv5 字符串** id(`uuid5(NAMESPACE_URL, "astral_dice:hanna_float_speed")` = `aef19f28-557c-564f-bb6c-e72d5f6926d9`,生成口径同 `jasmine_sweep_speed`);掉落免疫 / 近战闪避 / 禁用末影珍珠三条语义在 `HannaSignItem` 的事件里;图标 `textures/mob_effect/hanna_float.png` 从**立牌贴图逐字节复制**))。**26.1.2 线曾为例外，2026-09-19 已收口**：`neoforge-26.1.2` 原为旧「待命」等待器实现(自带该线本地 `effect/ReadyEffect`),实测 **33** 个,比两个发布线多 `haiqing_ready` / `bonnie_ready` / `moses_ready`;**2026-09-19「立牌主动前置门控收口」移植到该线后为 32 个、当时 id 集合与两个发布线逐项一致**(⚠️ **2026-09-27 复核:该「逐项一致」已过时** —— 两个发布线此后累积到 37 个,26.1.2 仍停在 **32** 个,差额 5 个正是**尚未迁移**的三批:`zhao_blessing`/`misfortune`(风水师)、`teru_descent`/`teru_huguang`(辉夜/照)、`nardis_privilege`(绿洲女王);26.1.2 的 id 集合目前是两个发布线的**真子集**,计数器不一致属预期,不是缺陷)(该移植同时删掉 `BaseSignItem.isSkillWaiting`/`tickSignReadyTimeout` 及其调用点、本地 `effect/ReadyEffect.java`、三个立牌子类的 `READY_TYPE` 与 `addEffect(…,MAX_VALUE)`、`AstralPartyCommand` 的伴生清理,把 `handleUse` 从 `abstract` 改为发布线同款「默认实现 + WARN」,并按实测删除该线多出的 3 个 `effect.astral_dice.*_ready` lang 键与 3 张 `mob_effect/*_ready.png` ⇒ lang 键 **686→683**、`textures/mob_effect/` **35→32**,两侧一致);主线分支 `multi-1.20.1-1.21.1` 的 1.21.1 子项目仍为 **33** 个(该三件套由本合并 KI-M1 只在两个发布线移除,见 KNOWN-ISSUES)。⚠️ 该移植还证明一条硬约束:**`@EventBusSubscriber` 不能留在「零 `@SubscribeEvent` 方法」的类上**(`net.neoforged:bus:8.0.5` 对这种情况直接抛异常)⇒ 删掉某个类里全部订阅器时必须同时去掉注解。
- 数据组件注册 id(共 12 个,完整以 `component/ModDataComponents.java` 为准):`temporary_card`(**临时牌标记**,绿洲女王立牌 nardis 批新增:Boolean,1.21.1 走 `DataComponentType`、1.20.1 走 `ItemDataKey`;装配进骰子后改由 `AppliedStone.temporary()` 承载并由 `CardInventoryMenu` 双向透传)、`temporary_card_expires`(**临时牌到期刻**,2026-09-24「自毁机制」批新增:long、绝对 `gameTime`,发放写「现在 + 3:00」、由 `TemporaryCardUtil#realignExpiry` 在**发放 / 再次释放 / 登录**三处按效果剩余重写;**刻意不 `.networkSynchronized`** —— 纯服务端判定,客户端不读)、`misaki_sign_stacks`、`card_uses`(战斗牌剩余次数)、`weapon_enhancement`(**骰子**铁砧升星 + 已装配卡牌/费用配置;立牌无此组件)、`owner_uuid`(专属牌绑定)、`jasmine_atk_bonus`/`jasmine_def_bonus`、`padman_atk_bonus`/`padman_def_bonus`/`padman_last_refresh`/`padman_force_six`
- 玩家附件注册 id(共 **91** 个;1.20.1 为 **93** 个,多 `damage_effect_bonus` 与 `curse_original_amount`(**2026-09-21 飞星批新增 1 键后重新计数:1.21.1 = 86+1 = 87、1.20.1 = 88+1 = 89 —— ⚠️ 本行口径始终是**玩家级**附件;若机械计数 `ATTACHMENTS.register(` 行数会得到 **93 / 95**(差额 9/10 为 `LivingEntity` 等非玩家作用域的键),两者不要混用;新增键 = `mamushi_awakening`(int,**两线均 `.sync()`** 供 tooltip 读 + `.copyOnDeath()`;觉醒层数,**8 层封顶**)、`mamushi_bite_bonus_active`(boolean,不同步,撕咬加成锁存)、`mamushi_forced_cooldown_until`(long,不同步,强制冷却硬闸门,**不受任何减免**)—— 三者同时进 `DeathPreservedBonuses` 第 3 槽位(仅 awakening)与 1.20.1 的 `AstralData#onPlayerClone` 白名单;**1.20.1 的 synced 键 29 → 30**(+`mamushi_awakening`);**2026-09-26 本批新增 7 键后计数:1.21.1 = 78、1.20.1 = 80;2026-09-25 的计数为 71/73(1.21.1 = 67、1.20.1 = 69 + 游戏大师 ren 的 4 键);此前记的 62/64 为「立牌主动技能三态化」之前的数字,当时新增的 5 个锁定态键两版本均有,不属差异项;上批新增的 `sign_active_max_cooldown` 同样两版本均有**),完整以 `component/ModAttachments.java` 为准;1.20.1 的 30 个 synced 键见上方「forge-1.20.1 子项目关键差异速记」):忍者 `komachi_use_count`/`komachi_last_card`/`komachi_damage_bonus`(旧 `komachi_extra_plays` 出牌银行附件已删除)、治愈 `healing_points`/`healing_prev_blessing`/`healing_timer_end`(见「治愈流派规范」)、调查员 `rin_pages`/`rin_gift_signature`/`rin_gift_tick`、骇客 `nancy_lu_passive_type`/`nancy_lu_active_bonus`/`nancy_lu_active_bonus_until`/`nancy_lu_hidden_until`/`nancy_lu_ender_pearl_immune_until`、大当家 `fen_recharge`/`fen_last_blessing_tick`、看板娘 `mimi_returned_card_count`、枪匠 `moses_broken_attack_rewarded`/`moses_dodge_counter_rewarded`、肉弹战车 `pandaman_max_health_bonus`/`pandaman_taunt_source`、魔法秘典 `magic_tome_use_count`/`magic_tome_last_card`、来源 `weak_mark_source`/`undercover_source`、`investigation_stage`(调查阶段)、`player_starlight`(星光)、`sign_active_cooldown_end`(立牌主动冷却,玩家级)、`sign_active_max_cooldown`(立牌主动技能**本次冷却实际使用的最大冷却 tick**,是全部冷却减免方的唯一基准;**仅服务端使用、不 `.sync()`**。**语义已变更(2026-09-25「立牌主动技能三态化」)**:旧写法是"由 4 个「开始冷却」入口与 `sign_active_cooldown_end` **成对写入**";现改为「**锁定开始时先写基准**(供锁定期间各减免方读它并累加进减免池);**真正进入冷却时按 `max(0, 基准 − 减免池)` 抵扣后改写并把冷却起点设在当前时刻**」——即"锁定态下 `cooldown_end` 为 0,基准先写、抵扣后改写",**不再与 `cooldown_end` 成对写入**。三态判定入口 `BaseSignItem.isSignActiveLocked`(**硬上界 `sign_active_lock_end` 未到 且 本立牌门控效果实例仍在**;外部把同一效果刷新得更长**不会**延长锁定,因为硬上界不随刷新重算;`lock_end==0` 的忍者只按锁定标记判定)、结束迁移 `endLockAndStartCooldown`)、`sign_active_cooldown_end`(立牌主动冷却,玩家级)、**锁定态 5 键(均玩家级、均不 `.sync()`)**:`sign_active_lock_sign`(锁定中的立牌注册 id,**全限定**如 `astral_dice:komachi_sign`;`""` = 未锁定)、`sign_active_lock_end`(门控计时器**硬上界**:触发时刻算定的本技能施加的全部计时器到期刻取 max;`0` = 无自身计时器,仅忍者)、`sign_active_reduction_pool`(锁定期间累计的冷却减免池,进入冷却时一次性抵扣并归零)、`sign_active_lock_grace_end`(忍者 1:00 宽限到期刻;`0` = 宽限已失效/不适用)、`sign_active_lock_played`(忍者宽限期内是否出过效果牌;`true` = 宽限保险失效、改为遵循出牌周期)、`sign_ready_type`/`sign_ready_expire`(待命等待器:当前主线在用,见下方「需指定目标的技能」)、`magic_quiver_tracking`/`magic_quiver_first_card`/`magic_quiver_cooldown_end`、`buffer_shield_cooldown_end`、`star_coin_hammer_bonus`、`cursed_sword_bonus`/`cursed_sword_blessing_triggered`、`candy_chip_play_bonus`、`satellite_give_cooldown_end`/`satellite_play_bonus`/`satellite_play_bonus_cooldown_end`、`fight_poison_with_poison_regen_at`、`effect_timer_ends`(计时器守卫)、`defense_card_consumed_blessing`、`warp_engine_portal_cooldown_end`、`ender_die_totem_cooldown_end`、`guide_book_given`(首次加入赠书)、`eight_sided_roll_accum`、`damage_effect_bonus`、`dice_curse_ratio`、`effect_card_cooldown_end`/`effect_card_play_count`/`effect_card_bonus_plays`(出牌轮一次性 +1 标记)、`fate_active_until`、`lulu_last_hurt_tick`、小猪存钱罐 `piggy_bank_use_count`、`flashlight_granted_targets`(手电筒已发放目标)、`airbag_cooldown_end`(安全气囊冷却)、`electric_glove_aoe`(电击手套法伤扩散武装)、`railgun_cooldown_end`(电磁炮冷却)、`empower_decay_at`(赋能递减计时)、`living_page_cycle_bonus`(活体书页本周期出牌数加成)——以上 6 个为**两版本共有**(此前漏列);`sherry_reasoning_layers`(int,怪力侦探 sherry 的「推理时间」层数真值;**两线均进死亡保留** —— 1.21.1 `.copyOnDeath()`、1.20.1 走 `AstralData#onPlayerClone` 白名单 **+** `DeathPreservedBonuses` **第 4 槽位**;**不 `.sync()`** ⇒ 不进 `SYNCED_KEYS`(客户端 tooltip 的层数计数器读的是**镜像效果实例** `sherry_reasoning` 的 amplifier)、`sherry_reasoning_targets`(String,怪力侦探「**已推理目标**」UUID 逗号串 —— 「每攻击一个**新**目标获得 1 层、**每个目标仅 1 层**」的唯一判据;**卸下立牌即清空**(与层数同寿命)、**刻意不跨死亡**、不 `.sync()`;上限 256、超出**按最旧淘汰**(与手电筒筹码 `flashlight_granted_targets` 的「满则停发」不同,理由见 `SherrySignItem#MAX_TRACKED_TARGETS`))、`hanna_doll_craft_layers`(int,人偶师 hanna 的「人偶制作」层数真值 —— **0..6,永不为 7**(满 7 由立牌归零转挂 `hanna_doll_complete`);**两线均不跨死亡保留**(技能原文未声明 ⇒ 按未声明即不保留的既定口径,故**不**进 1.20.1 白名单、也**不**占 `DeathPreservedBonuses` 槽位)、`hanna_doll_complete`(boolean,hanna「人偶完成」状态真值;同上不跨死亡)、`hanna_fantasy_cooldown_end`(long,hanna「幻想千金」的 1:00 触发冷却;**骰点 6 与路过共享同一冷却** —— 字面「该能力每 1:00 仅触发 1 次」的口径)、`hanna_blessing_cooldown_end`(long,hanna「挚友祝福」的 1:00 触发冷却,**与幻想千金各自独立**)——以上 4 键**两版本均有**、**均不 `.sync()`**、均仅服务端使用、`curse_original_amount`(青之诅咒原始值)为**仅 1.20.1**。**2026-09-26 本批(风水师 zhao)新增 7 键,两版本均有且键名逐字相同**:`zhao_blessing_active`(是否持有白泽赐福的真值)/`zhao_blessing_skip_cycles`(两分支用的跳过计数)/`zhao_prev_blessing`(上一 tick 的骰神赐福真值,下降沿基准)/`zhao_overflow_bonus`(Int,溢出治疗转来的攻击力加分)/`zhao_overflow_remainder`(Float,取整余数累加器)/`huo_card_next_damage_tick`(Long,厄运下一次周期伤害的绝对刻) + 出牌轮 `fu_card_cycle_bonus`(符卡-福本周期出牌数加成;**本批唯一 `.sync()` 的新键** —— 1.21.1 内联 `.sync`、1.20.1 经 `SYNCED_KEYS`,依据是客户端预检 `isBlockedOnClient → isBurstFull → getMaxAllowed` 必须在本轮上限上看到这份额外 +1,否则客户端会误判「已打满」)。**死亡重生保留的键为 `rin_pages`、`komachi_damage_bonus` 与 `guide_book_given` 三个**,由**两层机制**共同保证(2026-09-15 用户裁决,必须遵守):① **克隆复制**——1.21.1 经 `AttachmentType.Builder#copyOnDeath()`、1.20.1 经 `AstralData#onPlayerClone` 的死亡分支复制这三个键(两侧键集合必须一致);② **死亡暂存/重生回写**——`component/DeathPreservedBonuses`(`LivingDeathEvent` 暂存;`PlayerEvent.Clone` 以 `priority=LOWEST` 回写、`PlayerRespawnEvent` 兜底,表项取走即空操作)。**为什么必须两层**:默认 gamerule(`keepInventory=false`)下死亡会把立牌从饰品槽丢出,Curios 的 tick 轮询判定"已离身"→ `onUnequip` → 各立牌 `clearSignData` 把这两个键清零,且**早于**克隆复制——只做①会被抢先抹掉(测试世界是 `keepInventory=true`,永远测不出这个坑);回写 handler **必须保持 LOWEST**,否则会被克隆复制覆盖成 0。其余键与 1.21 附件默认行为一致——死亡不复制;死亡清理(`LivingDeathEvent`,在旧实体上执行)不得清除这些键(尤其**禁止清除 `guide_book_given`**——清了就必然在下次登录补发一本)。**《恋的规则书》的赠书守卫(2026-09-15 用户裁决,必须遵守)**:**仅在玩家第一次进入世界时发放一次,此后任何情况下都不得自动发放**——退出重进、**死亡重生后再次登录**、把书丢弃/放进箱子/销毁后重登,一律不得补发;守卫是附件 `guide_book_given`(发放前只查它,**不查背包**;`GameplayConstants.GIVE_GUIDE_BOOK_ON_FIRST_JOIN` 为**类初始化读一次**的静态开关,改配置须重启生效),因此它**必须在死亡保留集合内**(否则死亡后回默认 `false`,而发放挂在 `PlayerLoggedInEvent` ⇒ 下次登录必再发一本——2026-09-15 实测缺陷,已修)。注意手册物品本身是 `patchouli:guide_book`,`astral_dice:astral_guide` 只是数据组件里的书籍 id。此外**这两个值只在佩戴对应立牌时才作为加成生效**:唯一判定入口是 `SpellDamageRegistry#effectCardDamageBonus`(忍者)与 `#livingPageBonusPages`(调查员),伤害结算与 tooltip 显示都必须走它们,禁止直接读原附件值做加成或显示加成(死亡掉落立牌后累计值保留但不再加成,直至重新装备)。**2026-09-24 本批(磨刀石保命冷却 + 保命优先级显式化)新增 1 键,两版本均有且逐字相同**:`whetstone_guard_cooldown_end`(Long,磨刀石「保留 1 血」的 1:00 触发冷却;**不 `.sync()`** —— 仅服务端 `event/ChipDamageHandler` 判定用),写入点 = `WhetstoneChipItem#modifyIncomingDamage` 里 cap **实际削减了伤害**时(不致命的攻击不耗冷却);保命优先级同时明确为 **安全气囊 > 磨刀石**(气囊接管即 `return`,磨刀石本次完全不参与,连 -2 减伤也不叠加)。 **2026-09-24 本批(onEquip 参数语义修复 + 星光发放闸门)新增 1 键,两版本均有且逐字相同**：`starlight_equip_grant_flags`(Int 位掩码,「装备时获得 N 层星光」的**装备会话发放闸门**；**不 `.sync()`**、不进 1.20.1 `SYNCED_KEYS` —— 仅服务端判定)。位含义 = `StarLightManager.GRANT_BIT_{ATM,STAR_COIN_HAMMER,BANK_CARD_UNLIMITED,FEN_SIGN}`(2026-09-25 又加 `BANK_CARD_LOW = 16` / `BANK_CARD_HIGH = 32`,银行卡-余额少/多;`FEN_SIGN` 位是大当家立牌的「1 分钟计时重置」、**非星光发放**),置位/清位入口 `StarLightManager#claimEquipGrant`/`#releaseEquipGrant`,清位点 = 各筹码 `onChipUnequip`(星币锤复用既有那个)与大当家立牌 `clearSignData`。 **2026-09-25 本批(「卸除即扣除」)再新增 1 键,两版本均有且逐字相同**：`starlight_equip_grant_amounts`(Int,**发放账本**,每位 4 bit 打包存「本次装备**实际**获得的星光量」;槽位序 = `GRANT_BIT_*` 的位号 bit0→最低 4 bit;**不 `.sync()`**、不进 1.20.1 `SYNCED_KEYS` —— 仅服务端判定)。读写入口 `StarLightManager#grantedStarlight`(读)/ `starLightEquipGrantAmounts` 附件(经由 `ModAttachments#getStarlightEquipGrantAmounts`),写入点 = `StarLightManager#grantStarlightOnEquip` / `#grantStarlightFloorOnEquip`,归零+扣回点 = `StarLightManager#revokeStarlightOnUnequip`。
...
11. **目标选择器的可选目标集合(冻结)**:
    ...
    | `huo_card` | `ENEMY_OR_RIVAL` | false | 敌对生物 ∪ 中立生物(宠物除外) ∪ **非同队玩家**;**队友玩家 / 被动生物 / 自己不可选** |
...
5. lang 中物品/效果/tooltip key 使用该 id。**新立牌文案里的字面百分号必须写成 `%%`**（如 `§e50%%§7`）：立牌 tooltip 经 `ModTooltipHandler.translationString` 无条件 `String.format`，裸 `%` 会让**把鼠标指向该立牌就崩客户端**（2026-09-21 教主立牌 `teru_active` 即因此崩过，见「语言文件同步规范」）；写完跑 `pwsh -NoProfile -File tools/check_lang_sync.ps1` 确认退出码 0。
6. 在 `ModItems` 注册、`ModCreativeTabs` 加入创造栏、`datagen/ModItemModelProvider` 加入模型、`datagen/ModRecipeProvider` 加入配方、`curios/tags/item/stand.json` 加入标签、**`astral_dice:signs` 汇总标签（`data/astral_dice/tags/item/signs.json`）**加入标签（新增立牌必须进汇总标签，否则图鉴/联动按标签检索时会漏）。
   ...
   - **配方网格必须全局唯一(2026-09-27 用户裁决「三组一起拆开」,必须遵守)**:两条「形状 + 材料完全相同、只有产出不同」的配方里,**原版合成台只会产出其中一条** —— `RecipeManager#getRecipeFor`(`RecipeManager.java:94-104`)取 `byType(CRAFTING)` 中**第一条** `matches()` 命中的配方,而 `byType` 是 `ImmutableMultimap`,其顺序来自 `SimpleJsonResourceReloadListener#prepare` 的 **`HashMap` 迭代序**(`SimpleJsonResourceReloadListener.java:32`)⇒ 另一条**永远合不出来**(连 JEI 里点它,合成台给的也是赢家)。⇒ **同档立牌不得照抄另一立牌的整张合成表**,填充材料必须换成该立牌专属材料(且该材料在同类配方中未被同格位使用);新增/改写配方后必须跑 `pwsh -NoProfile -File scripts/verify/verify_crafting_recipe_uniqueness.ps1`(⚠️ **必须在 `runData` 之后**,只看生成目录 + 手写目录)并保持 **0 重合**;该脚本已挂进 `scripts/test/TESTING-SPEC.md` §9 静态守门清单。**已修的三组实例**:`zhao_sign` 保留 `GCG/RER/ZPZ` ⇒ `teru_sign` 改 `GCG/LEL/ZPZ`(**荧石**;2026-09-23 由荧石粉改)、`padman_sign` 保留 `WYW/TET/TDT` ⇒ `nardis_sign` 改 `CYC/TET/TDT`(仙人掌)、`fen_sign` 保留 `RPR/RER/GDG` ⇒ `mamushi_sign` 改 `SPS/SES/GDG`(**海晶灯**;2026-09-23 由海晶碎片改)。
...
| 中文名 | 注册 id | 品质 | 效果 |
|---|---|---|---|
| 手电筒-强光 | `flashlight_chip` | 紫 | 攻击敌对目标时星光 +1,**同一目标仅 +1 层**(已发放目标 UUID 记录于附件 `flashlight_granted_targets`,上限 256 个;记录满后不再发放,不做淘汰;卸下筹码或死亡时清空,见 `FlashlightChipItem.onAttack`);每 4 点星光 +1 点**攻击伤害**(2026-09-25 起走**额外加伤**通道 `astral_dice:extra_damage`,不再计入攻击力) |
| 八面骰 | `eight_sided_dice_chip` | 蓝 | 触发骰神赐福后掷 1d10,累计每满 8 点 +1 星光;骰点恰为 8 时立即获得 8 星币(卸下清除累计;**星光已满时保留累计点数**) |
| ATM机 | `atm_chip` | 蓝 | 装备时星光 +1(**卸下按账本严格扣除**,见「星光流派规范」);星光兑换星币时兑换量 +40%(**向下取整且下限为 1**,`ResourceConversion.starlightToStarCoins`) |
| 银行卡-余额少/余额多 | `bank_card_low_chip`/`bank_card_high_chip` | 蓝/紫 | 装备期间星光基础值 +4/+7(下限,`StarLightManager.getBasePoints` 实时计算;可兑换量 = 当前值 − 基础值)。装备时抬到下限并记账、**卸下按账本严格扣除** ⇒ 装卸不再刷星光 |
| 银行卡-用不完 | `bank_card_unlimited_chip` | 金 | 装备时星光 +3(**卸下按账本扣除**);每次骰神赐福结束后,自身及友方玩家(同队成员;未加入任何队伍时为全服在线玩家)获得 3 星币(`onBlessingEnd`,死亡清场不发放) |
| 星币锤 | `star_coin_hammer_chip` | 金 | 装备时星光 +5(**卸下按账本扣除**);持有星币超过 20 枚时,每次进入骰神赐福消耗 6 星币并按持有总数 30% 提升**攻击伤害**(2026-09-25 起走**额外加伤**通道)(`CONSUME_COINS = 6`,2026-09-24 由 3 上调)(星币袋按 9 算;**零散星币不足时自动拆开 1 个星币袋**再按枚扣除,拆袋前做「可存放性预检」——优先拆单袋槽位,否则必须已存在可容纳整袋 9 枚的空槽或未满星币堆,预检不通过则整次消耗放弃,**绝不因物品栏已满而让星币掉落到地上**;加成附件 `star_coin_hammer_bonus`,赐福结束清除) |

| 紫色飞星 | `purple_shooting_star_chip` | 紫 | 路过**敌对目标**时,使其受到 **1 点**伤害(真伤 `astral_dice:true_damage`,忽略护甲值/盔甲韧性;击杀归属玩家)并使自身星光 **+1**;判定半径 3 格(原先的「不对其发动攻击」条件已于 2026-09-25 删除 —— 它读的 `getLastHurtByMob()` 是单槽位、会被任意实体覆盖且 100 tick 后被原版自动清空,实测无效)。粒子(**自发光「发光尘」** `astral_dice:glowing_dust`)自目标**头顶上方**下落(行程 = 旧口径的 **3.6 倍**〔历史 1.2 × 高度 3〕、**下落 10 tick = 0.5 秒** ⇒ 速度 = 原口径的 **6 倍**;落体几何入口 `fallDistance`/`launchOrigin`/`impactPoint`),**落到目标头顶(碰撞箱上沿)即视为命中**并结算;**每 10 秒触发一次**。⚠️ **只对敌对目标生效**(2026-09-24 裁决):中立生物须**已被激怒**才触发(判据 `isStarTarget`) |
| 金色飞星 | `golden_shooting_star_chip` | 金 | 同上,但基础伤害 **2 点**;若目标为**精英怪物或 Boss**(见 `combat/EliteTargets`:血量 > 40 或 护甲 > 20 / `BossEntityUtil` / 神化 mod 持久化 NBT 的 `apoth.boss`、`apoth.miniboss`),额外造成**自身当前星光层数**的伤害。**与紫色飞星共享同一 10 秒冷却计时器**(用户裁决;附件 `shooting_star_cooldown_end`;两枚同时装备时先落紫色粒子、命中后隔 **0.5 秒**(原 1 秒)再落金色);该计时器**不创建效果**,只在 tooltip 显示剩余秒数。执行器 `combat/ShootingStarManager`(两线各一套) |
...
| 美工刀-初级 | `cutter_chip` | 紫 | 生命值 ≥ 60% 时**攻击伤害** +2(额外加伤通道),并加上当前治愈点数(处于汲取 `papara_bite` 状态时视为满血,跳过生命值判定) |
| 美工刀-锋利 | `cutter_blade_chip` | 金 | 与初级一致但基础**攻击伤害** +4;与初级为不同物品,可同时装备叠加 |
...
| 中文名 | 注册 id | 品质 | 效果 |
|---|---|---|---|
| 普通瞄具 | `scope_chip` | 紫 | 攻击力 +2(**无条件**);⚠️ **骰神赐福期间**攻击时对目标施加 1 层标记(2026-09-24 用户裁决「瞄具类攻击受**骰神赐福门控** —— 不按骰神赐福模式攻击(近战类)则不应该触发」⇒ `DiceCombatModifiers` 的攻击修饰器已按 `DICE_BLESSING` **补齐门控**;旧实现只判「是否佩戴」;同日第二版裁决:**tooltip 与手册已删去「骰神赐福期间」字样,仅改文本、代码门控保留** ⇒ **文案不再体现该门控**,属用户有意裁决) |
| 鹰眼瞄具 | `eagle_scope_chip` | 金 | ⚠️ **骰神赐福期间**按目标标记层数 ×2 获得攻击力加成并施加 1 层标记(门控口径同普通瞄具:「骰神赐福期间」= `DICE_BLESSING` 生效,修饰器只判「是否佩戴」的旧实现已修;**tooltip 与手册同日已删去该字样**,代码门控保留) |
| 标靶 | `target_chip` | 蓝 | 骰神赐福期间攻击力 +1;触发骰神赐福后对**本次攻击目标之外**距离最近的敌对目标施加 1 层标记(无范围限制;⚠️ 2026-09-24 裁决「不判定正在攻击的那个目标」⇒ 代码里排除本次 `target` 与玩家自身) |
| 标记喷罐 | `marker_sprayer_chip` | 蓝 | 对目标造成远程或魔法伤害后,使其获得 1 层标记 |
| 忍术飞镖 | `ninja_star_chip` | 金 | 效果牌出牌数 +1(固定来源);对目标造成的远程/魔法伤害获得目标标记层数加成(**佩戴即生效**,2026-09-24 起无「已使用伤害效果牌」前提) |
| 手持风扇-小 | `hand_fan_small_chip` | 蓝 | 使用主动技能后,对周围 16 格敌对目标施加标记 |
| 手持风扇-大 | `hand_fan_big_chip` | 紫 | 使用主动技能后,获得一张随机效果牌(不含专属)并对周围范围内敌对目标施加标记 |
| 魔法箭袋 | `magic_quiver_chip` | 紫 | **使用过伤害效果牌**(激光/板砖/轨道炮/定向爆破/活体书页,判定 `BaseEffectCardItem.isDamageEffectCard`)并对**已有标记**目标造成法伤 → 施加一层标记并返还**第一张**使用的伤害效果牌(每 30 秒一次(`COOLDOWN_TICKS = 600`))。⚠️ **2026-09-24 用户裁决第二版**:**取消**首版「活体书页命中必定触发」的例外(活体书页本身即计入该集合,但仍须先「用过一张」);**追踪只统计伤害效果牌**。追踪附件 `magic_quiver_tracking`/`magic_quiver_first_card`,冷却 `magic_quiver_cooldown_end`;**卸下筹码清除追踪记录**)。⚠️ **「带标记」按命中那一刻判定**:修饰器的 `isActive` 在全部 `onHit` 之前统一求值,而活体书页自己那一层是在 `hurt` 返回**之后**才施加 ⇒ **首次打干净目标不触发**(需先用标记喷罐/上一发书页留下标记);2026-09-19 两线实机验证见 `TESTING-SPEC.md` 附录 A「（续 7）」 |
...
| 永动机 | `perpetual_motion_chip` | 金(传奇) | 触发骰神赐福时,若当前充能不足 6 点则**补充至 6 点**(常量 `CHARGE_TARGET = 6`,由 `DiceCombatEvents` 赐福新周期开始处调用);**金品筹码,可进入 Bountiful `astral_rews` 奖励池(2026-09-25 起传奇档开放)** |
| 电磁炮 | `railgun_chip` | 金(传奇) | 充能 ≥ 6(`CHARGE_REQUIRED = 6`)时攻击力 +5(`ATTACK_BONUS = 5`,经 `DiceCombatModifiers`);攻击敌对目标时消耗 6 层充能,延迟 1 秒(`STRIKE_DELAY_TICKS = 20`,经 `RailgunStrikeScheduler`)**对目标 3 格内敌对目标降下雷击**并进入 1:00 冷却(`COOLDOWN_TICKS = 1200`)。**雷击伤害 = `STRIKE_DAMAGE_BASE = 5` + 本次攻击(骰战最终)伤害 × `STRIKE_DAMAGE_RATIO = 0.5`,且不低于 `STRIKE_DAMAGE_MIN = 5` 点**——`onAttack` 在伤害结算前段以即时伤害兜底登记,`DiceCombatEvents` 在 `setNewDamage/setAmount` 之后用 `finalDmg` 回填;降雷时经原版 `LightningBolt#setDamage` 覆写字段默认值 5.0F(点火与闪电苦力怕由 `!visualOnly` 分支独立驱动)。雷击为独立的**真伤**伤害源(`astral_dice:true_damage`,登记于 `minecraft:bypasses_armor` → 无视护甲值与盔甲韧性;由 `mixin/EntityThunderHitMixin` 只对电磁炮降下的闪电替换伤害源,标记集合见 `damage/RailgunBolts`,详见「真伤伤害类型」章节)。**命中范围已收窄为仅敌对目标**(= 敌对生物 ∪ 中立生物(宠物除外),见「「敌对目标」判定规范」一节;其余实体整段取消:不受伤也不被点燃),不进入骰战结算、不触发骰神赐福;金品筹码,可进入 Bountiful 奖励池(2026-09-25 起传奇档开放) |
| 原初核心 | `primordial_core_chip` | 金(传奇) | 每消耗 1 层充能获得 1 层赋能(`EmpowerManager.onChargeConsumed`,经 `ChargeManager.consume` 统一挂钩);每层赋能攻击力 +1、防御力 +1(经 `DiceCombatModifiers` 计入,防御力按 1 防御力 = 2 护甲值折算)。赋能上限 10 层、每 0:30 递减 1 层(`DECAY_INTERVAL_TICKS = 600`),卸下筹码清除全部赋能;**金品筹码,可进入 Bountiful `astral_rews` 奖励池(2026-09-25 起传奇档开放)** |
...
| 电击手套 | `electric_glove_chip` | 紫(史诗) | 使用效果牌时充能 +1(`CHARGE_GAIN_PER_CARD = 1`);充能不少于 4 层(`CHARGE_REQUIRED = 4`)时,**使用伤害类效果牌**会消耗 4 层充能并武装本周期的法伤扩散——本周期内的远程和魔法伤害同时命中目标 3 格(`AOE_RADIUS = 3`)范围内的**其他目标**(⚠️ 2026-09-24 口径统一:波及选目标改用 `isBlessingTarget` 闸门、与法伤链一致;文案同步)(每个效果牌持续周期仅触发一次,武装状态存附件 `electric_glove_aoe`)。⚠️ **触发判定实际是「法伤白名单」而非字面的远程/魔法**:任何经 `DamageEffectCardHandler` 修饰器链的伤害都算,故**活体书页**(`card_spell`)同样触发,且**不要求目标已有标记**(与魔法箭袋相反)、首次打干净目标也扩散;`apply` 为空操作 ⇒ **主目标不吃额外伤害**,唯一表现是 3 格内**其它**敌对目标掉血(单挑时机理上「看不到变化」);扩散用 `astral_dice:true_damage` ⇒ 不递归、不进骰战、不影响书页的标记/credit/返还牌。⚠️ **2026-09-24 修复:波及选目标此前未排除施法者自己** —— 该闸门把「无队伍玩家」视为敌对(相对他人成立、对**本人**不成立)⇒ 近战距离下 AOE 会波及自己;现已显式排除(`e != ctx.attacker`),**定向爆破同病同批同修**(2026-09-19 代码级验证,见 `TESTING-SPEC.md` 附录 A「续 8」) |
| 安全气囊 | `airbag_chip` | 紫(史诗) | 受到致命伤害时消耗 6 层充能(`CHARGE_COST = 6`)使本次伤害无效,随后进入 1:00 冷却(`COOLDOWN_TICKS = 1200`,附件 `airbag_cooldown_end`);充能不足或冷却中不生效。经 `ChipDamageHandler` 在最终伤害阶段以最低优先级拦截,**保命优先级高于磨刀石 > 不死图腾 > 末影骰子**;致命判定一律按**吸收(黄心)之后**的伤害(1.21.1 手动换算 `damage - getAbsorptionAmount()`(下限 0);1.20.1 的 `LivingDamageEvent` 天然在吸收之后);对**无视无敌**(`DamageTypeTags.BYPASSES_INVULNERABILITY`,如 `/kill`、其它模组真伤)的致死伤害**同样有效**(伤害阶段不再按该标签排除),**但虚空伤害 `minecraft:out_of_world` 例外**——虚空击杀优先级最高,气囊与磨刀石一律不阻止(刻意**只按伤害类型**排除 `out_of_world`:同一标签还含 `generic_kill`,一律查标签会把 `/kill` 也变成不可救);并另有死亡事件兜底(`EventPriority.HIGHEST`:绕过伤害管线直接致死时取消死亡并抬血到 1,同样跳过 `out_of_world`) |
...
| 智能手表 | `smart_watch_chip` | 紫 | 玩家物品栏中卡牌数量不足 6 张(`CARD_THRESHOLD = 6`,2026-09-24 由 10 下调)时,**每击杀 1 个敌对目标**获得一张随机卡牌(`SmartWatchChipItem.onHostileKilled`,`LivingDeathEvent` + `HostileTargets` 判定,无冷却无计数;达到阈值后不再发放) |
...
- 魔法箭袋追踪在**伤害效果牌**使用后开启(2026-09-24 用户裁决第二版:只统计伤害效果牌、取消活体书页例外 —— 判定见 `BaseEffectCardItem.isDamageEffectCard`;返还卡类型映射见 `BaseEffectCardItem.cardByTypeId(String)`,旧文档里的 `MagicQuiverChipItem.effectCardByType` 已不存在,2026-09-19 修正)。
...
- **基础值(下限)默认 0**:由 `StarLightManager.getBasePoints` 实时计算——银行卡-余额少/多(银行卡-用不完除外)装备期间提供常驻基础值 +4/+7;可自由支配(可兑换)星光 = **当前值 − 基础值** ⇒ 下限本身不可兑换出去。装备时经 `StarLightManager#grantStarlightFloorOnEquip` 抬到下限并**记账**,**卸下时按账本严格扣除**(不再是「只是停止提供下限」)。
- **消耗后自动补回**:`spend()` 消耗星光后若低于基础值,`set()` 自动补充回基础值(`Math.max(base, ...)`)。
- **⚠️ 「卸除即扣除」全筹码底线(2026-09-25 用户裁决,必须遵守)**:**任何**「装备时获得星光」的筹码,其卸下清理**必须**调用 `StarLightManager#revokeStarlightOnUnequip(player, bit)` 把本次装备**实际**获得的星光扣回;发放侧**必须**走统一入口(一次性 +N → `#grantStarlightOnEquip`;银行卡等下限型 → `#grantStarlightFloorOnEquip`),两者都会申领装备会话闸门并**按实际抬升量记账**。
  - **严禁**「装备加、卸除不加回」(2026-09-25 用户实报的「银行卡刷星光」即此根因:反复装卸可无限累积,银行卡卸下后下限消失、原本不可兑换的 4/7 点立刻变成可兑换 ⇒ 稳定的星币产线)。
  - **严禁**照名义值扣:星光已到上限(`MAX_STARLIGHT = 32`)时装备**一点没涨**,照名义值扣 = 白扣玩家自己攒的星光。
  - **扣回不许走 `set()`**:卸下回调执行时槽位状态未必已更新,`set()` 的基础值下限会被**尚未移除**的银行卡重新抬回去 ⇒ 扣不动;必须直接写 `ModAttachments#setStarlight`。
  - 新增同类筹码**未接这条底线 = 违规**(发放与扣回必须**一一配对**,位号在 `StarLightManager.GRANT_BIT_*` 里登记)。
...
- **架构**（`neoforge-1.21.1/src/main/java/com/merlinkitsune/astral_dice/`）：
  - `TargetType`（现存 dev 线位于共享库 `com.merlinkitsune.starenginelib.target`）：PLAYER / ENEMY / LIVING / **ENEMY_OR_RIVAL**（客户端过滤 + 服务端权威校验共用；排除选择者自身）。ENEMY_OR_RIVAL = 敌对生物(红) 或 非队友玩家(黄,选择者无队伍时对所有玩家生效),供占星师(虚弱印记)/秘密侦探(隐匿调查)等立牌主动使用;**匹配规则（该描述已过时,见下）**: `target instanceof Enemy` 或 `target instanceof Player other && other != selector && (selector.getTeam() == null || selector.getTeam() != other.getTeam())`。⚠️ 现行口径 = 可选中判定统一走 `target/SelectorTargets`（`ENEMY` → `HostileTargets.isHostile(target)` = 敌对生物 ∪ 中立生物(宠物除外)；`ENEMY_OR_RIVAL` → 该判定 ∪ 非队友玩家分支），**禁止**在玩法代码里再写裸 `instanceof Enemy`（2026-09-18 t46b 按 R2-03 标注）。
...
- **确认 / 取消（❌ 旧口径已作废）**：~~确认 = 鼠标右键（默认）或 `CONFIRM_TARGET_KEY`（默认 Enter，可改绑）；取消 = Esc（不打开暂停界面）~~ ⇒ 见下方 `### 现行口径` 第 1–5 条（左键确认 / 右键仅提示 / 右键+潜行取消 / ESC 菜单取消 / J 取消【仅限「按键开启的会话」，2026-09-24 收窄】）。
...
2. **右键（不潜行）= 对自身使用**：**当前**未实现 `target/SelfTargetable` 的选择器类立牌（`allowSelf()` 恒 `false`：占星师 / 秘密侦探 / 枪匠）都只弹 actionbar `msg.astral_dice.target_select.self_unsupported`，**不发包、会话保留**（`TargetSelectionClient.java#useOnSelfBySecondaryClick()`，DEBUG 行 `key=right action=self_unsupported`）。**管线已就绪**：动作若同时实现 `SelfTargetable` 并让 `allowSelf()` 返回 `true`，该标志经 `TargetSelectStartPayload.allowSelf` 下发，客户端右键改为发 `TargetSelectConfirmPayload(token, 自己)` 并 `deactivate()`，稳态提示也换成 `prompt.no_target_self`（**注意**：库内 `TargetType.matches` 仍排除选择者自身 ⇒ 真正启用还需同时放宽目标类型校验，见 `target/SelfTargetable` 的 javadoc）。
...
5. **J（主动技能键 `ACTIVATE_SIGN_KEY`，默认 J）= 取消【⚠️ 仅限「按键开启的会话」；2026-09-24 用户报 BUG 后收窄】**：走 `KeyMapping#consumeClick` 消费（`client/KeyBindingSetup.java#ClientEvents.onClientTick(...)` 的 `ACTIVATE_SIGN_KEY.consumeClick()`），**不是**键盘拦截；DEBUG 行 `key=j action=cancel`。判据 = `TargetSelectionClient.isActive() && !TargetSelectionClient.isHoldToSelect()` —— **「手持即选择」类会话（效果牌在主手自动开启）不吞此键**，否则玩家握着选择器类效果牌（活体书页等）时**永远触发不了立牌主动技能**；服务端同源判据 = `target/TargetSelectionManager#isSelectingByKey`（见第 14 条）。
6. **输入锁定（鼠标 + 滚轮 + 界面）**：
...
14. **效果牌「手持即选择」（2026-09-25 用户裁决；两发布线同构，移植必带）**：四张可对他人使用的效果牌（加急加快 `express_delivery` / 奢华大餐 `luxury_feast` / 你有我有 `you_have_i_have` / 狂暴 `berserk`）**不再需要按键** —— 把卡牌拿在**主手**上即自动开启目标选择器，**卡牌离开主手即关闭**，且该会话**没有倒计时**（服务端 `Session.holdToSelect = true` ⇒ `expireTick = 0`、下发载荷 `durationTicks = 0`；客户端 `expireTick = Long.MAX_VALUE`、`remainingSeconds()` 恒 0）。四件套缺一即退化：
    ...
    - **抑制闩**：显式取消（**下蹲+右键**；⚠️ **2026-09-24 起 J 不再取消「手持即选择」会话**，见下条）会写 `HOLD_SUPPRESSED`（`TargetSelectionManager#cancel`，日志 `hold suppressed … (release the card to re-arm)`）——**只要该牌仍在主手上就不会再次自动开会话**；卡牌离开主手时由 `tick` 清除（日志 `hold suppression cleared`）；登出 / 死亡 / `cancelSessionForTests` 由 `clearSessionState` 一并清理。
    - **主动技能键与手持类会话必须分流（2026-09-24 用户报 BUG 修正；三线同构）**：J 只取消**按键开启**的会话 —— `client/KeyBindingSetup` 判 `isActive() && !isHoldToSelect()`；`target/TargetSelectionManager#isSelectingByKey`（= 有会话且 `!session.holdToSelect`）是**服务端唯一判据**。⚠️ `BaseSignItem.performSkill` 的**第 2 步守卫**与**第 6 步「是否延后冷却」**都必须用它：第 6 步若仍写 `isSelecting`，玩家握着效果牌放出的技能会被误判成「本次主动开了选择会话」⇒ **不进冷却/锁定，电流核心也不充能**。手持类会话在技能触发后**保持瞄准**。
...
- 备注区:浅紫(§d)、无标题、无列表符号。
- **第二被动（联动技能，如「心意相连」「完美帮手」「挚友守护」「挚友祝福」）一律放备注区**，用 `addSignNoteLines(...)` 单独一个键承载（**不要**塞进 `*_passive` 正文，也不要写成 `- ` 子项）：渲染行序 = …被动正文 → `tooltip.add(Component.empty())` → 备注区 → 计数器 → 冷却中。键名沿用语义拼音（`fen_xinyi` / `zhao_wanmei_bangshou` / `sherry_zhishou_shouhu` / `hanna_zhishou_zhufu`）。⚠️ 备注区底色是 `§d`，行内高亮后回落请写 `§7`（`addSignNoteLines` 会统一替换成 `§d`）；写 `§r` 会复位成纯白。
...
### 新增内容 tooltip 文案的唯一基准（2026-09-21 用户裁决）— 必须遵守

**新增立牌 / 卡牌 / 骰子 / 筹码的 tooltip 文案，唯一基准是「玩家创建该内容时给出的主动技能 / 被动技能（或功能）原文」。代理不得自行添加注释，也不得把「技能补充说明」段的内容搬进 tooltip。**

| # | 规则 | 说明 |
|---|---|---|
| 1 | **逐字照搬** | 实词、语序、条件、数量一律照抄玩家原文。只允许四类**排版级**处理：① 按句边界插 `\n` 分行；② 行首两空格标记子项；③ 染色码（`§e` 数值 / `§9` 时间 / `§7` 复位）；④ 标点归一（`"…"` → `「…」`、字面 `%` → `%%`）。**改词、调序、增删信息都算违规** |
| 2 | **「补充说明」「备注」段隔离** | 玩家常在同一条消息里附「技能补充说明」与「备注」——那是**给代理实现技能用的数值与口径资料，不是玩家文案**。整段不进 tooltip |
| 3 | **典型误引入项（黑名单）** | 范围格数（16 / 12 格）、内部上限（狐光 20 层）、容量上限（临时牌 9 张）、人数安全上限（32 人）、强制冷却（1:00）、发牌构成（2 战斗 + 1 效果）、快照 vs 实时口径、死亡 / 重登 / 离线的清除分支、效果图标说明、目标选择器的「排除生物与自身」。**判据：原文里写了才写，代理选的数字一律不写** |
| 4 | **玩家后续追加的指令要并入** | 玩家在创建之后单独发的技能要求（如「狐光防刷」「主动不进入冻结、叠加补给并重置有效期」）**属技能正文的延展，应当并入**；但必须在交付说明里逐条标注来源时间戳供复核 |
| 5 | **基准来源优先级** | ① DSH 会话 `user/message` 原话（唯一权威，多帧 zstd 需 `zstandard` 流式读，或读导入产物 `temp/t-import-dsh/raw/*.jsonl`）② DSH engram 项目记忆（用户后续追加指令）③ `docs/features/*-spec.md` 的「用户口径（原文要点）」段。⚠️ **`docs/features/*-spec.md` 是代理产出**：其数值表多为代理自选，只可当索引，**不得当文案基准** |
| 6 | **自检判据** | 逐句问「这句话的字面来源是哪条用户消息？」——答不上来就删 |
| 7 | **原文笔误按最小改动修正并声明** | 玩家原文里明显的笔误按**最小改动**修正（实例：nardis 被动「每装备一张攻击牌**牌**」→「攻击牌」，2026-09-21 `f3d92e70`），**必须在交付说明里单独声明**，不得静默改写，更不得借机"顺手润色" |
| 8 | **适用范围含手册条目与卡牌 tooltip** | `astral_dice.guide.entry.*` 的**技能描述段**与 `tooltip.astral_dice.card.*` 同样适用本表（2026-09-21 用户裁决，把原先「手册是另一套文本」的口径扩到本约束下）；手册**专用的导航信息**（配方档、由哪张牌发放）保留为客观事实，其余按玩家原文回归 |

落地流程、实现细节词表与校验见技能 `astral-dice-tooltip-optimizer`；染色另见 `tooltip-color-rules`。
...
- **文件**：三线 `lang/ja_jp.json`（各 **803 键**；2026-09-22 怪力侦探批后基线，原 802 → 更早 805），**以 `zh_cn.json` 为基线**（键集必须完全一致）。`key.categories.astral_dice`（1.20.1/1.21.1）与 `key.category.astral_dice.main`（26.1.2）是**预期平台差异键**，各线各留其一。
...
- **稀有度标准(2026-09-25 起 = 「自有 5 档 + 原版普通」,权威实现在前置库 `starengine_lib`)**:
  ...
  - **赏金数据层**:稀有 → `RARE`、史诗 → `EPIC`、传奇 → `LEGENDARY`、普通 → `COMMON`(一一对应);
    ⚠️ **巅峰档与奇特档都没有数据层对应值 ⇒ 一律不得写入任何赏金池**(守门脚本 `$EXCLUDED_TIERS = @('PINNACLE','BIZARRE')`
...
- 新增筹码:在 `ModItems` 注册 + `ModCreativeTabs` + `datagen/ModItemModelProvider` + `datagen/ModRecipeProvider` + `curios/tags/item/chip.json` + **`astral_dice:chips` 汇总标签（`data/astral_dice/tags/item/chips.json`）** + lang(名称/tooltip) + `ModTooltipHandler.onItemTooltip` 对应分支(无需任何"筹码类型"判定链);**品质必须按图标边框颜色确定**(蓝=稀有 `ASTRAL_DICE_RARE`、紫=史诗 `ASTRAL_DICE_EPIC`、金=传奇 `ASTRAL_DICE_LEGENDARY`,见「物品图标约定」);属性类筹码(速度轮滑/摩托头盔/夹心饼干)覆写 `ICurioItem.getAttributeModifiers(SlotContext, ResourceLocation, ItemStack)`,修饰器 id 用 `BaseChipItem.attributeModifierId` 按物品派生(同属性不同筹码不得共用 id,否则后装覆盖先装);同时需在帕秋莉手册 `entries/chips_*` 对应流派目录新增条目(见「帕秋莉手册同步规范」)。
...
- **生存 / 创造各一套**基座（相对 `guiLeft/guiTop` 的偏移）：
...
- **余额条**（96×24 深色圆角面板）显示当前余额：数字**右对齐**、右端固定「钱包按钮 x + 88」（= 条左端 + 90）、
  y = 钱包按钮 y + 7、纯白 `0xFFFFFF`、**不带阴影**。它由**钱包按钮自己在 `renderWidget` 里先画**再画按钮本体
  —— 顺序天然正确（条在按钮之下，图标不会被面板压住），且跟随同一个 y 偏移移动；
  条不做成独立 widget 是为了避免额外的鼠标拦截与「谁在上」的排序问题。
- **贴图来源与形状（2026-09-21 玩家换版）**：不复制第三方像素；**当前 7 张为玩家提供的原画**
  （余额条改成**金色描边**、钱包按钮改成金币叠图；此前那版由生成脚本自绘）。形状约定不变：
  钱包按钮为裸美术 18×20、余额条 96×24、星币/星币袋按钮 13×13 用**原版槽位风格边框**
  （浅灰描边 + 左上白高光 + `C6C6C6` 填充 + 右下暗边，圆角 2），中心放货币图标 ——
  ⚠️ 内区是 9×9，但图标**必须用 7×7 居中**（四周各留 1px 填充）。铺满 9×9 会让图标紧贴高光/暗边，
  看起来像「溢出了边框」（参考实现的硬币美术也正是 7×7）；
  三个按钮各带一张 `_highlighted` 悬停贴图（钱包 = 暖金描边光晕，取款按钮 = 描边/填充转蓝）
  ⇒ **不要**再用白色蒙版做悬停。
- ⚠️ 图标是像素画，缩小会糊：生成脚本一律「LANCZOS 下采样 → **量化回源图调色板** → alpha 二值化」，
  否则棋盘格状中间色会让它看起来发虚。**历史生成器**：`temp/wallet_gui_tex.py`（2026-09-21 玩家换版后
  不再由它产出这 7 张图，保留备查）。⚠️ **换版后必须两版逐字节同步**（`md5` 核对 7 张）。
...
- 客户端**看不到**权威余额（在服务端玩家 NBT 里），所以余额条的数字必须由服务端推上来：
  `economy/StarCoinBalanceSync`（服务端）→ neo `StarCoinBalancePayload` / forge `ModNetwork.StarCoinBalanceMessage`
  （S2C，只带一个 `long`）→ `economy/StarCoinWalletState`（纯静态缓存，**刻意不含任何客户端类型 import**
  —— 专用服务端也会加载到它）。
- ⚠️ **不用「每个改动点各自推一次」**：改余额的路径有自身按钮、发币漏斗、拾取吸收、库的 `/starcoin`、
  以及第三方（FTB）直接调 API，挨个挂必然漏。改用**每 20 tick（1 秒）轮询 + 与「上次发给该玩家的值」
  不同才发包**（`Map<UUID, Long>`）；钱包功能关闭时不发任何包。
- 登录 / 重生时客户端缓存不可信 ⇒ `forceResend`（清掉 last-sent 记录）无条件重发一次。
  ⚠️ forge 的 `PlayerTickEvent` 每 tick 派发两次，靠上面的「值相同即早退」天然幂等。
- 该数字**只用于显示**：一次点击存/取多少仍全部由服务端判定，篡改客户端缓存拿不到额外星币。

### 未实现（按用户「未提及的功能一律不实现」）

`/starcoin pay`、钱包物品、战利品表注入、跨币种换算；**已预留**「星币掉落」的位置。
（余额显示**已实现**：见上面的余额条与 S2C 同步小节。）
...
- **入池判定(闭集规则)**——`scripts/verify/verify_bountiful_pools.ps1` 是唯一守门(0 = 一致):
  ...
  - `rews` = `objs` + 全部卡牌(卡牌**不受**档位限制,**但专属牌除外**)+ 非「不进池档」筹码 + 非「不进池档」立牌;
  - ⚠️ **「不进池档」= 巅峰(`ASTRAL_DICE_PINNACLE`) + 奇特(`ASTRAL_DICE_BIZARRE`)** —— 这两档的**骰子/筹码/立牌**一律不进任何池
    (**二者都没有数据层 rarity 对应值** ⇒ 入池即让 `$want` 取到 `$null`、fail-loud)。
    ⚠️ **传奇档(`ASTRAL_DICE_LEGENDARY`)自 2026-09-25 用户裁决「加入传奇物品，排除巅峰和奇特」起已进池**
    (此前与巅峰/奇特一并排除;**数据层 rarity = `LEGENDARY`**);货币与卡牌不受档位限制(`golden_star_plate` 为传奇但属货币,仍在池中)。
    判据集中在守门脚本的 `$EXCLUDED_TIERS = @('PINNACLE','BIZARRE')`,勿在别处另写档位比较;
  - **专属效果牌一律不进任何池(2026-09-26 用户裁决,必须遵守)**:原话「"符卡-福"与"符卡-祸"均为专属效果牌。**严禁通过除风水师立牌以外的途径获得**,遵守全局专属效果牌规则,**因此禁止进入赏金池**」。判据为**双权威且必须同时一致**:数据层 `data/<ns>/tags/{item|items}/`**`is_exclusive.json`** + 代码层 `item/card/RandomCardHandler`(`EXCLUSIVE_CARDS` / `registerExclusiveCard`)与 `item/card/ExclusiveCardUtil#isExclusive`;当前 **6** 项 = `effect_card_living_page` / `effect_card_fate_guidance` / `fu_card` / `huo_card` / `attack_card_bite` / `attack_card_dragon_roar`(后两项见下条)。**理由**:赏金板是一种**获取途径**,专属牌只允许由对应立牌发放(随机卡牌池早已由 `RandomCardHandler` 强制排除,赏金池同属该规则的适用面)。守门脚本已把期望集合改为「卡牌 **− 专属效果牌**」,池内残留任一专属 id 即报**致命偏差**。✅ **既有违规已按用户裁决一并清出(2026-09-26 同一轮)**:`effect_card_living_page` 与 `effect_card_fate_guidance` 原先**早已**在 `astral_rews` 内(本批之前的既存状态),用户选定「**一并清出**」⇒ 已全部删除,**闸门不再保留任何例外白名单**(`$LEGACY_POOL_EXCEPTIONS` 已清空)。⚠️ **2026-09-25 复核发现 26.1.2 的 `astral_rews.json` 仍残留这 2 项**(该线此前未同步 ⇒ 属**三线不一致**),已一并清出。⚠️ 该删除属**玩家可见的平衡性变更**(赏金板少两个奖励)⇒ 必须写入两份 CHANGELOG 的「内容与平衡性调整 / Content & Balance」小节。⚠️ 运行守门脚本**必须显式传 `-Root <dev 工作树绝对路径>`**:其 `-Root` 默认 `'.'`,在非目标工作树的 cwd 下运行会去校验**另一棵树**并输出 `结果: ALL OK` 的**假绿**(2026-09-26 实测踩坑)。
  - **专属战斗牌同口径 + 一处既有架构缺口(2026-09-27 蛟龙批补齐,必须遵守)**:两张专属战斗牌 `attack_card_bite` / `attack_card_dragon_roar` 现为专属列表的 **5、6 项**(共 **6 项** = 上表 4 项 + 这两张),同样**无配方、不进随机池、不进赏金池**、只由蛟龙立牌发放。⚠️ **重要缺口**:专属守门在**效果牌**侧本来有三处(`BaseEffectCardItem` 的选择器开局 / 客户端预检 / `tryUseCard` 首行),但 **`CardItem`(战斗牌)此前完全没有 owner 检查** —— `CardItem` 没有 `use()` 覆写、`CardInventoryMenu` 的攻/防槽 `mayPlace` 只判类型与费用 ⇒ **专属战斗牌必须自行在装备入口加守门**:两个 `mayPlace` 各加 `if (ExclusiveCardUtil.isExclusive(stack) && !ExclusiveCardUtil.canUse(player, stack)) return false;`(本批已加)。新增专属战斗牌时的完整清单:① `ModItems` 注册(`stacksTo(64)` + `rarity` + `CARD_USES` 默认值;1.20.1 不写 `.component`)、② `CardRegistry` 的 `CardType` **与 `defaultUses` switch 分支**(⚠️ 漏了会静默回退 10 次耐久)+ 定值牌的 `minRoll`/`maxRoll`(`maxRoll` 参与「闪避失败攻击点数最大值」结算,不是纯显示)、③ `RandomCardHandler.registerExclusiveCard`(**不要**加进 `attackCards()`)、④ `ExclusiveCardUtil.isExclusive` 硬编码列表、⑤ `data/<ns>/tags/{item|items}/is_exclusive.json`、⑥ 装备入口 owner 守门、⑦ 贴图/lang/手册/创造栏/模型 provider。
...
- **价值 `unitWorth` 约定**(新增条目须落在同类同品质既有价值带内,数值可微调但不得越带):
  - 骰子按阶层:基础(T0)800/700、T1(黄金档:`golden_dice`/`glass_dice`/`netherrack_dice`)3000/2800、T2(钻石档:`diamond_dice`/`emerald_dice`/`obsidian_dice`/`weird_dice`/`amethyst_dice`)8000/7500(前者 `astral_objs`,后者 `astral_rews`);**T3(传奇档:`netherite_dice`/`crimson_dice`/`ender_dice`)24000/12000**(2026-09-25 传奇进池时定的新带,= T2 的 3 倍);T4(巅峰:`nether_star_dice`)不入池;
  - 货币:星币 250/220、袋装星币 = 9×星币、星盘 4000/3800、黄金星盘 16000/9500;
  - 卡牌:中 300、大 800、史诗 2500、传奇卡 2000~6500;筹码:稀有 900~1500、史诗 2400~3400;立牌:稀有 1200~1600、史诗 1600~3500;**传奇筹码/立牌 10000**(2026-09-25 定:epic 上界 4500 的 ~2.2 倍,且 < 传奇骰子 12000)。
- **价值平衡式**(必须保持,Bountiful 加载时校验,违反会告警 `top value rewards cannot be matched`):
  `astral_objs` 最高价值条目(1 条,`amount.max × unitWorth`) ≥ `astral_rews` 最高价值(2 条之和) × 0.9。当前:传奇骰子(objs)24000 ≥ 传奇骰子(rews)24000 × 0.9 = 21600 ✓(2026-09-25 传奇进池后;此前为「黄金星盘 16000 ≥ (9500 + 7500) × 0.9 = 15300」)。
...
- **品质由图标边框颜色决定（必须遵守）**:创建/新增**筹码**（以及遵循同一标准的其他物品）时，**必须查看图标（`images/` 原图或落地纹理）的边框颜色**来确定品质，不得凭感觉或沿用模板：**蓝框 = 稀有 `ASTRAL_DICE_RARE`**、**紫框 = 史诗 `ASTRAL_DICE_EPIC`**、**金框 = 传奇 `ASTRAL_DICE_LEGENDARY`**（2026-09-25 起为自有档，见 `ModItems` 顶部稀有度标准注释）。图标边框与代码 rarity 必须一致；若发现不一致，以图标为准修正代码，并同步 AGENTS.md 筹码表「品质」列。
...
**口径（2026-09-14 用户裁决；2026-09-15 补充带上下文重载；2026-09-24 重写）**：`敌对目标 = 敌对生物 ∪ 中立生物(宠物除外)`（带上下文时**并**「非同队伍且主动攻击过观察者的玩家」）。**唯一入口** = **前置库** `starengine_lib` 的 `combat/HostileTargets`（2026-09-22 已下沉出本仓）：单参 `isHostile(Entity)` 与带上下文 `isHostile(Entity viewer, Entity target)`；另有消费方注入入口 `installExtraHostileProbe`（把库不该知道的实体，如第三方测试假人，声明为敌对）。任何新增/修改的敌对判定一律调用该入口，**不得**再就地内联写裸判据；**能给上下文的调用点一律用两参重载**。
...
- **中立生物（宠物除外）** = `NeutralMob` 实例，且**不是已被驯服的宠物**（`TamableAnimal#isTame()`，即狼/猫/鹦鹉）。⚠️ **2026-09-24 用户裁决重写**：旧口径要求中立生物「已被激怒」（`isAngry()` = `getRemainingPersistentAngerTime() > 0`，20~39 秒）才计入，现**一律计入**。全原版 `NeutralMob` **直接实现者只有 6 个**（已从本机反编译源码逐个核对）：`EnderMan`/`ZombifiedPiglin`（二者即 `Enemy`，由上一条覆盖）＋ **狼 `Wolf` / 铁傀儡 `IronGolem` / 北极熊 `PolarBear` / 蜜蜂 `Bee`**——**只有这 4 个**是靠「中立生物」这一条进入敌对集合的。**「宠物除外」只用 `TamableAnimal#isTame()`**：它在三平台签名一致（库的 common 源码硬约束），且语义上无遗漏——另一个「有主人」的家族 `AbstractHorse`（马/驴/骡/骆驼）**本身不是 `NeutralMob`**，根本不在此集合里；`OwnableEntity` 在 26.1.2 已改为 `EntityReference` 体系、不再有 `getOwnerUUID()`。**实测影响面**：唯一「既是 `NeutralMob` 又是 `TamableAnimal`」的原版生物就是**狼**，故本条实际等价于「狼在**未驯服**时算敌对目标、驯服后不算」。
...
- ⚠️ **两套口径并存（2026-09-24 审计确立，必须遵守）**：**「需要敌对目标」的入口只有上一条的 `HostileTargets`**，但**法伤链与骰神赐福的闸门是另一套、且更宽** —— `DiceCombatEvents.isBlessingTarget(target, player)` = 任意**非同队玩家** ∪ `HostileTargets.isHostile(target)` ∪ Boss ∪ (`mob.getTarget()==player` || `mob.isAggressive()`)。
  ⇒ **凡 `isBlessingTarget` 已放行的地方，不得再用 `HostileTargets` 二次收窄**（那会静默漏掉三类目标：① **非同队但从未攻击过你的玩家**；② **Boss 实体**（不属 `Enemy`/`NeutralMob` 时）；③ **`isAggressive()` / `getTarget()==player` 为真却不属上述两类**的怪 —— 典型就是上一条列的**山羊/羊驼/狐狸/熊猫**等「会反击但原版未标中立」的生物）。
...
**真伤穿透无敌帧(2026-09-15)**:另有 `data/minecraft/tags/damage_type/bypasses_cooldown.json`(**两版本原版 client-extra 资源 jar 里都没有该标签文件** ⇒ 本模组的文件即其定义),把 `astral_dice:true_damage` 登记进去 ⇒ 真伤**穿透无敌帧**(`LivingEntity#hurt` 的 `invulnerableTime > 10` 早退分支不再对真伤生效;两版本同一处判据,1.21.1 `LivingEntity.java:1190` ⚠️ 2026-09-24 起 `astral_dice:skill_damage`(技能类伤害)**亦**登记于本标签(用户裁决「技能伤害增加真伤修饰器,使其始终造成真伤」)⇒ 技能伤害同样穿透无敌帧。 / 1.20.1 `:1131`)。**副作用如实写明**:电磁炮雷击走**同一个** `astral_dice:true_damage` 类型,因此同样受益;`astral_dice:dice_damage`(反击注入与绯红骰/王之力自伤)**不在其中**,行为不变。
...
**技能类伤害类型(2026-09-24 新增)**:`astral_dice:skill_damage`(`<子项目>/src/main/resources/data/astral_dice/damage_type/skill_damage.json`,三线同源,内容与 `card_spell` 同构),同样登记进 `bypasses_armor.json`(⇒ 无视护甲值 / 盔甲韧性),**并**登记 `bypasses_cooldown`(⇒ **穿透受击无敌帧**,与真伤同等待遇;2026-09-24 用户裁决「技能伤害增加真伤修饰器,使其始终造成真伤」);⚠️ **绝对不得加入法伤白名单**(`SpellDamageRegistry.MAGIC_DAMAGE_TYPES` 或任何 matcher —— 该文件里已留警告注释)。用途 = 立牌 / 技能的**固定点数**伤害(当前唯一使用者:怪力侦探「怪力投掷」落地结算 `SherryThrowManager#settle`)。用户裁决原话:「为技能类伤害创建单独的伤害标签,避免与法伤混用」。**三种自定义类型别混选**:`card_spell`(法伤 —— 要吃修饰器链) / `skill_damage`(技能类固定点数 —— 不吃) / `true_damage`(真伤 —— 修饰器加成的独立结算与各类 AOE 波及,且穿透无敌帧)。


**保命一律按「吸收之后」判定(2026-09-15;2026-09-24 起**磨刀石同口径**)**:**安全气囊的致命判定**与**磨刀石的一切修正计算**(减伤 / 「保留 1 血」/ 其冷却判定)都按**吸收(黄心)之后**的伤害(1.21.1/26.1.2 手动换算 `damage - getAbsorptionAmount()`(下限 0);1.20.1 的 `LivingDamageEvent` 天然在吸收之后,不做换算)。⚠️ **1.21.1/26.1.2 上磨刀石还要把「它省下的那部分」扣回事件**(`setNewDamage(damage - 吸收后净掉血 + 修正后净掉血)`)⇒ **黄心消耗量保持不变**、只有红心扣血被磨刀石改动(否则写回含黄心的新值会让黄心少扣);曾因「只喂给气囊、磨刀石按原值」而**过度削减** —— 带黄心时连黄心都少吃(血量 5 + 黄心 10 挨 8 点被削成 4),2026-09-24 用户实报「1.21.1 不符合预期、1.20.1 正确」;**虚空(`minecraft:out_of_world`)不受气囊保护**(伤害阶段与死亡兜底两处都只按该伤害类型整段跳过),`/kill`(`generic_kill`)仍可被气囊救回(不查 `bypasses_invulnerability` 标签,否则会把 `/kill` 一同变成不可救)。末影骰子保命改为**只清除有害效果**(`MobEffectCategory.HARMFUL`)**外加随标记一同施加的 `GLOWING`**(显式白名单 = HARMFUL ∪ {GLOWING},绝不用 `removeAllEffects()`;两版本同步)而非清空全部,增益一律保留。**为什么必须补 `GLOWING`(2026-09-15 本批 C4)**:`MarkManager.apply` 在施加 `MARKED`(HARMFUL)的同时经 `EffectTimerGuard.apply` 施加同寿命的 `GLOWING`(NEUTRAL,不属 HARMFUL)⇒ 只清 HARMFUL 会留下**残留发光轮廓**,与守卫 `ModEffectEvents.onModEffectRemovalPrevented`「标记仍存在时发光不得被单独清除」(发光与标记同寿命)的口径不一致。**口径收窄**:仅当玩家**确实带着本模组 `MARKED`** 时才连带移除 `GLOWING`——`GLOWING` 另有原版来源(光灵箭 `spectral_arrow`、`/effect`、其它模组),无标记时不得顺手清掉别人的发光;有标记时该效果实例即"标记随身的那份",随标记一起走。

**额外加伤类型(2026-09-25 新增)**:`astral_dice:extra_damage`(`<子项目>/src/main/resources/data/astral_dice/damage_type/extra_damage.json`,三线同源、与 `card_spell` 同构),登记进 `bypasses_armor.json` 与 `bypasses_cooldown.json`(⇒ 无视护甲/韧性与受击无敌帧,与 `skill_damage` 同待遇)。**用途 = 手电筒星光 / 星币锤星币 / 美工刀治愈点三项加伤** —— 它们自 2026-09-25 起**从「攻击力修饰器」(`ATTACK_MODIFIERS`)搬到「额外加伤修饰器」(`DiceCombatModifiers.EXTRA_DAMAGE_MODIFIERS`)**:命中落地后由 `DiceCombatEvents` 在骰战定稿处(`setNewDamage` 之后)**单独结算一段伤害**(复用大当家溅射范式:临时清零 `invulnerableTime` → `hurt` → 还原)。**为什么必须分开(用户裁决)**:并入 `ap` 会污染依赖「攻击力快照」的效果(教主立牌降神的狐光攻击基数 `B`)。⚠️ **文案口径(必须遵守)**:额外加伤一律写「**攻击伤害**」(en `Attack Damage` / ja `攻撃ダメージ`);加到玩家**基础伤害**的才写「**攻击力**」(en `Attack Power` / ja `攻撃力`)。

**Java 入口**(`damage/ModDamageTypes`):`trueDamage(Level)` = 无来源实体(用于电磁炮雷击:与旧的原版闪电一致,不算玩家攻击、无击杀归属);`trueDamage(Level, Entity causing)` = 直接伤害实体为空 + 击杀归属 causing(用于大当家溅射,与旧 `explosion(null, player)` 同形状)。 另有 `skillDamage(Level, Entity causing)` / `extraDamage(Level, Entity causing)` = 同形状(直接伤害实体为空 + 击杀归属 causing),但类型为 `astral_dice:skill_damage`(**技能类伤害,不进法伤链**,见下「技能类伤害类型」段)。**直接伤害实体必须为空**:`DiceCombatEvents#onLivingDamagePre` 以 `source.getDirectEntity() instanceof Player` 作为骰战入口判据,直接实体一旦是玩家,溅射/雷击就会被当成玩家攻击重走骰战并递归触发赐福。
...
4. **⚠️ 禁止在 `MobEffectEvent.Expired` / `Remove` 回调里新增或移除效果(2026-09-25 实测;违反即静默 bug)**:原版 `LivingEntity#tickEffects` 的顺序是「**先发 `Expired`、后 `iterator.remove()`**」,而它对 `activeEffects` 用的是 for-each + `iterator.remove()`;处理器此刻若 `addEffect`/`removeEffect`(= 结构性修改该 map),`iterator.remove()` 就抛 `ConcurrentModificationException`,而原版把它 `catch (ConcurrentModificationException) {}` **静默吞掉** ⇒ **该效果本拍不被移除**,其 duration 已为 0 ⇒ 下一拍 `tick()` 直接返回 false ⇒ **`Expired` 每拍再发一次**,回调里的副作用被**逐拍重放**,直到「不再改 map」为止。⚠️ **降层类效果的镜像必然命中**:原版 `MobEffectInstance#update` 只接受更高的 amplifier ⇒ 降 1 层必须「先 `removeEffect` 再 `addEffect`」。**判据/范式**:这类「效果结束时的副作用」一律**登记到下一 tick 的服务端 tick 末尾**执行 —— 1.21.1/26.1.2 用 `ServerTickEvent.Post`、1.20.1 用 `TickEvent.ServerTickEvent` + `phase == Phase.END`;既有先例 = `MarkManager#PENDING_DECAY`(标记减层)、新增 = `DiceCombatEvents#BLESSING_END_PENDING`(赐福结束的立牌减层:怪力侦探「推理时间」/ 枪匠「弱点识破」)。⚠️ **实测代价(两轮逐 tick 采样)**:一次赐福结束时的减层次数**恰等于当时层数**(4 层 ⇒ 连扣 4 拍直接归 0),即用户所报「骰神赐福结束变成全扣」;同批还会**放大**赐福结束段的全部其它副作用(用不完银行卡发星币、骇客刷新被动并发牌、大碗炖肉治疗、蛟龙/星币锤清锁存)——修好本陷阱即一并归位。⚠️ 仍存疑(未取证、勿据传闻改):`MarkManager#onMarkExpired` 的 `EffectTimerGuard.apply(GLOWING)` 在 GLOWING **缺失**时会 `put`(结构性)⇒ 理论上同类风险;正常路径下标记与发光同寿命、`update` 路径不结构性,故未观察到风暴。
...
- ~~`scripts/verify/verify_probe_case_args.ps1`（探针↔用例参数离线校验）~~ —— **2026-09-21 按用户裁决删除**（`git rm`）：输入对象已随测试资产清零、当前无输入可用，不在重建之列；需要时用 `git log --oneline -- scripts/verify/verify_probe_case_args.ps1` 定位删除前的版本取回。
- 文档内 55 处未来日期（09-25×38 / 09-26×17）待裁决；本批口径已统一为 2026-09-19/20。
- `neoforge-26.1.2` 侧本批未落地：该批功能与裁决 C 待按 `docs/compat-26.1.2-neoforge.md` 迁移。⚠️ **迁移时不得带上「禁止丢弃」（2026-09-20 已删除）**：即 26.1.2 侧的 `HuoCardItem` 只需 `tick` 镜像 + 周期伤害与选择器发牌路径，**不要**移植 `onDroppedByPlayer` / `onItemToss` / `canFitInsideContainerItems` / `msg.astral_dice.huo_card_no_drop` / `tooltip.astral_dice.huo_card_no_drop`（两线已删，26.1.2 从一开始就不该有）。
...
1. **主动「降神」**：`TargetType.PLAYER` + **不实现 `SelfTargetable`** ⇒ 只能选**其它玩家**（排除生物与自身）。锁定目标 `⌊攻击力×0.5⌋` / `⌊防御力×0.5⌋` 给施法者（施法瞬间快照）；**狐光攻击基数 `B` = 施加时的施法者攻击力（基础攻击力）+ `⌊目标攻击力×0.5⌋`**，即**施法者「获得目标 50% 加成后」的快照攻击力**（写法等价于 `⌊施法前施法者攻击力⌋ + ⌊目标攻击力×0.5⌋`；此后施法者的攻击力成长**不计入**）；目标**每攻击一个新目标**消耗 1 层狐光并按 `B + 消耗后剩余层数` 追加**骰战攻击力**（层数 0 ⇒ 不消耗不加成；施法者离线 ⇒ 不加成不消耗）。⚠️ **2026-09-21 用户口径（本条的权威表述）**：追加值 = **施加者的快照攻击力（施加时的基础攻击力 + 从降神目标身上获得的 50% 攻击力）+ 施加者自身狐光层数（按消耗后计算）**；落地位置 `TeruSignItem#castDescent`（基数快照）与 `#consumeHuguangForNewTarget`（消耗 + 加算）。
...
> **实施进展（2026-09-21 补记；26.1.2 侧当时未落地，迁移项见上方清单第 13 / 17 / 18 / 19 条，已于 2026-09-25 完成）**：
> - ✅ **绿洲女王 nardis**（1 号）—— 资产 + 技能 + 临时牌机制，两发布线已落地。
> - ✅ **蛟龙 mamushi**（2 号）+ **撕咬 / 龙之咆哮**（5 / 6 号）—— 资产 + 技能 + 两张专属战斗牌，两发布线已落地。
> - ✅ **怪力侦探 sherry**（3 号）—— 资产 + 技能 + 投掷飞行执行器，两发布线已落地（2026-09-21；三项用户裁决：配方档 = **钻石骰子 + 黄金星盘**、投掷 = **逐 tick 插值飞行**、英文名 = **`Sherry Sign`**）。
> - ✅ **人偶师 hanna**（4 号）—— 资产 + 技能 + 路过判定，两发布线已落地（2026-09-21；四项用户裁决：配方档 = **黄金骰子（无星盘）**、路过半径 = **3 格**、主动冷却 = **全局默认 180 秒**、人偶制作满 7 层 = **归零并转为「人偶完成」**）。原 §3 第 5 条待裁决项（`人偶完成.png` / `人偶制作.png` 的用途）**已由本批技能原文解答**：二者分别是「人偶制作」（层数镜像）与「人偶完成」（完成态）两个效果的图标。⚠️ sherry 的被动「挚友守护」按注册 id 字符串 `astral_dice:hanna_sign` 预留的判定**现已自动生效**（无需回头改 sherry）；另**同步把 sherry 的两处英文引用由 `Puppeteer Sign` 改为 `Hanna Sign`**（hanna 英文名定为 `Hanna Sign`，角色名风格与 `Sherry Sign` 一致）。
> ⇒ 本批 7 件**全部落地**；§0–§3 的「待裁决」条目现已全部作废，仅保留作追溯。
> ⇒ 本段 §0–§3 的「待裁决」条目对**已落地**的三件仅在**追溯**意义上有用；**hanna 相关条目仍然有效**。

### 0. 清单（7 件）与已定/待定字段

| # | 中文名 | 拟定注册 id | 类型 | 品质（用户给定） | 物品层 `Rarity`（既有映射，冻结） | 贴图（仓库根 `images/` 已有） |
|---|---|---|---|---|---|---|
| 1 | 绿洲女王立牌 nardis | `nardis_sign` | 立牌 | 稀有 | `ASTRAL_DICE_RARE` | `绿洲女王立牌.png` ✅ |
| 2 | 蛟龙立牌 mamushi | `mamushi_sign` | 立牌 | 传奇 | `ASTRAL_DICE_LEGENDARY`（**本仓「传奇」用的就是 `UNCOMMON`**，见品质表） | `蛟龙立牌.png` ✅ |
| 3 | 怪力侦探立牌 sherry | `sherry_sign` | 立牌 | 史诗 | `ASTRAL_DICE_EPIC` | `怪力侦探立牌.png` ✅ |
| 4 | 人偶师立牌 hanna | `hanna_sign` | 立牌 | 稀有 | `ASTRAL_DICE_RARE` | `人偶师立牌.png` ✅（另有 `人偶完成.png` / `人偶制作.png`，用途待裁决，见 §3） |
...
8. 若新建效果：`effect/ModEffects`（两线现各 37 个）+ `textures/mob_effect/<effect>.png`（无专用图时按取材规则复用载体物品图）+ lang `effect.*`。
...
7. **是否同步 26.1.2**：**必须同步** —— 该线已纳入主线、三线同批（原「只在两条发布线落地、之后再迁移」的口径作废）。落地时按 `docs/compat-26.1.2-neoforge.md` 处理平台差异，并补 `src/generated/clientResources` 的 `items/*.json` 物品模型定义（26.1.4+ 硬需求）。

### 4. 实施顺序（裁决后）

① 1.21.1 代码（subagent 并行 + 单一 owner）→ ② 1.20.1 同步 → ③ 双线构建 + 全流程冒烟（`preflight→build→env→launch→cases→report→stop`）→ ④ 测试资产（探针 + 用例）并逐条 `--case` 单跑取 PASS（**不要**依赖全目录跑，见下）→ ⑤ 文档（AGENTS.md 三表 + `scripts/test/TESTING-SPEC.md` 附录 A 工程条目）→ ⑥ **两份 CHANGELOG 同步补玩家侧条目**（中英条目数一致）→ ⑦ 本地提交（**不 push**）→ ⑧ 26.1.2 **同批落地** + 三侧读数 diff 一致性测试（`TESTING-SPEC.md` §13.2）。
...
- **26.1.2 同批**：该线纳入主线、与本批**同批落地**（各线一个 subagent 并行）。确有不落该线的例外时，必须在本清单追加迁移项。
...
`zhaogate` 的**真实按键会话路径**（`performSkillForCurio` → `confirm`）本批**未进用例** —— 立牌的「按键 → 会话」接线与其余选择器立牌共用 `BaseSignItem` / `TargetSelectionManager`，该共享路径已由 `LULU-SIGN` / `TERU-SIGN` 覆盖；本批的立牌门控改从**服务端权威入口**取证（生物被拒 / 自身放行）。另：跨维度选择、26.1.2 一致性测试均未覆盖（26.1.2 本批**零改动**；该线三侧读数 diff 仍待补，见 `TESTING-SPEC.md` §13.2）。
...
5. **待办（已裁决：本轮不扩范围，登记待办；2026-09-28 用户裁决）**：施工单 §B 末行「`signprep`/`signstate` 的 `stand` 字段改用**通用**判定」未做 —— 它现在仍读 `NardisSignItem.isEquipped`（对 parunan/lulu/jasmine/komachi 恒 0，且 `putInSlot` 之后同 tick 读回为空）⇒ LOCK-OFFLINE 的 `stand=1` 断言继续按「不依赖该字段」处理。**补做时的口径**：改成按槽内物品类型（`SignItem` 基类 / 槽位标签）判定，并连带复核 LOCK-OFFLINE 两线的 `stand` 相关断言（不得只改探针不回跑用例）。
6. **疑似产品行为（已裁决：登记为待办/限制，本轮收口；2026-09-28 用户裁决 —— 不动产品代码，保留放宽后的断言）**：`terufx` 作用在**真实目标（bot）**上把实例移除后，施法者侧镜像（`caster`/`target`/`cache`）在 **30 tick 内没有收敛**（`AP_X_FX:who=Bot1:removed=1:fx_d_on=0` 之后仍 `caster=0:target=1:cache=4`），与用例注释「真值仍在而实例没了 ⇒ 下一 tick 必然收敛」不符；该相位断言已改为只锚定「命令被正确调用 + 实例确实移除」。**注意：这是「探针用外力移除效果实例」的人造路径，不等于自然结束路径有问题**。⇒ **后续做「镜像/附件同步」专项时一并处理**（届时先加只读读数 + 冷启动两线复跑，先判定是产品缺陷还是 bot 目标特有的测试态差异，再决定是否改产品代码）。详见 `temp/sp_cleanup_report.md` §6.1。
