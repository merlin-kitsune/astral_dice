...
- **入池判定(闭集规则)**——`scripts/verify/verify_bountiful_pools.ps1` 是唯一守门(0 = 一致):
  ...
  - **传奇(物品层 `ASTRAL_DICE_LEGENDARY`)的「骰子」「筹码」「立牌」一律不进任何池**；⚠️ **巅峰(`ASTRAL_DICE_PINNACLE`)同样不得入池**(该档没有数据层 rarity 对应值)(货币与卡牌例外——`golden_star_plate` 为传奇但属货币,仍在池中);
