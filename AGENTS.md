...
| 枪匠 moses | 弱点反击:按下主动进入 **30 秒待命**,待命期内攻击**符合骰神赐福触发条件的目标**(近战武器 + `isBlessingTarget`,不额外限制"普通敌对生物")施加破绽 2:00(未触发不冷却;被动精密技巧使冷却 120 秒);2.0.0 开发线(`multi-dev-next` / `wt/2.0.0-vnext`)改经目标选择器选目标 | 弱点识破:任意来源闪避/反击或攻击带破绽目标获得 1 层(每目标闪避/反击限一次,最多 4 层);每层攻击/防御 +1、骰点最低数 +1;赐福结束减 1 层 |
