...
**技能类伤害类型(2026-09-24 新增)**:`astral_dice:skill_damage`(`<子项目>/src/main/resources/data/astral_dice/damage_type/skill_damage.json`,三线同源,内容与 `card_spell` 同构),同样登记进 `bypasses_armor.json`(⇒ 无视护甲值 / 盔甲韧性),**不**登记 `bypasses_cooldown`;⚠️ **绝对不得加入法伤白名单**(`SpellDamageRegistry.MAGIC_DAMAGE_TYPES` 或任何 matcher —— 该文件里已留警告注释)。用途 = 立牌 / 技能的**固定点数**伤害(当前唯一使用者:怪力侦探「怪力投掷」落地结算 `SherryThrowManager#settle`)。用户裁决原话:「为技能类伤害创建单独的伤害标签,避免与法伤混用」。**三种自定义类型别混选**:`card_spell`(法伤 —— 要吃修饰器链) / `skill_damage`(技能类固定点数 —— 不吃) / `true_damage`(真伤 —— 修饰器加成的独立结算与各类 AOE 波及,且穿透无敌帧)。
