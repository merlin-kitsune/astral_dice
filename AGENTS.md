...
- **贴图全部自绘**（不复制第三方像素）：钱包按钮为裸美术 18×20；星币/星币袋按钮 13×13 用**原版槽位风格边框**
  （黑描边 + 左上白高光 + `C6C6C6` 填充 + 右下暗边 `555555`，圆角 2），中心放货币图标 ——
...
- ⚠️ 图标是像素画，缩小会糊：生成脚本一律「LANCZOS 下采样 → **量化回源图调色板** → alpha 二值化」，
  否则棋盘格状中间色会让它看起来发虚。生成器：`temp/wallet_gui_tex.py`（产物 7 张，两版逐字节一致）。
