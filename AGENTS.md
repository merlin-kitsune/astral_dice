...
- 数据组件注册 id(共 11 个,完整以 `component/ModDataComponents.java` 为准):`temporary_card`(**临时牌标记**,绿洲女王立牌 nardis 批新增:Boolean,1.21.1 走 `DataComponentType`、1.20.1 走 `ItemDataKey`;装配进骰子后改由 `AppliedStone.temporary()` 承载并由 `CardInventoryMenu` 双向透传)、`misaki_sign_stacks`、`card_uses`(战斗牌剩余次数)、`weapon_enhancement`(**骰子**铁砧升星 + 已装配卡牌/费用配置;立牌无此组件)、`owner_uuid`(专属牌绑定)、`jasmine_atk_bonus`/`jasmine_def_bonus`、`padman_atk_bonus`/`padman_def_bonus`/`padman_last_refresh`/`padman_force_six`
