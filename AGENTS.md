...
8. **Curios 15 的饰品栏尺寸只能经「槽位修饰符 → `update()`」改写,禁止直接 `getStacks().grow/shrink`(2026-09-17 实测崩服,必须遵守)**:1.21.1 用的是 `ICurioStacksHandler#grow/shrink`(由 `CurioStacksHandler` **同时**调整 `stacks` 与 `cosmeticStacks`);Curios 15 已把这两个方法从接口移除(只留在 `IDynamicStackHandler` 上),移植时若图省事写成
   ...
   ⚠️ **【未解决缺陷】26.1.2 上筹码在重登后被移出栏位**:存档里 chip 栏位确实有该筹码(已解开玩家 NBT 逐字节确认,同栏位还带我们的 permanent 修饰符),但重登后 `chipItem=none`、物品回到玩家背包(不丢、槽位数仍正确 `chipSlots=chipCosmetic=目标值`)。已做对照并排除:**标签合法性**(换成 `curios:chip` 标签内的 `flashlight_chip` 同样复现)、**数据包 chip 槽尺寸 0/1**、**修饰符 transient/permanent**、**本模组代码弹出**(主动弹出会打 `setSlotCount ... 槽位物品被移出并归还背包` 警告,日志 0 条)。疑点在 Curios 登录迁移 `CurioInventory#loadInventoryConfiguration`(每次进入世界 `onDatapackSync` 触发:先按数据包 `createDefaultInventory()` 重建默认栏位,再按**新栏位槽数**搬移旧物品,搬不进去的进 `invalidStacks` → 回背包)。复现/取证全文见 `docs/compat-26.1.2-neoforge.md` §7.6 与用例 `CHIP-RELOG-A/B-26.1.2`(B 保持 FAIL = 缺陷仍在)。⚠️ 探针 `equipslot` 是**直接写栏位、绕过校验**,测筹码必须用 `curios:chip` 标签内的物品(用 `blank_chip` 这类材料会造出假缺陷)。
