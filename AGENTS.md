...
| 贯穿之铳 | `piercing_gun_chip` | 金 | **佩戴时**对敌对目标造成的远程/魔法伤害额外增加目标防御力点数(**无「已使用伤害效果牌」前提**,2026-09-24 起与忍术飞镖同口径;仅保留「目标须为敌对目标」这一范围检查)(**公式 = 2 + min(护甲,20)÷2 + 1.4×韧性,结果向下取整**;与骰战 `defensePower` 前三项逐字同构、与骰战界面显示口径一致;不含防御骰/防御卡。护甲上限 20 与骰战一致,来源为原版 `CombatRules.MAX_ARMOR`) |
