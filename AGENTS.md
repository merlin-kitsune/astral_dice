...
| 紫色飞星 | `purple_shooting_star_chip` | 紫 | 路过**敌对目标**且不对其发动攻击时,使其受到 **1 点**伤害(真伤 `astral_dice:true_damage`,忽略护甲值/盔甲韧性;击杀归属玩家)并使自身星光 **+1**;判定半径 3 格,「不对其发动攻击」= 该目标最近一次受伤非来自本玩家(窗口 = 冷却同周期 200 tick)。粒子自目标**头顶上方**下落(行程 = 旧口径的 **3.6 倍**〔历史 1.2 × 高度 3〕、**下落 10 tick = 0.5 秒** ⇒ 速度 = 原口径的 **6 倍**;落体几何入口 `fallDistance`/`launchOrigin`/`impactPoint`),**落到目标头顶(碰撞箱上沿)即视为命中**并结算;**每 10 秒触发一次**。⚠️ **只对敌对目标生效**(2026-09-24 裁决):中立生物须**已被激怒**才触发(判据 `isStarTarget`) |
