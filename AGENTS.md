...
6. **现有克隆（只读，可直接复用）— 优化/渲染类**：ImmediatelyFast 仓库 <https://github.com/RaphiMC/ImmediatelyFast> ⇒ 克隆点 `F:\MCProject\temp_immediatelyfast\26.1`（目标版本 = 本仓 `run\26.1.2\mods` 内 `ImmediatelyFast-NeoForge-1.15.3+26.1.jar`；引用时同样写「分支 + commit + 版本号」）；Iris 仓库 <https://github.com/IrisShaders/Iris> ⇒ 克隆点 `F:\MCProject\temp_iris\26.1`（26.1.2 对应分支）。这两处是为定位 **KNOWN-ISSUES KI-D1**（26.1.2 + 光影 + ImmediatelyFast 的 `Missing sampler Sampler1` 崩溃）而取源，规则同第 1–4 条（克隆在仓库之外、经代理、按版本、结论带 `文件:行号 + 原文片段`）。
...
9. **提示分工**：中央 HUD **只有一行**「目标名 + 距离 + 类型标签」（`client/TargetSelectOverlay.java#render(...)` 的 `guiGraphics.drawString` 行，整行组成见上方「架构」的 `client/TargetSelectOverlay` 条），**F1 隐藏 HUD 时整层不画**（见第 13 条）；其余提示**每 tick 经 `ActionBarManager.show(component, 40)` 走 actionbar**（`TargetSelectionClient.java#refreshActionBarPrompt(...)`，内调 `ActionBarManager.show(prompt, ACTIONBAR_TICKS)`）—— 稳态 = 四态 `prompt.{valid|rejected|no_target_self|no_target}` + 末尾黄色 `time`（见第 1/2 条与第 9 条上方「架构」），瞬态 = `self_unsupported` / `cancelled` 等 40 tick 点击反馈（**不带**剩余时间，F-V2 裁决）。⚠️ `msg.astral_dice.target_select.active` 与 `…rejected` 现为**死键（源码零引用）**，lang 条目保留仅为兼容，**不得**再当作「默认常驻提示」引用（2026-09-18 t46b 按 R2-01 更正）。
...
12. **立牌门控 `SignSelectionGate` 与可选中判定 `SelectorTargets` 都属现行口径（移植必须一并带上；2026-09-18 t26 登记门控、t46 补可选中判定与 `SelfTargetable`）**：`target/` 包现行**四个文件** —— `TargetSelectionManager.java`、`SignSelectionGate.java`、`SelectorTargets.java`、`SelfTargetable.java`（`SignSelectionGate` 与 `SelectorTargets` 都曾被本文档漏写：**移植者据旧文会漏掉门控、或让可选中判定退回裸 `instanceof Enemy`**）。三个「选择器类」立牌（占星师 `haiqing_weak_mark` / 秘密侦探 `bonnie_undercover` / 枪匠）走「先开会话、确认后才施效」两段式：按下主动键时 `BaseSignItem.performSkill` 的第 2.5 步只做 `TargetSelectionManager.start` + `SignSelectionGate.arm(...)` 登记待执行动作并立即 `return`（**不施效/不发牌/不进冷却/不充能**）；`TargetSelectionManager#confirm` 校验通过后调用动作 `apply`（效果/冷却/充能由动作自写），再由恢复点 `BaseSignItem.resumeGatedActiveSkill` 完成「风扇筹码发牌 + 抛 `SignActiveTriggeredEvent`」；取消/超时/会话被替换/登出/死亡 ⇒ 待执行记录清除、该次主动等同**未使用**。移植时**四个部件都要**：`SignSelectionGate`（门控登记处）、`BaseSignItem` 的第 2.5 步门控 + `resumeGatedActiveSkill`（恢复点）、三个立牌的 `selectorActionId()` 覆写、**`SelectorTargets`（可选中判定唯一入口，三处调用点 = 客户端准星判定 / 客户端半径内高亮 / 服务端 `TargetSelectionManager#confirm`）**；**缺 `SignSelectionGate` 会让立牌退化为「按下即施效」；缺 `SelectorTargets` 会让判定退回裸 `instanceof Enemy` ⇒ 被激怒的中立生物不可选（`f6f03a3` 刚修掉的缺陷会在移植线重现）**。另有 `SelfTargetable`（`allowSelf()` 缺省 `false` 的可选接口，`TargetSelectStartPayload.allowSelf` 的来源）—— **当前无任何动作实现它**，移植时带上即可、玩家可见行为不变。两发布线现存实现对等（`SignSelectionGate` 两线均 2975 字节、内容同构）。
