...
| 教主 teru | 降神(**目标选择器类**,只能选**其它玩家**,不能选生物与自身):锁定目标 50% 攻击力与防御力加给自己(施法瞬间快照),目标每攻击一个新目标消耗 1 层狐光并按「狐光攻击基数 + 消耗后剩余层数」追加一次攻击力;持续到**该目标自己的下一次骰神赐福结束**,生效期间无法再次施放 | 狐光:层数资源(上限 20,**跨死亡保留**);自身(以及降神目标)合成/获得/装备攻击牌 +1 层/张,施放降神立即 +3 层;**防刷**:拾取地面上的攻击牌不计层、反复把同一张战斗牌插入骰子卡牌栏再卸除也不计层(按「历史同时装备张数」去重) |
