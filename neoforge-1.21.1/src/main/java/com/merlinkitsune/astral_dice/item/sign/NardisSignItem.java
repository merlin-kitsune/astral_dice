...
public class NardisSignItem extends BaseSignItem {
    ...
    protected InteractionResultHolder<ItemStack> handleUse(Level level, Player player, ItemStack stack) {
        ...
        // 3. 有效期**重置**为 3:00:六参且 showIcon=true(照 JasmineSignItem:63)——
     ...
     * (1.21.1 实测 {@code MobEffectInstance.java:138-181}:只有"新时长**严格更长**且放大器相同"
     ...
     * {@code CLAMP_TOLERANCE = 20} 就把实例移除重加、裁回记账值**(1.21.1 实测
     * {@code EffectTimerGuard.java:136-141} 的"减速"分支)⇒ 只把实例拉长、不同步记账,下一 tick 就会被打回旧值。
     * 故这里三步一起做:
     * <ol>
     *   <li><b>效果已存在</b> ⇒ {@link Player#forceAddEffect}:它把 {@code activeEffects} 里的实例
     *       **无条件替换**为新实例(1.21.1 实测 {@code LivingEntity.java:1014-1024}:
     *       {@code activeEffects.put(...)} + {@code onEffectUpdated}),**不经过任何合并/取长规则**
     *       ⇒ 替换后剩余时长**必然**是 3600,与旧实例剩多少无关。同一条 API 也是原版客户端处理
     *       {@code ClientboundUpdateMobEffectPacket} 时用的写入途径(1.21.1 实测
     *       {@code ClientPacketListener.java:1668-1685})⇒ 服务端/客户端看到的剩余时长一致;</li>
     *   <li><b>效果不存在</b> ⇒ {@link EffectTimerGuard#apply}(常规施加,与既有实现逐字一致);
     *       但 {@code forceAddEffect} **不**触发 {@code MobEffectEvent.Added}
     *       (1.21.1 实测 {@code LivingEntity.java:1014-1024} 里没有 post),而本仓的守卫记录钩子
     *       {@code event/ModEffectEvents#onEffectTimerRecord} 正是挂在该事件上 ⇒ 必须走第 3 步;</li>
     *   <li><b>守卫记账同步</b> ⇒ 显式 {@link EffectTimerGuard#record}(先例:
     *       {@code item/RenShieldManager#refreshShieldEffects} 的「addEffect + record」延长写法)。
     *       记账取 {@code max(旧 endTick, now + 3600)},而本立牌自己的旧记账必然
     *       {@code ≤ now + 3600}(3600 就是本效果的最大时长)⇒ 结果恒为 {@code now + 3600}。</li>
     * </ol>
     *
     ...
     * {@code EffectTimerGuard.clear(player)} 清空全部记账(1.21.1 实测 {@code PlayerLifecycleHandler.java:232}),
    ...
    private static void applyPrivilegeDuration(Player player) {
        MobEffectInstance refreshed = new MobEffectInstance(ModEffects.NARDIS_PRIVILEGE,
                NardisPrivilegeEffect.DURATION_TICKS, 0, false, false, true);
        if (player.hasEffect(ModEffects.NARDIS_PRIVILEGE)) {
