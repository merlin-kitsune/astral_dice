...
/**
 * 「人偶完成」:人偶师立牌(hanna)被动「幻想千金」的资源进入**完成态**后的常驻状态载体。
 *
 * <p>触发链(用户 2026-09-21 技能原文):「人偶制作」达到 {@code HannaSignItem.MAX_CRAFT} = 7 层 ⇒
 * **层数归零**并转换为本状态(用户裁决四选一:「归零并转为『人偶完成』状态」);
 * 此后路过友方玩家时,**额外**使该玩家获得 迅捷 II (1:00) 与 3 星币。
 *
 * <p>既然状态本身没有时长(完成态不可逆、也不随死亡以外的条件解除),本效果**无限时长**且
 * **不登记 {@code EffectTimerGuard}**(照 teru 狐光/降神、mamushi 真龙形态、zhao 白泽赐福的口径)。
 * 卸下立牌时由 {@code HannaSignItem#clearSignData} 一并移除。
 *
...
public class HannaDollCompleteEffect extends MobEffect {
    ...
    public static boolean has(Player player) {
        return player != null && player.hasEffect(ModEffects.HANNA_DOLL_COMPLETE);
    }
}
