 ...
 *
 * <p><b>满层转换</b>:层数达 7 时由 {@code HannaSignItem#addCraftLayers} 归零本效果并改挂
...
public class HannaDollCraftEffect extends MobEffect {
    ...
    public static int getStacks(Player player) {
        ...
        MobEffectInstance instance = player.getEffect(ModEffects.HANNA_DOLL_CRAFT);
    ...
    public static void mirror(Player player, int count) {
        ...
        ModEffectRemoval.remove(player, ModEffects.HANNA_DOLL_CRAFT);
        if (count > 0) {
            player.addEffect(new MobEffectInstance(ModEffects.HANNA_DOLL_CRAFT,
