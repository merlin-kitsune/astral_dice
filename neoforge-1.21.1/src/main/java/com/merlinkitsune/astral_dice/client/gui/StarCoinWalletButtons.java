...
/**
 * 星币钱包的三个物品栏按钮（NeoForge 1.21.1 客户端）。
 *
 * <p>不建独立界面：沿用「把按钮注入原版物品栏界面」的做法，在 {@link ScreenEvent.Init.Post}
 * 时给 {@link InventoryScreen} 与 {@link CreativeModeInventoryScreen} 各挂三个按钮；
 * 点击只发**意图包**（C2S），一切判定在服务端（见 {@link StarCoinWalletActions}）。
 *
 * <p><b>基座坐标与尺寸与 Magic Coins 的同类按钮逐像素一致</b>，且**生存与创造各一套**基座
 * （创造栏的物品栏标签页整体右移、顶部有标签栏，故 y 为更小的负值）。坐标都是相对界面左上角
 * （{@code guiLeft/guiTop}）的偏移：
 * <pre>
 *   按钮        尺寸    生存 (x, y)   创造 (x, y)
 ...
 *
 * <p><b>创造栏的动态处理</b>：只有「物品栏」标签页显示按钮（切到建筑/装饰等标签页即隐藏），
 * 且 {@code guiLeft/guiTop} 随标签页变化 ⇒ 位置在**每帧渲染时**重算，而不是初始化时定死。
 *
 * <p>钱包功能关闭（{@code enable_star_coin_wallet = false}）时**一个按钮都不创建**，
 * 界面与改动前完全一致。
 */
