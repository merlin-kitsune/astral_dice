 ...
 * (第一行判定,实测 1.21.1 {@code ServerPlayer.java:2053})。而 {@code ItemTossEvent} 覆盖的是
 * **更宽的集合**:① Q 键、② **把栈拖拽到物品栏 GUI 之外**(原版 javadoc 原话
 * "or drag-n-drops a stack of items outside the inventory GUI screens")、③ 其它模组直接调
 * {@code Player#drop(ItemStack, boolean)} 的路径。临时牌的语义是「只能在物品栏和手中」,
 * 这些口子全部要堵住。
 *
 * <h2>时序与「尽力退还」</h2>
